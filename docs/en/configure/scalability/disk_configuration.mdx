...
| Spec | Minimum requirements | Recommended | Notes |
|---|---|---|---|
| sequential IOPS | 50 | 500, higher is better | Most cloud providers publish concurrent IOPS rather than sequential IOPS; the published concurrent IOPS can be 10x greater than the sequential IOPS. |
| disk bandwidth | 10MB/s | 100MB/s, higher is better | More disk bandwidth buys faster recovery times when a failed member has to catch up with the cluster. |
| throughput (sequential 8K write with fdatasync) | 50 times in 10ms | 500 times in 2ms | |
