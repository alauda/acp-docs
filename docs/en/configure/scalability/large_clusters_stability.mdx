...
| Node reporting intervals | Default node reporting can create churn in very large clusters. | Recommended: kubelet `node-status-update-frequency: 1m`; controller-manager `node-monitor-grace-period: 5m`; API server tolerations `default-not-ready-toleration-seconds: 60` and `default-unreachable-toleration-seconds: 60`.
