...
---
This guide helps cluster operators and SREs reduce control-plane overload, improve reliability, and limit blast radius in very large Kubernetes clusters.
...
| Node reporting intervals | Default node reporting can create churn in very large clusters. | Recommended: kubelet `node-status-update-frequency: 1m`; controller-manager `node-monitor-grace-period: 5m`; API server tolerations `default-not-ready-toleration-seconds: 60` and `default-unreachable-toleration-seconds: 60`. [4]
...
4. https://docs.redhat.com/en/documentation/openshift_container_platform/4.19/html/scalability_and_performance/scaling-worker-latency-profiles#scaling-worker-latency-profiles
