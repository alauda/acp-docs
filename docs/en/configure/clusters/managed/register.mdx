...
* Depending on the type of managed cluster, the versions and parameters of Kubernetes and other components on the managed cluster must meet the [Version and Parameter Requirements for Managed Clusters](/overview/kubernetes-support-matrix.mdx#version-support-matrix).
