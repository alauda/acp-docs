...
description: "Import an existing CCE cluster as a third-party cluster."
weight: 45
---
...
2. If the test fails, see [How to trust an insecure image registry?](/configure/clusters/managed/how-to/image-repo-trust.mdx).
...
See [How to obtain cluster information?](/configure/clusters/managed/how-to/fetch-kubeconfig.mdx) to obtain the import cluster token.
...
3. Configure the `Image Registry` related parameters according to the following instructions.
    ...
    | ------ | -------- |
    |  **Image Registry**  | Repository for storing platform component images required by the cluster.<br />**- Platform Default**: Image registry configured during global cluster deployment.<br />**- Private Registry**: Pre-built registry storing platform required components. Enter the **private image registry address**, **port**, **username**, and **password** for accessing the image registry.<br />**- Public Registry**: Use image registry services located on the public network. Before use, update [public image registry cloud credentials](/configure/clusters/how-to/update_public_repository_credentials.mdx) to obtain registry authentication permissions. |
...
6. After connectivity check passes, click **Import** and confirm.
    ...
    * Click the ![](/00phase.png) icon on the right side of a cluster in **Importing** status to view the cluster's execution progress (status.conditions) in the popup **Execution Progress** dialog.
...
To ensure network connectivity between the global cluster and the imported cluster, see [Imported Cluster Network Configuration](/configure/clusters/managed/how-to/annotate-platform-url.mdx).

## Follow-up Operations

### Ingress (Inbound Rules) and Storage Initialization

After importing the cluster, if you need to use Ingress (inbound rules) and storage-related features, see [Huawei Cloud CCE Cluster Ingress Initialization Configuration](/configure/clusters/managed/cloud-init/network/huawei-cce.mdx) and [Huawei Cloud CCE Cluster Storage Initialization Configuration](/configure/clusters/managed/cloud-init/storage/huawei-cce.mdx).
