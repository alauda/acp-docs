...
The load balancer must correctly forward traffic to ports `6443` and `11780` on all control plane nodes in the cluster.
...
1. Provide `External Access` for the workload cluster to ensure the `global` cluster can access it. Network requirements must ensure `global` can access ports `6443` and `11780` on all control plane nodes.
