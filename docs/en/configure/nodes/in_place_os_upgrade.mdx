---
title: "Plan an In-Place OS Upgrade for Nodes"
description: "Plan and verify an in-place operating system upgrade on self-built cluster nodes."
weight: 22
author: dev@alauda.io
category: howto
queries:
  - in-place OS upgrade for platform nodes
  - upgrade operating system on self-built cluster nodes
  - verify platform nodes after OS upgrade
  - node OS upgrade checklist
---

# Plan an In-Place OS Upgrade for Nodes

Cluster administrators can use this checklist to plan an in-place operating system upgrade for nodes in <Term name="productShort" /> self-built clusters. Complete the <Term name="productShort" /> checks before and after the operating system changes.

<Directive type="warning" title="Scope and support boundary">
Use this checklist together with the operating system provider's supported upgrade procedure during a maintenance window. Confirm the target operating system and kernel against the <Term name="productShort" /> support matrix before you upgrade any node. The checklist does not guarantee that every operating system upgrade path is supported.
</Directive>
...
- Clusters that use [Immutable Infrastructure](../clusters/immutable-infra.mdx). Apply operating system changes by replacing nodes with new images.
...
### Step 3: Record the current node baseline

Record the node state before the operating system upgrade. Use the records for comparison after the node is upgraded.

Run the `Base node configuration check` block from [Step 2: Verify base node configuration](#verify_base_node_configuration) on the node now, and keep its output. It captures the SELinux mode, the AppArmor and host firewall state, and the firewall rules in use. The post-upgrade checks compare against exactly this output, so a node without it cannot be verified.

Record the runtime and node component versions as well:
...
You can use the <Term name="productShort" /> console to evict Pods from the node. For the console operation, see [Manage Nodes](./node_management.mdx).
...
### Step 2: Verify base node configuration \{#verify_base_node_configuration}
...
# SELinux mode. Both Disabled and Enforcing are valid states; the value has to
# match the one recorded before the upgrade.
if command -v getenforce >/dev/null; then
  echo "SELinux: $(getenforce)"
else
  echo "SELinux: getenforce not available on this system"
fi

# AppArmor. A missing unit means the system has no AppArmor at all, which is not
# the same as AppArmor being present but turned off.
if systemctl list-unit-files --no-legend apparmor.service 2>/dev/null | grep -q .; then
  echo "AppArmor: enabled=$(systemctl is-enabled apparmor.service 2>&1) active=$(systemctl is-active apparmor.service 2>&1)"
else
  echo "AppArmor: unit not present on this system"
fi

# firewalld. Same distinction as AppArmor.
if systemctl list-unit-files --no-legend firewalld.service 2>/dev/null | grep -q .; then
  echo "firewalld: enabled=$(systemctl is-enabled firewalld.service 2>&1) active=$(systemctl is-active firewalld.service 2>&1)"
  firewall-cmd --list-all 2>/dev/null || true
else
  echo "firewalld: unit not present on this system"
fi

# ufw. The unit reports active (exited) once it has ever run, so the unit state
# does not show whether the ruleset is enabled. Only ufw status does.
...
SELinux, AppArmor, and the host firewall must return to the state that the cluster was installed with. If the cluster was installed with these mechanisms disabled and the operating system upgrade re-enabled any of them, disable it again. If the cluster runs with any of them enabled, confirm that it is still enabled and that its policy and firewall rules are complete after the upgrade. For the required configuration and the port list, see [Hardened Node Security Configuration](/install/prepare/node_preprocessing.mdx#hardened_node_security) and [Communication Matrix](/install/prepare/communication_matrix.mdx).
...
You can also resume scheduling from the <Term name="productShort" /> console. For the console operation, see [Manage Nodes](./node_management.mdx).
...
| `kubelet` fails with security policy errors | The mechanism was turned on or off by the operating system upgrade, or the vendor shipped a new policy and the file labels no longer match it | Run the `Base node configuration check` block and compare it with the pre-upgrade record, then list the denials with `ausearch -m avc -ts boot` | If the state changed, restore the state the cluster was installed with. If the state is unchanged, the policy or the file labels changed: restore the labels on the paths named in the denials with `restorecon -Rv <path>`, or run `touch /.autorelabel` and reboot if the labels are wrong across the file system. Contact technical support if the denials persist |
...
| Pre-upgrade | Record whether SELinux, AppArmor, and the host firewall are enabled, and record the firewall rules in use | | |
...
| Post-upgrade | Confirm that SELinux and AppArmor match the cluster installation configuration | | |
| Post-upgrade | Confirm swap is disabled | | |
| Post-upgrade | Confirm that the host firewall matches the cluster installation configuration | | |
