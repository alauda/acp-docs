...
# SELinux must be disabled. Record the value so that you can confirm the
# operating system upgrade did not turn it back on.
...
# AppArmor must be disabled. A missing unit means the system has no AppArmor at
# all, which is not the same as AppArmor being present but turned off.
...
SELinux and AppArmor must be disabled. If the operating system upgrade re-enabled either of them, disable it again according to your operating system policy. See [Why SELinux and AppArmor Must Be Disabled](/install/prepare/node_preprocessing.mdx#selinux_apparmor_disabled).

The host firewall must return to the state that the cluster was installed with. If the cluster was installed with the host firewall disabled and the operating system upgrade re-enabled it, disable it again. If the cluster runs with the host firewall enabled, confirm that it is still enabled and that its rules are complete after the upgrade. For the required configuration and the port list, see [Hardened Node Security Configuration](/install/prepare/node_preprocessing.mdx#hardened_node_security) and [Communication Matrix](/install/prepare/communication_matrix.mdx).
...
| `kubelet` fails with security policy errors | SELinux or AppArmor was re-enabled by the operating system upgrade | Run the `Base node configuration check` block and compare it with the pre-upgrade record | Disable the mechanism again according to your operating system policy, then reboot the node if the operating system requires it. Contact technical support if the errors persist |
...
| Post-upgrade | Confirm SELinux and AppArmor are disabled | | |
