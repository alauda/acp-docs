...
3. Locate the **<Term name="product" /> Snapshot Management** cluster plugin, click **Install**, and wait until deployment succeeds.

## Deploying via YAML

For YAML installation, see [Installing via YAML](../../../extend/cluster_plugin.mdx#installing-via-yaml).

**<Term name="product" /> Snapshot Management** is a **Non-config plugin**, and the module name is `snapshot`.
