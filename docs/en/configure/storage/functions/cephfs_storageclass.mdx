...
After clicking **Deploy** on the **Distributed Storage** page, choose either [Deploying in Internal Mode](/storage/storagesystem_ceph/installation/create_service_stand.mdx) or [Deploying in External Mode](/storage/storagesystem_ceph/installation/access_storage_service.mdx).
...
5. Configure the relevant parameters according to the following instructions.
   ...
   | **Allocate Project** | Allocate the projects that can use this type of storage.<br />If no project currently needs this type of storage, leave the allocation empty and update it later.                                                                                   |
