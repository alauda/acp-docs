...
Use local storage only for applications that provide application-layer data replication and backup. Local storage does not provide an independent data persistence guarantee when the underlying node fails.

For TopoLVM behavior, see the [TopoLVM user manual](https://github.com/topolvm/topolvm/blob/main/docs/user-manual.md).

## Deploy Volume Plugin

After clicking deploy, on the newly opened page [Install LVM Storage](/storage/storagesystem_topolvm/installation.mdx).
