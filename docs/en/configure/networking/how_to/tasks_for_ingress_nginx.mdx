...
## Prerequisites
[install ingress-nginx](./install_ingress_nginx_via_ingress_nginx_operator.mdx)
...
[add pod annotation](../functions/endpoint_health_checker.mdx#add_pod_annotation_in_ingress_nginx)

## How To Set NodeSelector And Tolerations For  ingress-nginx-operator
...
```
## How To Set NodeSelector And Tolerations For  ingress-nginx
update the `IngressNginx` resources 
```bash
# $NAME is the deployed ingress-nginx, in default ns is ingress-nginx-operator
kubectl patch IngressNginx $NAME -n $NS --type='merge' -p '
{
  "spec": {
    "controller": {
      "nodeSelector": {
