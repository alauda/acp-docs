...
This folder conatains a dashboard named **DeepFlow Agent**, which provides insights into the status and performance of nodes where DeepFlow agents are deployed.
...
DeepFlow supports parsing unique Request IDs injected by applications (e.g., almost all gateways inject X-Request-ID) and associating diffrent requests with the same Request ID into a single trace.
