...
- **Ensure that an operable ALB exists**: Create or use an existing ALB, where the name of the ALB is replaced with `<otel-alb>` in this document. For instructions on creating an ALB, refer to [Deploy ALB](./deploy_alb.mdx).
