...
4. Follow the instructions below to complete the network configuration:
   ...
   | **GatewayClass**  | The embedded `exclusive-gateway` is provided by Alauda Container Platform and backed by ALB. It will create a container-network-mode ALB to implement the GatewayAPI gateway specification.            |
   | **Specification** | Set the specifications appropriately based on your business needs. You can also refer to [How to properly allocate CPU and memory resources](../how_to/decide_performance_selection.mdx) for guidance. |
