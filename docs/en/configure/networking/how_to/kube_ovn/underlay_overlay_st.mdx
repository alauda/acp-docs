...
The default value of `--ls-ct-skip-dst-lport-ips` is `true`. For Underlay subnets with ACL-based isolation, you **must** set it to `false`. This is because:

Gateway-to-Pod traffic will fail (e.g., ping requests reach the Pod but replies are dropped), as ACL isolation uses `allow-related` which requires conntrack state — without it, replies cannot be identified as "related" and get dropped.
