...
The default value of `--ls-ct-skip-dst-lport-ips` is `true`. For Underlay subnets with ACL-based isolation, you **must** set it to `false`. This is because ACL rules require connection tracking (conntrack) to work properly, and skipping conntrack in Underlay scenarios would bypass the ACL enforcement.
