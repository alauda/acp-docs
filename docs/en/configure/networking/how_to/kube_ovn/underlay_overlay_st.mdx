...
With ACP 4.3 defaults, enabling `u2oInterconnection` does not automatically route traffic from cluster nodes to Underlay Pods through `ovn0`:
...
| `priority` | Rule priority (higher values are evaluated first; recommended range: 1002-1899) |
