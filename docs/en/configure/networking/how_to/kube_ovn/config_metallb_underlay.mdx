...
> ⚠️ **Critical**: The LoadBalancer VIP and the backend Pod IPs **must be in the same Underlay subnet**. MetalLB L2 mode advertises the VIP via ARP on the local network, and OVN performs DNAT to forward traffic directly to backend Pods. If the VIP and Pod IPs belong to different subnets, the ARP response and DNAT routing will fail, causing the service to be unreachable.
