...
### PolicyAttachment Via TargetRefs \{#policyattachment-via-targetrefs}
...
We recommend using [EnvoyGatewayCtl](../../../networking/operators/envoy_gateway_operator.mdx#envoygatewayctl) to manage those global configurations.
...
- Accessing gateway via node IP directly
...
There are two approaches to configure hostNetwork mode, depending on whether you want to use privileged ports (< 1024) directly:
...
- More secure - runs as non-root user without additional privileges
...
**Pros:**
- Can use standard ports (80, 443) directly
