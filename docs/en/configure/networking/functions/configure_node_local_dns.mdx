...
5. **Persistence across node rebuilds and upgrades**: When a node is rebuilt or the cluster is upgraded, kubelet configuration changes are lost. To make the NodeLocal DNS `clusterDNS` setting persistent, configure it according to the node operating system.

   **Immutable Infrastructure**: configure `clusterDNS` through Machine Configuration with a kubelet configuration drop-in. See <ExternalSiteLink name="immutable-infra" href="/machine-configuration/kubelet_config.html" children="Configuring Kubelet" />.
