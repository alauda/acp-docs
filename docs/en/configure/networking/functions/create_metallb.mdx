...
4. In the deployment configuration dialog, you can optionally configure the following parameters:
   ...
   **Note**: 
   - If no node selectors are specified, MetalLB components will be scheduled based on default scheduling rules.
   - If no tolerations are specified, MetalLB components will not be scheduled on nodes with taints.
