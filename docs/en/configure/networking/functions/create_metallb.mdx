...
4. In the deployment configuration dialog, you can optionally configure the following parameters:
   ...
   | --- | --- |
   | **metallb Node Selectors** | Configure label selectors to specify which nodes the MetalLB components (operator, controller and speaker) should run on. Click **Add** to add multiple label key-value pairs. |
   | **metallb Node Tolerations** | Configure tolerations to allow MetalLB components to be scheduled on nodes with specific taints. Click **Add** to add multiple tolerations with Key, Value, and Type. |

   **Note**: 
   - These configurations will be applied to MetalLB components (operator, controller, and speaker). If not specified, components will use their default scheduling configurations.
   - The configured Metallb node selectors and tolerations will be merged with the default settings.

5. Click **Install** to deploy the plugin.

6. Wait until the deployment status shows **Deployment Successful** to complete the deployment.
