...
### <span id="qtjc">Exit cluster interconnection</span>
...
2. Download the <a href="/scripts/clean-ic-az-db.sh" download="clean-ic-az-db.sh">cleanup script</a> and execute the relevant operations.

    * Using the Containerd tool, copy the script file to the interconnected cluster controller container with the following command.

        ```
        nerdctl --namespace k8s.io cp ./clean-ic-az-db.sh ovn-ic-db:/kube-ovn/
        ```

    * Enter the ovn-ic-db container and execute the `/bin/bash` command with the following command.

        ```
        ctr -n k8s.io task exec -t --exec-id ovn-ic-db ovn-ic-db /bin/bash
        ```

3. Perform the cleanup operation. There are two specific cleanup methods, choose one according to your needs.

    * Execute the cleanup operation with the name of the original cluster. Replace *\<cluster-name>* with the **name of the original cluster** in the following command.

        ```
        ./clean-ic-az-db.sh <cluster-name>
        ```

    * Execute the cleanup operation with the name of any node in the original cluster. Replace *\<node-name>* with the **name of any node in the original cluster** in the following command.
