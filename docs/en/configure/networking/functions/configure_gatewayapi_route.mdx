...
## Prerequisites
Install a `Gateway`, please follow the instructions in the [installation guide](./configure_gatewayapi_gateway.mdx).
...
It follows the [Hostname Intersection Rules](./configure_gatewayapi_gateway.mdx#intersection_rule).
...
| Object | Method | Value Types | Description | Value Requirements | Max Items |
|-----------|------------|-------------|-------------|-------------------|-----------|
| **Path** | `Exact` | path (string) | Exact path match (case-sensitive) | Must start with `/`, no consecutive `//` | - |
| | `PathPrefix` | path (string) | Prefix match on path elements | Must start with `/`, no consecutive `//` | - |
| | `RegularExpression` | path (string) | Regular expression match | Implementation-specific (POSIX, PCRE, RE2, etc.) | - |
| **Header** | `Exact` | name (header key) + value | Exact header value match (case-insensitive header name) | Header value: 1-4096 characters | 16 |
| | `RegularExpression` | name (header key) + value | Regular expression match on header value | Implementation-specific | 16 |
| **QueryParam** | `Exact` | name (param key) + value | Exact query parameter value match | Parameter value: 1-1024 characters | 16 |
| | `RegularExpression` | name (param key) + value | Regular expression match on parameter value | Implementation-specific | 16 |
...
## Related Tasks
- [how to attach to listener created in other namespace](../how_to/tasks_for_envoy_gateway#attach_to_gateway_create_on_other_ns)
