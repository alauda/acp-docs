...
**Note**: The container network in Kube-OVN Underlay requires support from the physical network. Please refer to the best practices [Preparing the Kube-OVN Underlay Physical Network](../how_to/kubeovn_underlay_py.mdx) to ensure network connectivity.
...
## (Optional) Add Bridge Network \{#kube-ovn_underlay_bridge_network}
...
**Note**: The platform also pre-configures a **join** subnet for communication between nodes and Pods in Overlay transport mode. This subnet will not be used in Underlay transport mode, so it is crucial to avoid IP segment conflicts between **join** and other subnets.
