...
- MetalLB selects an available node to host the VIP based on the [algorithm](#vip), which then forwards the traffic.
...
## MetalLB's Algorithm for Selecting VIP Host Nodes\{#vip}
