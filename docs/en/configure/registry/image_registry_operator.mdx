...
The Image Registry Operator installs and manages the cluster-wide <Term name="productShort" />
Registry instance. It reconciles registry runtime resources from `Config/cluster` and scheduled
pruning resources from `ImagePruner/cluster`.
...
Install the Image Registry Operator from the <Term name="productShort" /> web console when the Operator package is available in OperatorHub. Use the YAML path only for controlled automation or support-guided installation when the web console is not available.
