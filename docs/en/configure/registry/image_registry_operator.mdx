...
violet push cluster-image-registry-operator.stable.amd64.<version>.tgz \
  ...
  --image-pull-secret="<platform-pull-secret>"
```

The push must use a registry account with `push` permission on the target repositories. An account
with pull-only access fails with `requested access to the resource is denied`. The
`--image-pull-secret` value must name a Secret that exists in the namespace where the Operator runs.
