...
Both paths require the `cluster-image-registry-operator` package to be present in the target
CatalogSource. A fresh platform installation does not include it. If the package is not present,
follow the platform's [Operator package upload procedure](../../extend/operator.mdx) before
continuing, and see [Install by Using YAML](#install-by-using-yaml) for a worked example.
...
If it is missing, upload the Operator package first. For example:
...
Before upgrading, confirm that the current Registry is healthy. For Alauda Container Platform
Registry versions earlier than `v4.4`, contact technical support for upgrade and migration plans.
