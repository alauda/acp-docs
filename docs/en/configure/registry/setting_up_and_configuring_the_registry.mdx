...
Use this page to configure storage, Registry request behavior, service account pull credentials, image
limits, and scheduled image cleanup for Alauda Container Platform Registry.
...
```bash {name=registry-config:patch-development-storage}
...
```bash {name=registry-config:verify-development-storage}
...
```yaml {name=registry-config:pvc-manifest}
apiVersion: v1
...
```bash {name=registry-config:apply-pvc}
...
```bash {name=registry-config:patch-pvc-storage}
kubectl patch configs.imageregistry.operator.alauda.io cluster \
  --type=merge \
  -p '{
    "spec": {
...
```bash {name=registry-config:verify-pvc-storage}
...
```bash {name=registry-config:create-s3-secret}
...
```bash {name=registry-config:verify-s3-secret}
...
```bash {name=registry-config:create-ca-configmap}
...
```bash {name=registry-config:verify-ca-configmap}
...
```bash {name=registry-config:patch-s3-storage}
kubectl patch configs.imageregistry.operator.alauda.io cluster \
  --type=merge \
  -p '{
    "spec": {
...
> **Note:** The `Config` CRD also exposes a `storage.oss` field, inherited from the upstream image
> registry operator. This operator does not implement it. Setting `storage.oss` is treated as "no
> storage backend configured": the Operator reports `Degraded=True` with reason
> `StorageNotConfigured`, and the Registry data plane does not reconcile. Use only the seven
> supported fields listed above.

Verify the configuration and rollout:

```bash {name=registry-config:verify-s3-storage}
kubectl get configs.imageregistry.operator.alauda.io cluster -o yaml
kubectl -n image-registry-system rollout status deployment/image-registry --timeout=300s
...
```bash {name=registry-config:patch-request-handling}
kubectl patch configs.imageregistry.operator.alauda.io cluster \
  --type=merge \
  -p '{
    "spec": {
...
```bash {name=registry-config:verify-request-handling}
kubectl get configs.imageregistry.operator.alauda.io cluster -o yaml
kubectl -n image-registry-system rollout status deployment/image-registry --timeout=300s
```
...
```bash {name=registry-config:patch-managed-pull-secrets}
kubectl patch configs.imageregistry.operator.alauda.io cluster \
  --type=merge \
  -p '{
    "spec": {
...
```bash {name=registry-config:verify-managed-pull-secrets}
kubectl get configs.imageregistry.operator.alauda.io cluster -o yaml
```
...
```bash {name=registry-config:check-workload-puller}
...
```yaml {name=registry-config:quota-manifest}
apiVersion: v1
kind: ResourceQuota
metadata:
...
```bash {name=registry-config:apply-quota}
...
```yaml {name=registry-config:limits-manifest}
apiVersion: v1
kind: LimitRange
metadata:
...
```bash {name=registry-config:apply-limits}
kubectl apply -f team-a-image-limits.yaml
```

Verify the quota and limits:

```bash {name=registry-config:verify-limits}
...
```yaml {name=registry-config:pruner-manifest}
...
```bash {name=registry-config:inspect-storage}
