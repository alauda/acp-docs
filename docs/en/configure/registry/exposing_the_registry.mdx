...
By default, use the internal service for workloads inside the cluster. Expose Alauda Container
Platform Registry only when developer machines, CI systems, or other external clients must push or
pull images.
...
```bash {name=exposing:enable-default-route}
...
```bash {name=exposing:verify-default-route}
...
```bash {name=exposing:resolve-default-host}
REGISTRY_RULE_HOST="$(
  kubectl -n image-registry-system get ingress default-route \
...
```bash {name=exposing:select-host}
...
```bash {name=exposing:login-default-host}
ac registry login \
...
```bash {name=exposing:create-tls-secret}
...
```bash {name=exposing:configure-custom-route}
kubectl patch configs.imageregistry.operator.alauda.io cluster \
  --type=merge \
  -p '{
    "spec": {
...
```bash {name=exposing:verify-custom-route}
kubectl get configs.imageregistry.operator.alauda.io cluster -o yaml
...
```bash {name=exposing:login-insecure}
ac registry login \
  --registry registry.example.com \
  --insecure
```
...
```bash {name=exposing:inspect-ingress}
...
```bash {name=exposing:login-secure}
ac registry login \
  --registry registry.example.com
```
...
```bash {name=exposing:external-push-pull}
