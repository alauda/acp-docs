...
# <Term name="product" /> Registry Data Backup and Recovery

## Overview

Use this procedure to back up and recover <Term name="productShort" /> Registry data when registry image data is stored in S3-compatible object storage.
...
* The <Term name="productShort" /> Registry Cluster Plugin is installed and configured, and its `ModuleInfo` resource exists and is in a healthy state.
...
Extract the S3 storage configuration from the `ModuleInfo` resource that manages the <Term name="productShort" /> Registry Cluster Plugin.
...
## Data Recovery
Before recovery, install the <Term name="productShort" /> Registry Cluster Plugin in the target environment, such as a new cluster or repaired cluster. Then modify the configuration so the registry can access the restored image data.
...
**Key point**: This operation triggers a rolling update of the <Term name="productShort" /> Registry Pods. Newly started Pods use the new configuration to connect to the specified target storage bucket.
