...
image limits, and scheduled image cleanup for the Operator-managed Registry.

## Prerequisites

- The Operator-managed Registry is installed, and `Config/cluster` exists.
...
In the Operator-managed Registry, image size and tag-count limits are represented with Kubernetes
`LimitRange` and `ResourceQuota` objects. Do not use the legacy Registry gateway limit ConfigMap for
Operator-managed Registry deployments.
