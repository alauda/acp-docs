...
The Image Registry Operator installs and manages the cluster-wide Operator-managed Registry
...
Do not use the legacy Registry Cluster Plugin to install the Operator-managed Registry. The legacy
Registry Cluster Plugin remains available only for legacy Registry deployments.
...
Setting the management state to `Removed` stops the Operator-managed Registry runtime components.
Push and pull traffic, the Image API server, and scheduled pruning are unavailable while the
Registry is removed. Use this state only during a maintenance window or for support-guided recovery.
...
The CronJob uses the Operator-managed Registry internal service URL by default.
