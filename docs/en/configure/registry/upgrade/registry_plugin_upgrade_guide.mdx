...
# Legacy Registry: Upgrade from ACP v4.1 or Earlier

## Prerequisites
1. Administrator privileges for <Term name="productShort" />.
2. The old plugin refers to versions of <Term name="productShort" /> v4.1 and earlier.
3. The new plugin refers to versions of <Term name="productShort" /> v4.2 and later.
...
This procedure is only for the legacy Registry Cluster Plugin name change from ACP v4.1 and earlier
to ACP v4.2 and later. It is not a migration path from the legacy Registry Cluster Plugin to the
