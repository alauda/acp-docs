...
- **Memory pressure** — the container gets `OOMKilled` and the Pod restarts (exit code 137).
- **CPU pressure** — under heavy churn such as bursts of Pod creation/deletion, large logical-flow updates, or a high packet rate, the container can hit its CPU limit. `ovn-controller` and `ovs-vswitchd` get throttled — the Pod is not killed, but CPU starvation may cause occasional packet loss (the data plane can't keep up), along with slow Pod-network bring-up and forwarding latency.
