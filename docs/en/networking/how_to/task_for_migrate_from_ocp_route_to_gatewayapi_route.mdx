...
Migrate from OpenShift Container Platform (OCP) Routes to Kubernetes Gateway API HTTPRoutes with Envoy Gateway. Use the feature-specific mappings below to translate each OCP Route feature into the equivalent Gateway API configuration.
...
:::note
Unlike OCP's `requiredHSTSPolicies` cluster-wide enforcement, Gateway API requires explicit configuration per route. There is no global HSTS policy enforcement mechanism in Gateway API. ACP provides Gateway-wide HSTS through the Envoy Gateway `ClientTrafficPolicy` extension; for more information, see [How To Configure HSTS](./tasks_for_envoy_gateway.mdx#configure_hsts).
:::
