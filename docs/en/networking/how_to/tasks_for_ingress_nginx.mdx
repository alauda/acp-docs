...
Ingress NGINX supports request-rate and concurrent-connection limits through Ingress annotations. For the complete annotation reference, see [Rate Limiting](https://kubernetes.github.io/ingress-nginx/user-guide/nginx-configuration/annotations/#rate-limiting).

### Limit a specific path

Annotations are applied to an Ingress object. If only one path, such as a login endpoint, must be limited, put that path in a separate Ingress object so that other paths are not limited by the same annotations.
...
- `limit-rps` limits requests per second for each client IP handled by each Controller Pod.
- `limit-burst-multiplier` allows a burst in addition to the configured rate. The burst size is calculated from `limit-rps` and this multiplier.
- `limit-connections` limits concurrent connections for each client IP handled by each Controller Pod.

These limits are local to each Controller Pod. With multiple replicas, the effective cluster-wide threshold depends on traffic distribution and the replica count; it is not a single shared counter. Preserve the real client IP before relying on IP-based limits.

### Configure the rejection status code

`limit-req-status-code` and `limit-conn-status-code` are Controller-wide settings. Configure them on the `IngressNginx` resource instead of editing the generated ConfigMap:
...
Changing these values affects every Ingress handled by that Controller instance. Use a dedicated Controller when another application cannot accept the same rejection status code.
...
- **Tainted nodes**: If the selected nodes have a `NoSchedule` taint, configure a matching toleration for both Ingress-Nginx and MetalLB
...
spec:
  controller:
    ...
    nodeSelector:                           # Schedule pods only on nodes matching these labels. This selector must match the MetalLB L2Advertisement node selector
      dedicated: ingress
    tolerations:
      - key: dedicated
        operator: Equal
...
- The `nodeSelector` must match the nodes selected by the MetalLB `L2Advertisement` so that Ingress-Nginx pods are scheduled on nodes that can receive traffic from MetalLB.
- Configure the same node selection and toleration on the MetalLB custom resource: use `spec.nodeSelector` and `spec.speakerTolerations` for the speaker, and `spec.controllerNodeSelector` and `spec.controllerTolerations` for the controller. By default, MetalLB tolerates no additional taints.
