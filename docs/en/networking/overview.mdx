...
## Comparison Among [Service](./functions/configure_service.html), [Ingress](./functions/configure_ingress.html), [Gateway API](./operators/envoy_gateway_operator.html)
...
Currently, we support GatewayApi through [Envoygateway](./operators/envoy_gateway_operator.html#understanding_envoy_gateway). For migration from ingress to gatewayapi please refer to [migration guide](https://gateway-api.sigs.k8s.io/guides/migrating-from-ingress)
