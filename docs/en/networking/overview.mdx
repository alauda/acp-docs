...
**Responsibility**: Provide clear visibility into network behavior so that operators can understand traffic patterns and quickly diagnose connectivity or performance issues.

**Implementation**:

- **Continuous Visibility**
   - Collect cluster-wide network flow logs with NetObserv
   - Enrich flow records with Kubernetes metadata
   - Store and query data for troubleshooting and auditing

- **On-Demand Troubleshooting**
   - Use the CLI to capture flows or packets for short-term analysis
   - Support real-time inspection in TUI mode or background capture tasks
   - Export results for offline analysis tools such as Wireshark

- **Typical Use Cases**
   - Identify dropped connections, latency hotspots, and unexpected traffic paths
   - Verify service-to-service communication and detect anomalies
   - Investigate incidents with both summarized flows and raw packets

Learn more about [Networking Observability](./observability).
...
## Comparison Among [Service](/networking/functions/configure_service.mdx), [Ingress](/networking/functions/configure_ingress.mdx), [Gateway API](/networking/operators/envoy_gateway_operator.mdx)

<Term name="product" /> supports multiple ingress traffic specifications in the Kubernetes ecosystem.
Compare Service, Ingress, and Gateway API to choose the traffic entry pattern that fits your workload and operational requirements.
