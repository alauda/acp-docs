...
2. In the left navigation bar, click **Networking** > **Container Network** > **Subnets**.
...
2. In the left navigation bar, click on **Networking** > **Container Network** > **Subnets**.
...
:::info
The container network in Kube-OVN Underlay requires support from the physical network. Please refer to the best practices [Preparing the Kube-OVN Underlay Physical Network](../how_to/kube_ovn/kubeovn_underlay_py.mdx) to ensure network connectivity.
:::
...
2. In the left navigation bar, click **Networking** > **Container Network** > **Bridge Networks**.
...
### Add Bridge Network by using the CLI \{#add-bridge-network-by-using-the-cli}

```shell
kubectl apply -f test-provider-network.yaml
```

### Add VLAN by using the web console (Optional) \{#add-vlan-by-using-the-web-console-optional}
...
2. In the left navigation bar, click **Networking** > **Container Network** > **VLANs**.
...
2. In the left sidebar, click on **Networking** > **Container Network** > **Subnets**.
...
:::note
**Expanding the default Pod subnet**

If a cluster is running out of Pod IP addresses, you can expand the default Pod subnet (`ovn-default`) in the same way. Existing Pods keep their addresses, and new Pods receive addresses from the enlarged range.

A subnet can only be enlarged while keeping the same network base address (the network prefix is made shorter); you cannot change the base address or shrink the range. If the default Pod subnet cannot be enlarged in place—for example, a larger range would not align to the current base address—add an additional subnet and bind it to the namespaces that need more addresses instead.

The Service CIDR cannot be changed after the cluster is created. Plan the Service network size before installation.
:::
