...
Configure a `Gateway` after the Envoy Gateway operator and `EnvoyGatewayCtl` are ready.
...
In the recommended workflow, configure a `Gateway` after [Envoy Gateway Operator](/networking/operators/envoy_gateway_operator.mdx)
and before [Configure GatewayAPI Route](./configure_gatewayapi_route.mdx).

To expose additional listeners owned outside the Gateway namespace, see [Configure Gateway API ListenerSet](./configure_gatewayapi_listenerset.mdx).
...
#### Allow ListenerSets \{#listener_allowed_listenerset}

A Gateway can accept additional listeners from ListenerSets in the same or
another namespace. Configure `spec.allowedListeners.namespaces` to control
which ListenerSet namespaces are accepted:
...
For `Selector`, configure a label selector under
`.spec.allowedListeners.namespaces.selector`. For more information, see
[Configure GatewayAPI ListenerSet](./configure_gatewayapi_listenerset.mdx).
...
To limit the total Envoy data plane requests in a namespace, create an
[`EnvoyProxyResourceQuota`](./configure_envoyproxy_resource_quota.mdx) in that namespace. The quota evaluates this
companion `EnvoyProxy` along with other `EnvoyProxy` resources in the namespace.

#### Recommended Resource Sizing

For typical HTTP and HTTPS north-south traffic, use the following values as starting points for a single `envoy-proxy`
Pod. These recommendations apply to
`.spec.provider.kubernetes.envoyDeployment.container.resources`.

| Peak QPS per `envoy-proxy` Pod | Recommended CPU | Recommended Memory |
| ------------------------------ | --------------- | ------------------ |
| 4,000                          | 2 cores         | 1Gi                |
| 8,000                          | 4 cores         | 2Gi                |
| 12,000                         | 8 cores         | 4Gi                |
...
- These values are starting points, not hard limits. Validate them against your actual traffic profile.
- TLS termination, enabled policies, route complexity, request and response sizes, and long-lived connections can all increase resource usage.
- When using multiple replicas, scale based on the observed QPS handled by each replica instead of only the total Gateway QPS.
...
To limit Envoy data plane resources at the namespace level, see
[Configure EnvoyProxy Resource Quotas](./configure_envoyproxy_resource_quota.mdx).
