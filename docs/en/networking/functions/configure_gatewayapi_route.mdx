...
Routes can also attach to an external `ListenerSet` listener. See [Configure Gateway API ListenerSet](./configure_gatewayapi_listenerset.mdx).
...
##### Attach to a ListenerSet \{#publish_to_listenerset}

The Web Console also lists accepted ListenerSet listeners. Select the
ListenerSet and listener name when publishing a Route. The ListenerSet
namespace, parent Gateway allowance, listener protocol, and listener
`allowedRoutes` must permit the attachment.
...
To attach to a ListenerSet in YAML, set `parentRefs[].kind` to `ListenerSet`,
set `name` and `namespace` to the ListenerSet, and set `sectionName` to the
listener name:
...
For more information, see [Configure GatewayAPI ListenerSet](./configure_gatewayapi_listenerset.mdx).
