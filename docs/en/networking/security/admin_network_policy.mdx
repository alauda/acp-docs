...
- In Kube-OVN network mode, this feature is a Technology Preview. The upstream API is still `policy.networking.k8s.io/v1alpha1` and continues to evolve, so evaluate this feature within its documented limits instead of treating it as production-supported. For the maturity definition, see [Version and Lifecycle](../../overview/version-and-lifecycle.mdx).
...
2.  In the left navigation bar, click **Networking** > **Cluster Network Policies**.
