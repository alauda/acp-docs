...
Configure network mode, replicas, resource limits, node selection, and taint toleration:
...
The `nodeSelector` and `tolerations` settings control where the Ingress Nginx controller pods are scheduled. By default, the Ingress Nginx controller tolerates no taints.
...
The `.spec` field of the `IngressNginx` resource directly corresponds to the Helm chart values for Ingress Nginx, including `.spec.controller.nodeSelector` and `.spec.controller.tolerations`. For additional configuration options, please refer to the [official Ingress NGINX documentation](https://kubernetes.github.io/ingress-nginx/).

### Default Value Differences Compared to Official Chart

1. By default, each IngressNginx instance creates an IngressClass with the name `$ns-$name` and controllerValue `ingress-nginx.cpaas.io/$ns-$name`. These values can be customized using the `.spec.controller.ingressClassResource.name` and `.spec.controller.ingressClassResource.controllerValue` parameters.
