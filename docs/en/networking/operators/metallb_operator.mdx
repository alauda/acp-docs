...
## Installing and Uninstalling the MetalLB Operator \{#installing_and_uninstalling_metallb}
...
### Configure Component Placement \{#configure-component-placement}

After the MetalLB Cluster Plugin is installed, configure component placement directly on the `MetalLB` custom resource in the `metallb-system` namespace. The Cluster Plugin creates this resource during installation when it does not already exist; the controller and speaker scheduling settings are managed by this resource.
...
`controllerNodeSelector` and `controllerTolerations` apply to the MetalLB controller Deployment. `nodeSelector` and `speakerTolerations` apply to the MetalLB speaker DaemonSet. By default, these fields are empty, so MetalLB does not tolerate any additional taint.

In Layer 2 mode, the `L2Advertisement.spec.nodeSelectors` configuration determines which speakers can announce VIPs. Align it with the MetalLB component placement and the `IngressNginx.spec.controller.nodeSelector` so that the speaker can announce VIPs from nodes that run the Ingress Nginx pods.

### Adjust Log Level \{#adjust-log-level}

Set `spec.logLevel` on the `MetalLB` custom resource to adjust the log level used by both the controller and speaker:
...
Supported values are `all`, `debug`, `info`, `warn`, `error`, and `none`. The default is `info`.
...
Learn more about [Upload Package](../../extend/upload_package.mdx).
