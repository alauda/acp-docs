...
- **Ensure that an operable ALB exists**: Create or use an existing ALB. The examples below use `<otel-alb>` as the ALB name. For instructions on creating an ALB, see [Configure ALB](/networking/functions/configure_alb.mdx).
