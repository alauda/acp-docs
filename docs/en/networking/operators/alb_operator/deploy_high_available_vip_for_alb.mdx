...
Before using this, ensure the cluster supports LoadBalancer Services. You can use the platform built‑in implementation. For setup, see [Configure MetalLB](/networking/functions/configure_metallb.mdx).

After MetalLB is ready, you can add the following annotations to `alb.spec.config.vip.lbSvcAnnotations` to tweak MetalLB behavior. See [ALB networking configuration](/networking/functions/configure_alb.mdx#alb_networking_configuration).
