...
# RoleTemplate Custom Role Configuration
...
> Note: `spec.rules` and `spec.aggregationRules` are mutually exclusive.
...
> Note: `acp-namespace-resource-manage` is **not allowed** in custom roles.
...
> Note: `aggregate-to-namespace-auditor` must match labels on existing ClusterRoles. If not, create a read-only ClusterRole and add the label first.

## 7. FunctionResource Index Appendix

- [FunctionResource Index](./roletemplate-function-resources.md)
