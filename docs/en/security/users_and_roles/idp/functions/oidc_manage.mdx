...
The platform supports the OIDC (OpenId Connect) protocol, which allows Platform Managers to log in to the platform using a third-party account after adding OIDC configuration. At the same time, it supports Platform Manager to update and delete the docked OIDC services.
...
4. Configure the **Basic Information** parameters.

5. Configure the **OIDC Server Configuration** parameters.
...
1. In the **IDP Service Configuration Validation** area, enter the **Username** and **Password** of a valid OIDC account to verify that the configuration is correct.
