...
For OpenLDAP, request the DN, `objectClass`, `uid`, `cn`, and `mail`. For Active Directory, request the DN, `objectClass`, `sAMAccountName`, `userPrincipalName`, `displayName`, `mail`, `userAccountControl`, and `memberOf`. Confirm that the intended population is below the candidate Base DN before narrowing the Filter.

### 5. Choose the Identity and Login Attributes

| Purpose | OpenLDAP candidates | Active Directory candidates | Selection rule |
| --- | --- | --- | --- |
| ACP user identity | `uid` | `sAMAccountName`, or a verified stable `userPrincipalName` | Present for every intended user, unique, stable, and non-empty |
| Login Field | `uid`, `mail` | `sAMAccountName`, `userPrincipalName` | Known to users and returns exactly one entry |
...
These mappings are starting points for the listed layouts. Add exclusions for disabled, service, or system accounts only when required by the customer's directory policy and after the broad Filter returns the correct population.

| Directory layout | User Filter | User Name Attr | Login Field | Group Attr | User Attr | Group Name Attr |
| --- | --- | --- | --- | --- | --- | --- |
| OpenLDAP without group sync | `(&(objectClass=inetOrgPerson)(uid=*))` | `uid` | `uid` | Not configured | Not configured | Not configured |
...
| Active Directory | `(&(objectClass=user)(!(objectClass=computer))(sAMAccountName=*))` | `sAMAccountName` | `sAMAccountName,userPrincipalName` | `member` | `DN` | `cn` |
...
8. Optionally open **Advanced Settings** and enable **Auto Sync**.
9. Click **Add**.
...
Enable **Auto Sync** under **Advanced Settings** when scheduled reconciliation is required. Automatic synchronization uses the configured Base DNs, Filters, and mappings; it does not authenticate directory user passwords.
...
Deleting the Connector without cleanup invalidates users from that source; it does not delete them automatically. If **Clean up IDP users and User Groups** is selected during deletion, ACP deletes the users and groups generated from that Connector.

## Configure LDAP with YAML

The following v2 `Connector` templates use `bindPW` only as an initial-creation input. ACP moves the submitted password to a managed Secret, and stored YAML later contains a managed `clientSecretRef` instead of `bindPW`. Replace every example value before use.
...
spec:
  config:
    ...
    insecureSkipVerify: false
    startTLS: false
    usernamePrompt: AD username or UPN
    userSearch:
...
| StartTLS | 389 | `false` | `true` | Keep `insecureSkipVerify: false` and provide a trusted CA when required |
| LDAPS | 636 | `false` | `false` | Keep `insecureSkipVerify: false` and provide a trusted CA when required |
...
The top-level `id` must equal `metadata.name`; current scheduled synchronization looks up the Connector by that identity.
...
| StartTLS | `insecureNoSSL: false`, `startTLS: true` | `-H ldap://ldap.example.com:389 -ZZ` | TLS is negotiated before the bind |
...
The query examples below use plain LDAP and therefore match only a Connector with `insecureNoSSL: true`. For StartTLS or LDAPS, replace the `-H` and `-ZZ` combination in every example with the matching row before running it.
...
**External command: inspect the LDAPS certificate chain**

```bash
openssl s_client -connect ldap.example.com:636 -showcerts
```

**External command: inspect the StartTLS certificate chain**

```bash
openssl s_client -starttls ldap -connect ldap.example.com:389 -showcerts
```

### External: Windows Active Directory Tools

The Active Directory cmdlets require the RSAT Active Directory module and directory read permission.

**External command: test TCP reachability, read the directory root, inspect users, and inspect a group**
...
Get-ADGroup -Identity 'ACP Users' -Properties member |
  Select-Object -ExpandProperty member
```

**External GUI tool: `ldp.exe`**

Open `ldp.exe`, then use **Connect**, **Bind**, **View Tree**, and **Search** in that order. In **Search**, enter the candidate Base DN and Filter and select the **Subtree** scope. This reproduces the directory search independently of ACP.
