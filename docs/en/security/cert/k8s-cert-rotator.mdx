...
For advanced `ClusterPluginInstance` YAML installation and configuration, see [Install Via YAML](/configure/registry/install/install_via_yaml.mdx). Keep each renewal threshold shorter than its corresponding certificate duration. Changing a duration also causes certificates with a different validity period to be renewed.
