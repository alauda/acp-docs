...
For advanced plugin YAML installation and configuration, see [Cluster Plugin](/extend/cluster_plugin.mdx). Keep each renewal threshold shorter than its corresponding certificate duration. Changing a duration also causes certificates with a different validity period to be renewed.
