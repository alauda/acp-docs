...
The plugin exposes the following certificate settings through its installation form:

| Setting | Default | Description |
| --- | --- | --- |
| Control-plane certificate duration | `8760h` (365 days) | Validity period for renewed control-plane certificates. |
| Control-plane renewal threshold | `1752h` (73 days) | Renew when less than this duration remains. |
| Kubelet certificate duration | `2184h` (91 days) | Validity period for renewed kubelet certificates. |
| Kubelet renewal threshold | `720h` (30 days) | Renew when less than this duration remains. |
...
The plugin applies the configured values to subsequent checks and rotations.
