...
The corresponding `ClusterPluginInstance` in the target cluster contains the same `spec.config`. With the default `manage-update-by: cluster-transformer` label, `ModuleInfo` is the update source and direct edits to the `ClusterPluginInstance` can be overwritten during reconciliation. Keep each renewal threshold shorter than its corresponding certificate duration. Changing a duration also causes certificates with a different validity period to be renewed.
