...
The plugin exposes the following certificate settings through its installation form. The controller uses the configured renewal threshold for each certificate type and checks certificates at the configured interval.

### Certificate settings

| Certificate | Validity period | Renewal threshold | Approximate rotation point |
| --- | --- | --- | --- |
...
The approximate rotation point is derived from the validity period and renewal threshold. Actual timing can vary by one check interval. If you customize these values, the configured values take precedence.
...
2.  **Rotation Trigger Condition**
    ...
    The `notBefore` and `notAfter` fields of the certificate indicate the validity period. Rotation is triggered when the remaining validity period reaches the configured renewal threshold for that certificate type. A certificate whose validity period differs from the expected duration is also renewed.
