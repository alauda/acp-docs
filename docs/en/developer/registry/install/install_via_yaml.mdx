...
3. Configure session affinity for the Registry Ingress.

   When the Registry uses multiple replicas with shared file storage such as NFS or another RWX StorageClass, configure session affinity at the Ingress controller layer. A single image push sends multiple `POST`, `PATCH`, `PUT`, `HEAD`, and manifest requests; those requests should reach the same Registry Pod. The Kubernetes Service `sessionAffinity: ClientIP` setting does not guarantee stickiness when the Ingress controller load-balances directly to Pod endpoints.

   For the default ACP ALB Ingress, use source IP hash affinity:
...
| `spec.config.ingress.annotations`      | Ingress controller annotations. Use this field to configure session affinity for multi-replica Registry deployments exposed through Ingress. | `alb.ingress.cpaas.io/session-affinity-policy: sip-hash` |
