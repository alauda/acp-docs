...
The Alauda Container Platform provides command line tools for users to interact with the ACP Registry. The following are some examples of common operations and commands:

Let's assume that ACP Registry for the cluster has a service address of registry.cluster.local and the namespace you are currently working on is my-ns.
...
Pushes an image from a remote image repository to a specific namespace in the ACP Registry.
