...
# Legacy Registry: Common CLI Command Operations
...
This page documents legacy Registry CLI operations. For the Operator-managed Registry, use the
mode-aware Image and ImageStream commands in [Managing Images with ac](../registry_v2/managing_images_with_ac.mdx).
...
- `<REGISTRY_API_URL>` is the legacy Registry API endpoint used by `ac get images` and `ac delete images`.
- In legacy external access scenarios, `ac` should typically use an explicitly specified
  `--registry-url` instead of relying on the default in-cluster Registry address.
...
When `ac` runs outside the cluster in legacy mode, explicitly specifying `--registry-url` is
recommended. Otherwise, the CLI may fall back to the default in-cluster Registry address, which is
often unreachable from a local workstation.
