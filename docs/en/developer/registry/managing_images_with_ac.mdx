...
Use the latest `ac` image commands to work with <Term name="productShort" /> Registry and the
<Term name="productShort" /> Image API. ACP 4.4 Registry is selected by `modern` registry mode and
provides Image and ImageStream workflows for <Term name="productShort" /> clusters.
...
| --- | --- | --- |
| `modern` | `image.alauda.io/v1` Image and ImageStream API plus the registry service. | ACP 4.4 Registry management. |
| `auto` | Automatic detection. | Discovery. Set `modern` before managing the ACP 4.4 Registry. |
