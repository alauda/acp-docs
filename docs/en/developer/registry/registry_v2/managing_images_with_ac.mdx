...
Registry v2 maps each image repository to one namespace and one ImageStream name. Use `<namespace>/<imagestream>:<tag>` when referencing an image in the integrated Registry. Repository paths with an additional path segment, such as `<namespace>/<group>/<imagestream>:<tag>`, are not supported. Use a single ImageStream name such as `group-imagestream`, or use a separate namespace for grouping.
