...
Use the latest `ac` image commands to work with **Registry v2** and the <Term name="productShort" /> Image API. Registry v2 is selected by `modern` registry mode and provides Image and ImageStream workflows for <Term name="productShort" /> clusters. Compatibility entry points remain available for environments that still use the legacy Registry.
...
| `legacy` | Legacy <Term name="productShort" /> Registry catalog, tag, and manifest metadata. | Environments that still use the legacy Registry. |
...
List images through the integrated Registry compatibility entry point. Query a specific namespace whenever possible:

```bash
ac get images -n team-a
```

In `modern` mode, `ac get images` lists cluster-scoped Image resources with `NAME` and `IMAGE REFERENCE` columns. In `legacy` mode, it keeps the old <Term name="productShort" /> Registry catalog output and lists repository tags.

Use structured output:

```bash
ac get images -n team-a -o yaml
ac get images -n team-a -o json
```

Legacy-only options remain available for compatibility:

```bash
ac get images -n team-a --registry-url=http://old-registry.example.com:5000
ac get images -n team-a --fast --registry-url=http://old-registry.example.com:5000
```
