...
Use the latest `ac` image commands to work with the Operator-managed <Term name="productShort" />
Registry and the <Term name="productShort" /> Image API. The Operator-managed Registry is selected
by `modern` registry mode and provides Image and ImageStream workflows for <Term name="productShort" />
clusters. Compatibility entry points remain available for environments that still use the legacy
Registry.
...
| --- | --- | --- |
| `modern` | Operator-managed `image.alauda.io/v1` Image and ImageStream API plus the registry service. | Current Registry management. |
...
API usage through the Operator-managed Image API graph.
...
[Managing access and cleanup](../../../configure/registry/registry_v2/managing_access_and_cleanup.mdx).
...
```bash
ac tag quay.io/example/demo:1.0 demo:latest
```
...
The preferred Operator-managed Registry workflow is to delete ImageStream tags:
