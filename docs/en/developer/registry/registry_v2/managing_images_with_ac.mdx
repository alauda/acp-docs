...
Use the latest `ac` image commands to work with **Registry v2** and the ACP Image API. Registry v2 is selected by `modern` registry mode and provides Image and ImageStream workflows for ACP clusters. Compatibility entry points remain available for environments that still use the legacy Registry.
...
| `legacy` | Legacy ACP Registry catalog, tag, and manifest metadata. | Environments that still use the legacy Registry. |
