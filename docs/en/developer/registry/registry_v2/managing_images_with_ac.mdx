...
Use `modern` mode before managing ImageStreams, importing images, tagging images, viewing Image API usage, verifying signatures, or pruning through the Registry v2 Image API graph.
...
| Administer usage and cleanup | `ac adm top images`, `ac adm top imagestreams`, `ac adm prune images`, `ac adm registry gc`, `ac adm verify-image-signature` | Image API commands require `modern` mode. Prune and GC are dry-run by default. |
