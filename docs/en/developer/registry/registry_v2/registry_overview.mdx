...
**Registry v2** is the Operator-managed integrated image registry for ACP clusters. It follows the OpenShift image registry model where it applies to ACP and provides internal image storage, ImageStream metadata, namespace-based access control, managed service account pull credentials, and image pruning.
...
- OpenShift image registry concepts such as `Image`, `ImageStream`, `ImageStreamTag`, `ImagePruner`, image puller and pusher roles, and ImageStream layer authorization are preserved where they apply to ACP.
