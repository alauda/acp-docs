...
## Managing namespace members by using new web console

This page mainly describes the legacy console workflow, but namespace member management in the new web console is still a project-scoped action.

Use the **Applications** view when you need to manage member access for a namespace that already belongs to the current project.

When you manage namespace members from that view, confirm that:

- the namespace is already associated with the current project
- the member source still matches the project-level ownership model because imports come from project members
- the role change affects namespace access only and does not replace project-level membership governance
- the resulting namespace member list still matches the intended operational ownership of the workloads in that namespace
