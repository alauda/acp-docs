...
<img src="./assets/ns-import.svg" width="400" />
...
## Review points before importing a namespace

Before importing a namespace, confirm that:

- the namespace is in a cluster that is already associated with the current project
- the namespace is not still governed by another project when exclusive association rules apply
- the existing workloads, quotas, policies, and metadata in that namespace are acceptable under the target project

Importing a namespace changes governance and visibility expectations, not only list membership.

## Importing from the Applications view in the new web console

The new web console keeps namespace import in the **Applications** view because import is a project-scoped governance action.

When you use that workflow:

- Start from the target project and open **Namespaces**.
- Use **Import Namespace** only for namespaces in clusters already associated with the current project.
- Review the namespace state before import so that monitoring, quota governance, and policy expectations still make sense after association.
- Verify after import that the namespace appears in the current project and remains associated with the expected cluster.

## Special notes for new web console

This page mainly describes the legacy console workflow.

If you use the new web console and need the project-scoped import workflow, see [Manage Namespaces](../../new_web_console/namespaces/manage-namespaces.mdx).

If you need background information about the new web console entry and scope, see [New Web Console](../../../ui/new_web_console/index.mdx).
