...
- Use the **Applications** view when the namespace should be managed inside the current project scope and the target cluster is already associated with that project.
- Use the **Clusters** view when you enter from a concrete cluster and need direct cluster-side namespace governance instead of project membership management.

This distinction matters because the surrounding list context, supported actions, and follow-up governance checks are different even when the namespace object is the same Kubernetes resource.
...
## Review points before creating a namespace

Regardless of whether you create the namespace from the legacy console or the new web console, confirm:

- that the namespace name matches your cluster or project naming expectations
- that labels and annotations reflect the intended operational use
- that resource quota, limit range, and security settings match the workloads that will run there

These settings should be treated as workload-admission and resource-governance inputs, not only as descriptive metadata. Small differences here can change how later workloads are admitted, scheduled, scaled, or defaulted.

## Creating a namespace in the new web console
...
- the labels, annotations, quotas, and limit settings match the workloads that will later run there
- the main goal is cluster-side namespace governance rather than project association
- the namespace details, YAML, quota, and limit information can be reviewed afterward as expected

The **Clusters** view does not cover importing the namespace into a project during the same workflow.
