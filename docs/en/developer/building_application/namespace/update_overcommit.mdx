...
## Updating overcommit ratio by using new web console
...
Use the **Applications** view when the namespace is already associated with the current project and you want to adjust request-to-limit governance in that project scope.

Before you update the ratio, confirm that:

- the namespace belongs to the current project
- the new CPU and memory ratios still match the workloads that will be created or recreated later in that namespace
- the change is evaluated together with quota, default limits, and scheduling expectations in the current project
