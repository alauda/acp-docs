...
When reviewing a Pod, confirm the cluster, namespace, and source workload before treating the observed state as a project-wide issue.

#### Pod Parameters

Below are some parameter explanations:

The runtime information on the Pod list or details page is especially useful when you need to determine whether an issue comes from workload ownership, node placement, repeated restarts, or incorrect resource assumptions.
...
- The Pod appears in the expected project, cluster, and namespace context
