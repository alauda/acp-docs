...
## When to use a DaemonSet in the Applications view
...
If only a limited number of interchangeable application replicas are needed, review [Deployments](./deployment_create.mdx) instead.

## Node-wide rollout review points

Before creating or updating a DaemonSet, confirm that:

- The selected project, cluster, and namespace are the intended runtime scope
...
The update operation keeps the DaemonSet in its current cluster and namespace. If node-level agents are needed in another cluster or namespace, deploy a separate DaemonSet in that target scope instead of trying to move the existing one through update.
...
- The DaemonSet appears in the expected project, cluster, and namespace context
...
After deleting a DaemonSet, verify that:

- The DaemonSet no longer appears in the list
- Node-level workload coverage changes match your expected outcome
