...
3. **Select** or **Input** an image, and click **Confirm**.
   ...
   **Note**: Image filtering is available only when using images from the platform's integrated image registry. For example, an integrated project name like containers (registry-projectname) indicates the platform's project name projectname and the image registry's project name containers.
...
When you later update this CronJob from the Applications view, the CronJob remains in its current cluster and namespace. If the scheduled workload must run in a different cluster or namespace, create a separate CronJob in that target scope instead of treating update as relocation.
...
- The CronJob appears in the expected project, cluster, and namespace context
