...
In both cases, the ConfigMap still belongs to a specific namespace, and workloads can only reference ConfigMaps from the same namespace.
...
Before you submit the ConfigMap, confirm that the keys, expected file names, and value formats match the way the workload reads the configuration.
...
When workloads consume ConfigMaps through environment variables or mounted files, updating or deleting the ConfigMap can change runtime behavior. Some changes require rollout, restart, or Pod recreation before the updated configuration takes effect.

If imported binary content is used, confirm whether the current UI allows only key changes after creation before you choose ConfigMaps for frequently changing binary data.
...
## Using ConfigMaps in the new web console
...
Use the **Applications** view when the ConfigMap is managed as part of the current project scope.

Before creating or updating the ConfigMap from this view, confirm that:
...
- the keys, file names, and value formats match the way the workloads read the configuration
- the ConfigMap appears afterward in the selected project and cluster context

### Create or update from the Clusters view

Use the **Clusters** view when you start from a concrete cluster and manage ConfigMaps as namespace resources in that cluster.

Before creating or updating the ConfigMap from this view, confirm that:

- you have opened the target cluster first
- you have selected or confirmed the target namespace in that cluster
- you are reviewing namespace-local workload impact rather than project membership
- the ConfigMap appears afterward in the expected namespace

### Verify workload impact

No matter which view you use, also verify that:

- the referenced workloads are still in the same namespace as the ConfigMap
- any required rollout, restart, or Pod recreation has been completed after the change
- deleting the ConfigMap does not break later workload creation or recreation because of a missing reference

If the data is sensitive, use **Secrets** instead of ConfigMaps.
