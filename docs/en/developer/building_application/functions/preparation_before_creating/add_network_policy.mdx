...
Network Policy (NetworkPolicy) is implemented by the network plugin and is a Kubernetes resource that belongs to a namespace. Through network policies, network data access traffic for workloads within the project can be controlled using a whitelist method, achieving network isolation and reducing the risk of attacks.

By default, there is no isolation between workloads, allowing inbound (ingress) and outbound (egress) traffic from any source. Once a network policy is added, the targeted workloads in the policy will only accept traffic from the whitelist.
...
- When configuring network policies, bidirectional traffic must be considered. For example, if target workload A allows outbound traffic to remote workload B, and B does not make any settings, then A can access B normally; if B configures an inbound whitelist but does not include A, then B will deny A's access.
...
4. Refer to the following instructions to complete the relevant configuration.
   ...
   | ------------------ | --------------------- |
   | **Target Workload / Label Selector** | The workload corresponding to the web application         |
   | **Direction**             | Inbound                    |
   | **Protocol**             | TCP                   |
...
| Parameter                 | Example              |
| ------------------ | --------------- |
| **Target Workload / Label Selector** | The workload corresponding to the MySQL instance |
| **Direction**             | Inbound              |
| **Protocol**             | TCP             |
| **Port**             | 6379            |
...
| Parameter                 | Example                    |
| ------------------ | --------------------- |
| **Target Workload / Label Selector** | The workload corresponding to the instance             |
| **Direction**             | Outbound                    |
| **Protocol**             | TCP                   |
| **Port**             | 80 (HTTP) and 443 (HTTPS) |
| **Remote Type**           | IP Range                  |
...
For more detailed information, please refer to the official documentation on [Network Policies](https://kubernetes.io/docs/concepts/services-networking/network-policies/) .
