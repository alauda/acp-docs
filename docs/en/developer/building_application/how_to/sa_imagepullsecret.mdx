...
- Alauda DevOps Connectors 1.12 is installed, the Connector is Ready, and `enable-pod-image-pull-via-connector` is enabled.
...
The `--docker-server` value must identify the Connector proxy, not the original registry. If `.status.proxy.httpAddress.url` includes a Connector path, follow the [Connectors 1.12 workload guide](https://docs.alauda.io/alauda-devops-connectors/1.12/connectors-oci/how_to/using_oci_connector_in_workload.html) to determine the host and port form to use as the registry server. The command creates a Secret of type `kubernetes.io/dockerconfigjson`.
