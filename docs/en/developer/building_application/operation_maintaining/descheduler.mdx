...
Locate the descheduler policy ConfigMap across namespaces. Do not assume that
the package uses `kube-system`; the namespace is determined by the installed
ModulePlugin:
...
```bash
kubectl get configmap -n <descheduler-namespace> <descheduler-configmap-name> \
  -o yaml > descheduler-configmap.backup.yaml
```

Edit the ConfigMap returned by the previous command:

```bash
kubectl edit configmap -n <descheduler-namespace> <descheduler-configmap-name>
```
...
Verify that the `ModuleInfo` has transitioned to the `Running` state. Discover
it across namespaces because the plugin namespace is installation-specific.

Run this against the `global` (management) cluster: `ModuleInfo` is not served
by workload clusters, where the command fails with `the server doesn't have a
resource type "moduleinfo"`. The ConfigMap, CronJob, Deployment, and Pod checks
that follow run against the cluster the plugin is installed on.
...
```bash
kubectl get configmap -n <descheduler-namespace> <descheduler-configmap-name> -o yaml
```
...
```bash
kubectl get jobs -n <descheduler-namespace> -l app.kubernetes.io/name=descheduler
```

If running as a `Deployment`, confirm the running Pod and restart it after policy changes:

```bash
kubectl get pods -n <descheduler-namespace> -l app.kubernetes.io/name=descheduler
kubectl rollout restart deployment -n <descheduler-namespace> \
...
```bash
kubectl logs -n <descheduler-namespace> -l app.kubernetes.io/name=descheduler --tail=100
```
