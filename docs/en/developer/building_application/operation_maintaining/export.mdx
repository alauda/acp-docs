...
You can export either the current application state or a specific historical version from the application details page when a stable earlier revision is the one that should be archived, reused, or distributed.
...
### Review points before exporting a historical version

Before exporting a historical version, confirm that:

- The selected version is the one you actually want to distribute or archive.
- The export format matches the intended reuse scenario.
- Referenced resources and dependencies are still meaningful for the target environment.
- You understand whether the exported result reflects the current live state or an earlier preserved revision.
...
After exporting the YAML to a Git code repository, you can refer to <ExternalSiteLink name="gitops" href="/functions/create_argocd_application/create_application_via_platform.html" children="Creating GitOps Applications" /> to create a `Custom Application` type of GitOps application across clusters.
