...
The application details page also provides an application-scoped log entry where you can switch between Pods and containers associated with the application before narrowing the issue down to a specific workload.
...
When using logs from the application details page, first confirm which Pod and container are currently selected. Application-level log review is convenient for quick investigation, but the visible stream still reflects one concrete runtime instance at a time.
...
- **Retention Policy**: Live logs follow Kubernetes log rotation configuration. For historical analysis, use <ExternalSiteLink name="logs" href="/functions/log.html" children="Logs" /> .
:::
