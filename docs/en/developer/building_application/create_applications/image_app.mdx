...
## Create and update image-based applications in the Applications view

If you use the **Applications** view, the image-based workflow is part of the project-scoped application creation and update flow.

When you create an image-based application in that view:

- Select the target **Cluster** first.
- Select the target **Namespace** in the chosen cluster.
- Configure the application metadata and workload model.
- Review image source, runtime parameters, service exposure, and referenced dependencies together before submission.

### Review points before creation

Before clicking **Create**, confirm that:

- The selected cluster and namespace match the intended deployment target.
- The image can be pulled from the target environment.
- Any required image pull secret already exists in the target namespace.
- Referenced ConfigMaps, Secrets, PVCs, and ServiceAccounts are already available in the same namespace.
- The selected workload model matches the intended runtime behavior.
- Health checks, ports, environment variables, and service exposure settings remain internally consistent.

### Understand update behavior in the Applications view

When you update an existing application in the **Applications** view, the workflow modifies the application in place.

The following fields remain fixed during update:

- Cluster
- Namespace
- Name

Depending on the current environment, the update workflow can expose a preview page where you can review the current structure, edit supported configuration items, and then submit the change.
