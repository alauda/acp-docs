...
## First-Time Use Process of Container Platform

If you are using the Container Platform for the first time, you can refer to the document [Quickly Deploy a Simple Application Using Images](./quick_start/quick_start_acp.mdx) to understand the basic usage.

## Application Types

In the platform's **Container Platform > Application Management**, the following types of applications can be created:

- **Native Application**: A complete business application composed of one or more associated computing components (Workloads), internal routes (Services), and other native Kubernetes resources. It supports creation through UI editing, YAML orchestration, and templates, and can run in development, testing, or production environments. Different types of native applications can be created in the following ways:
  - **Create by Image**: Quickly create applications using existing container images.
  - **Create by Helm Chart**: Create applications using Helm Chart packages.
  - **Create by YAML**: Create applications using YAML configuration files.
  - **Create by Code**: Create applications using source code.
- **Component Application**: Based on application components (Operator backed), you can quickly deploy a component application and leverage the capabilities of Operators to automate the entire lifecycle management of the application.
