...
The distributed call chain query feature provides full-link tracing capabilities for microservices architecture by collecting metadata information of inter-service calls, helping users quickly locate cross-service call issues. This feature mainly addresses the following problems:
...
<Steps>
  ...
  **Tip**: Query conditions can be combined for use. You can refine your query by adding multiple query conditions.
  ...
  | ------------------ | ------------------------------------------------------------------------------------------------------------------ |
  | **TraceID**        | The unique identifier for the complete link, which can be used to query the specified call chain.                  |
...
The time-series waterfall chart on the right side of the query results area clearly displays the call relationships between Spans in a single call chain. The main features of using time-series waterfall charts in call chain analysis are as follows:
