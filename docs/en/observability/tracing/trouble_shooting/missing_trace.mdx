...
# Unable to Query the Required Call Chain

## Problem Description

When querying the call chain in a service mesh, you may encounter situations where the target call chain cannot be retrieved.

## Root Cause Analysis

### 1. Call Chain Sampling Rate Configured Too Low

When the sampling rate parameter for the call chain is set too low, the system will only collect call chain data proportionally. During times of insufficient request volume or low-peak periods, this may lead to the sampled data being below the visibility threshold.

### 2. Elasticsearch Real-Time Limitations

The default configuration for Elasticsearch index is `"refresh_interval": "10s"`, which results in a delay of 10 seconds before data is refreshed from the memory buffer to a searchable state. When querying recently generated call chains, the results may be missing because the data has not yet been persisted.
