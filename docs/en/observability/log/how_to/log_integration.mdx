...
You can interface with external Elasticsearch or Kafka clusters by writing YAML configurations. Depending on your business requirements, you can choose to interface with only the external Elasticsearch cluster (while deploying Kafka in the current cluster), or you can interface with both the external Elasticsearch and Kafka clusters simultaneously.
...
1. In the left navigation bar, click on **Cluster Management** > **Resource Management**, then switch to the cluster that needs the plugin deployment.
...
2. In the top navigation, select the ***Cluster Name*** where you want to deploy the Log Storage with Elasticsearch plugin.

3. Click the action button on the right side of **Log Storage with Elasticsearch** > **Deploy**.
...
- Interfacing with the external Elasticsearch cluster while deploying Kafka in the current cluster:
