...
nevermore is deployed as a daemonset in the cpaas-system namespace of each cluster. This component consists of 4 containers:
...
- After Razor receives requests sent by nevermore from various business clusters, it first authenticates using the Token in the request. If authentication fails, the request is denied.
- If the deployed log storage component is ElasticSearch, it writes the corresponding logs into the Kafka cluster.
- If the deployed log storage component is Clickhouse, it passes the corresponding logs to Vector, which are ultimately written into Clickhouse.
