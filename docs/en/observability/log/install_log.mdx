...
3. The ACP Log Storage with Clickhouse plugin and the ACP Log Storage with ElasticSearch plugin cannot be installed in the same cluster. Please read the [Selection Suggestions](./architecture/component_suggestion.mdx) and choose to install one of the log storage plugins.

4. The ACP Log Storage with Clickhouse plugin needs Clickhouse operator, before installing the plugin, please ensure that the Clickhouse operator is uploaded in the cluster.
:::
