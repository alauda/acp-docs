...
- Set the storage path to `/var/cpaas/monitoring` instead of the default `/cpaas/monitoring`. In the console, use the storage path field shown for `LocalVolume` storage; in YAML, set `spec.config.storage.path`.
- Back that path with a persistent disk on every node listed in `spec.config.storage.nodes`, sized for the storage capacity you configure. Declare the disk in the persistent-disk field of the provider that manages the cluster, so that it survives node replacement:
...
3. Configure the following parameters:
   ...
   | **Storage Type**       | - **LocalVolume**: Local storage with data stored on specified nodes<br />- **StorageClass**: Automatically generates persistent volumes using a storage class<br />- **PV**: Utilizes existing persistent volumes<br />**Note**: Storage configuration cannot be modified after installation<br />**Note**: On Immutable Infrastructure, the `LocalVolume` path must be set to `/var/cpaas/monitoring`; see [LocalVolume Storage Path on Immutable Infrastructure](#immutable_storage_path) |
