...
2. **Multi-Cluster Installation Mode**
   ...
   - The central monitoring cluster is a dedicated workload cluster, not the `global` cluster. The `global` cluster installs its own full component set and monitors only itself, so that management cluster availability and application monitoring load stay independent.
   - When the `global` cluster is deployed with disaster recovery, VMAgent in a workload cluster must not point to either the primary or the standby `global` cluster, because monitoring data is not replicated between them.
