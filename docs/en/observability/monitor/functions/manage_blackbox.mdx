...
The blackbox monitoring feature of the platform is realized based on Blackbox Exporter, allowing users to probe the network via ICMP, TCP, or HTTP to quickly identify faults occurring on the platform.
...
The monitoring components must be deployed in the cluster, and the monitoring components must be functioning properly.
...
The configuration file for blackbox monitoring is located within the namespace where the Prometheus component of the cluster is deployed, with the default name being `cpaas-monitor-prometheus-blackbox-exporter`, which can be modified as needed based on the actual name.
...
- The target cluster must have monitoring components deployed.
