...
- Monitoring components are deployed in the target cluster (required when creating alert policies using monitoring indicators).
- Log storage components and log collection components are deployed in the target cluster (required when creating alert policies using logs and events).
