...
| **8** | Table        | Organizes data in a row-column format, making it easy to view and compare specific values. | Suitable for displaying structured multi-dimensional data, such as detailed information of nodes, detailed information of container groups, etc. |
| **9** | Stat Chart   | Displays the current value of a single key indicator, typically requiring textual explanation. | Suitable for showing real-time values of important monitoring indicators, such as numbers of container groups, number of nodes, current alert count, etc. |
...
3. Add a pannel to the current dashboard and reference the previously added variables, for example:
   ...
   - Metric Name: **Container Group Memory Usage under Compute Components**.
