...
If the configured Webhook endpoint requires custom request headers, associate the contact group with the Secret created in [Configure Custom Request Headers for Webhook Notifications](#configure-custom-request-headers-for-webhook-notifications).
