...
## Upgrade Fleet Monitoring \{#upgrade-fleet-monitoring}

Fleet Monitoring is upgraded through the same OperatorHub workflow that installs it. An upgrade does not delete Fleet Monitoring data. Metrics that connected clusters already wrote to the Global storage stay available.

### Compatibility Matrix

The table below lists the supported versions:

| Fleet Monitoring Version | Supported Alauda Container Platform Version |
| --- | --- |
| 1.0.1 | 4.3.x, 4.4.x |
| 1.0.0 | 4.3.x |
...
- **Patch-level upgrade**: upgrades between patch versions within the same minor version are compatible and can be performed directly, for example `1.0.0` → `1.0.1`.
- **Upgrade order**: upgrade **Alauda Container Platform Fleet Monitoring Central Service** on the Global cluster first, then **Alauda Container Platform Fleet Monitoring Cluster Service** on each connected cluster.
- The two services can run at different versions during the upgrade window. Connected clusters keep writing Fleet Monitoring data while the Global cluster is being upgraded.

### Upgrade Procedure

1. Push the new Operator packages with `violet push`, in the same way as the initial installation. Push the Central Service package to the Global cluster and the Cluster Service package to every connected cluster.

2. Go to **Administrator** > **Marketplace** > **OperatorHub** and select the target cluster.

3. Open the Fleet Monitoring Operator and start the upgrade.

   - With the `Automatic` upgrade strategy, OperatorHub upgrades the Operator after the new version becomes available.
   - With the `Manual` upgrade strategy, approve the pending install plan to start the upgrade.

4. Repeat the previous two steps on every cluster that runs a Fleet Monitoring Operator.

5. Verify that the new version is installed.

   ```shell
   kubectl -n <fleet-monitoring-namespace> get csv
   ```

   The Fleet Monitoring `ClusterServiceVersion` must report the new version with the `Succeeded` phase.

6. Verify the `FleetMonitoringHub` and `FleetMonitoringAgent` status again, as described in the previous sections.

Keep the following points in mind:

- Do not delete and recreate `FleetMonitoringHub` or `FleetMonitoringAgent` for an upgrade. Both resources are preserved and reconciled by the new Operator version.
- If the new version ships a new VMAgent image, the Fleet Monitoring VMAgent pod restarts on clusters that deploy one. Collection pauses for the restart and resumes automatically.
...
### A cluster is missing from Connected Clusters \{#a-cluster-is-missing-from-connected-clusters}
...
Check whether the dashboard was created on the Global cluster and has the following label:
...
Also check the namespace of the dashboard resource. The Fleet Monitoring page lists dashboards from the namespace where **Alauda Container Platform Fleet Monitoring Central Service** is installed, plus `cpaas-system` and the namespaces published by the platform dashboard configuration. A dashboard created in another namespace is not listed.

### A new cluster or project is missing from the dashboards

Some panels, such as the project quota panels and the long-range trend panels, read Fleet-level rollup metrics that the Global cluster evaluates once per hour. A newly connected cluster or a newly created project can take up to about one hour to appear on these panels, even though the raw Fleet metrics are already available.

If the panel is still empty after one rollup interval, follow the checks in [A cluster is missing from Connected Clusters](#a-cluster-is-missing-from-connected-clusters).
