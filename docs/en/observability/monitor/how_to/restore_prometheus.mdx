...
- **LocalVolume**: Copy from the `<storage path>/prometheus` directory on the node, where `<storage path>` is the path configured when the plugin was installed: `/cpaas/monitoring` by default, or `/var/cpaas/monitoring` on Immutable Infrastructure
...
2. Move data into the specified directory:
   ...
   **Shortcut**: When the storage type is **LocalVolume** during plugin installation, simply copy the backup data directly to the `<storage path>/prometheus/prometheus-db/` directory of the node where the plugin is installed. `<storage path>` is the path configured at installation: `/cpaas/monitoring` by default, or `/var/cpaas/monitoring` on Immutable Infrastructure.
