...
- The Monitoring with Prometheus plugin has been deployed (the name of the compute component is `prometheus-kube-prometheus-0`, and the type is `StatefulSet`)
...
2. Backup from the storage backend (based on the type of storage selected during deployment):
...
**Shortcut**: When the storage type is **LocalVolume** during plugin deployment, simply copy the backup data directly to the `/cpaas/monitoring/prometheus/prometheus-db/` directory of the node where the plugin is deployed.
