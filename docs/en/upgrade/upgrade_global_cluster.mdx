...
If `ProductBase.spec.registry.external` is `true`, `upgrade.sh` automatically skips image synchronization. Before continuing on this page, use the target-version Core Package to run both `res/upload.sh all` and `res/upload.sh necessary`, then validate the target payload as described in [Prepare the Registry Before an Upgrade](../install/prepare/external-platform-image-registry.mdx#prepare-the-registry-before-an-upgrade).
</Directive>
...
| External registry                             | Automatically set `SKIP_SYNC_IMAGE=true`; the target payload must already have been uploaded by following [Prepare the Registry Before an Upgrade](../install/prepare/external-platform-image-registry.mdx#prepare-the-registry-before-an-upgrade). |
...
2. If the application is listed in **ACP Upgrade to v4.4**, download its package and copy it into the extracted Core Package's `plugins/` directory. For the platform built-in Registry, rerun `upgrade.sh --only-sync-image`. For an external registry, rerun both manual upload modes described in [Prepare the Registry Before an Upgrade](../install/prepare/external-platform-image-registry.mdx#prepare-the-registry-before-an-upgrade).
