...
<Term name="productShort" /> consists of a **global cluster** and one or more **workload clusters**.
The global cluster **must** be upgraded before any workload clusters.
...
If the global cluster uses the **global DR procedure** and you are uploading images to the **standby** global cluster, you must specify the `--registry` parameter — this should point to the standby global cluster’s image registry:

  ```bash
  bash upgrade.sh --only-sync-image=true --registry <registry-address>   --username <username> --password <password>

  # Example:
  # bash upgrade.sh --only-sync-image=true --registry example.com   --username admin --password password
  ```
...
However, since service disruptions may occur regardless, we recommend including the Kubernetes upgrade to avoid multiple maintenance windows.
</Directive>
