...
The default annotation key is `config.cpaas.io/exempt-for-ver`, and its value is the target version:
...
The exemption is matched on the major and minor version, so an exemption recorded for one patch version also covers the other patch versions of the same minor release.
...
Do not force an upgrade past a failed check. `VersionUpgradePath` and `KubernetesVersionSupported` gate the supported upgrade path itself, and an upgrade that bypasses them reaches a state the platform has not validated. If a block cannot be resolved from this page, contact technical support before continuing.
