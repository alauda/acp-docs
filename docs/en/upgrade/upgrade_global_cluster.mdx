...
:::warning
1. When upgrading the **global cluster**, do **not** use the `--clusters` parameter to create CRs on workload clusters in the [Create only CRs from all packages in a directory](../extend/upload_package.mdx#only_create_cr) step.
...
After running the command, wait approximately **5–10 minutes**.
...
<Tabs>
  ...
  <Tab label="Upgrading from 4.0 / 4.1">
