...
If the offered ACP 4.4 target upgrades the cluster to Kubernetes 1.35 and `admin-gates` contains the following key, complete the Kubernetes 1.35 readiness checks before acknowledging it:

```text
ack-4.4-kubernetes-1.35-kernel-update
```

Complete the [Kubernetes 1.35 node readiness checks](./pre-upgrade.mdx#kubernetes-135-node-readiness) on every production node.
