...
After running `bash upgrade.sh --skip-sync-image`, when the **Upgrade** button becomes available in the **Functional Components** tab, you may see that the **Target** version for components such as **Alauda Container Platform Base** is displayed as a lower version.

**Action Required:**
You must manually change the **Target** to **Do not upgrade** for these components. Failing to do so may result in components being downgraded to an older version.
:::
...
After completing the upgrade in the **Functional Components** tab, the **Upgrade** button may still appear clickable.

**Do NOT click the Upgrade button again.** If you click it, you will see that the **Target** for all components that have already been upgraded to the new patch version is displayed as a lower version. Upgrading again will cause these components to be downgraded to an older version.
:::
