...
<Directive type="info" title="INFO">
When managing multiple clusters, use the `--clusters` parameter with [Create only CRs from all packages in a directory](../extend/upload_package.mdx#only_create_cr) to specify all target clusters. 

If you do not specify this parameter, the Operator's new version will be created only in the global cluster, which means the Operators on other workload clusters will not have a new version created and therefore cannot be upgraded.
</Directive>
