...
### External Registry Readiness \{#external-registry-readiness}

If the environment uses an external platform image registry, complete this checklist before the maintenance window:

1. Confirm that `ProductBase.spec.registry.external` is `true` and record the configured address:
...
2. Download the Core Package for the exact target Distribution Version and CPU architecture. Do not reuse an installation or upgrade payload from another version or architecture.
3. Download every target-version Aligned package required by the upgrade scenario and every other Aligned or Agnostic package required by the clusters in scope.
4. From the target Core Package's `installer/` directory, run both upload modes in [Prepare the Target-Version Payload](../install/prepare/external-platform-image-registry.mdx#prepare-the-target-version-payload). The two modes may run in either order, but both must succeed.
5. Publish the remaining Extension packages through the documented `violet push` workflow.

`upgrade.sh` automatically skips image synchronization for an external registry. A successful skip does not prove that the target payload is present.
