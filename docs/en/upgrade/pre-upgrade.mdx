...
| --------- | ----------- | ---------- |
| **Service mesh (Istio)** | All clusters running **1.20 or later** | Upgrade `Istio` and its instances BEFORE upgrading `Kubernetes` or <Term name="productShort" /> |
| **Kubernetes** | All clusters running **1.28 or later** | Upgrade Kubernetes BEFORE upgrading <Term name="productShort" />; be aware to upgrade `Istio` FIRST |
...
<Tabs>
  ...
  | Condition | Requirement | If Not Met |
  | --------- | ----------- | ---------- |
  | **ClickHouse log storage plugin** | Uninstall it **before the upgrade**, then reinstall it after upgrading ||
  </Tab>
</Tabs>
