...
If a required package is missing or is not ready, CVO stalls the cluster upgrade. Add packages from **ACP Upgrade to v4.5** through the `plugins/` publication path in [Sync upgrade artifacts](./upgrade_global_cluster.mdx#sync-upgrade-artifacts); publish other packages with `violet`. CVO detects newly available packages and continues the existing upgrade request automatically.
:::
