...
- If Elasticsearch is installed, you must apply the solution **"How to Correct the Issue of Node Role Settings in Big Cluster Elasticsearch"** before upgrading. The solution is available in **<Term name="company" /> Customer Portal > Knowledge**.  
</Tab>
...
- If ClickHouse is installed, it must be uninstalled before the upgrade and reinstalled afterward.
- Elasticsearch provided in 3.16.x (v6) CAN NOT upgrade to the one provided in 4.0.x (v8); To solve this problem, detailed instructions are provided in the checklist output. Please make sure you have execute the checklist before proceeding the upgrade.
</Tab>
</Tabs>
...
If **Service Mesh v1** is installed in your environment, you must also download **`For upgrading Service Mesh v1 from 3.16.x/3.18.x to 4.0.x`**.
</Tab>
</Tabs>
