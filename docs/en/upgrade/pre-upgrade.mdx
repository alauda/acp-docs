...
From the **<Term name="company" /> Customer Portal**, download the **<Term name="productShort" /> 4.4 Core Package** that matches your target architecture. The Core Package contains Core artifacts only; it does not contain any Aligned or Agnostic Extension package.

### Step 2: Download ACP Upgrade to v4.4 \{#download-acp-upgrade-to-v44}
...
Use the same `violet` installation from Step 2 to inventory installed Extensions and publish packages that are not part of **ACP Upgrade to v4.4**. For more information, see [Download Packages](/extend/download_package.html) and [Upload Packages](/extend/upload_package.html).
