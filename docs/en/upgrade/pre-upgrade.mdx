...
In the Customer Portal, go to **Marketplace > Batch Download > Install > Scenarios** and download **ACP Upgrade to v4.4** for the same architecture as the Core Package.
...
Navigate to the **CLI Tools** section in the **<Term name="company" /> Customer Portal** and download the `violet` tool. `violet` is required for inventorying installed Extensions and publishing packages that are not part of **ACP Upgrade to v4.4**. For more information about `violet`, see [Upload Packages](/extend/upload_package.html).
