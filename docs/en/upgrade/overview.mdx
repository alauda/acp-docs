...
| Aligned plugins (cluster plugins and operators released together with the matching ACP version) | Required for each installed plugin | CVO, after the target-version package is published with `violet`; an individual plugin can also be upgraded out of band through its own workflow when needed | Same |
