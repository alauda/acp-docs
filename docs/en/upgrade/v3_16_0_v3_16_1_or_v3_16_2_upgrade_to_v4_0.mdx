...
# Upgrade <Term name="productShort" /> v3.16.0, v3.16.1 or v3.16.2 To v4.0
...
- This upgrade document applies to upgrading x86 and ARM architecture versions v3.16.0, v3.16.1 or v3.16.2 to version v4.0.0.
- Before upgrading, ensure that all clusters have been upgraded to versions v3.16.0, v3.16.1 or v3.16.2. The k8s version must have been upgraded to at least version 1.28.
...
- The ClickHouse Log Center plugin has reached Beta level in the new version, but it does not support smooth upgrades. If the Clickhouse logging plugin is installed on the platform, please uninstall it before upgrading and instal ACP Log Storage with Clickhouse after the workloads cluster completes the upgrade.
- The CostManager plugin and Kubecost plugin have been discontinued in version 4.0. If the Costmanager plugin and Kubecost plugin are deployed on the platform, please uninstall them before upgrading.
...
1. Accessing any of the master nodes of the primary cluster, execute the following commands to stop the etcd synchronization pod.
```shell
...
kubectl get pod -n kube-system | grep etcd-mirror # make sure there is no etcd-mirror pod remaining
```

### Upgrade the standby cluster

1. Switch the domain name of the cluster access address to point to the **standby cluster**, then wait for 5 minutes and check whether the DNS resolution has been updated.
2. Access the **standby cluster** page using the standby cluster's VIP
    1. If the platform page cannot be accessed via IP, log in to the **standby cluster's** master1 node and modify prdb to add other platform access addresses:
    ```yaml
    spec:
      alternativeURLs:
      - https://192.168.162.10 # Replace IP with the actual standby cluster VIP
    ```
3. Refer to the [Upload Platform Images](#upload-platform-images) paragraph to upload the platform images to the **standby cluster** .
4. Refer to the [Upgrade the global cluster](#upgrade-the-global-cluster) paragraph to upgrade the **standby cluster's** global.
5. Wait for the upgrading of **standby cluster's** global to complete.

### Upgrade the primary cluster

1. Switch the domain name of the cluster access address to point to the primary cluster, then wait for 5 minutes and check whether the DNS resolution has been updated.
2. Refer to the [Upload Platform Images](#upload-platform-images) paragraph to upload the platform images to the primary cluster.
3. Refer to the [Upgrade the global cluster](#upgrade-the-global-cluster) paragraph to upgrade the primary cluster's global.
4. Wait for the upgrading of primary cluster's global to complete.

### Start the synchronization program
1. Start the pod for data synchronization.
    1. Check the 2379 port of the primary and standby cluster VIPs respectively, and confirm that they have been forwarded to their respective master nodes.
    2. Access the **standby cluster** platform page via the **standby cluster** Kubernetes VIP, click on the global cluster plugin deployment page, and then click to deploy the etcd synchronization plugin. Follow the on-screen prompts to enter the parameters, where:
        1. The data check interval is used to generate monitoring data, and the default value can generally be selected.
        2. The switch for printing detailed logs does not need to be turned on by default and is mainly used for debugging issues.
...
### [Upgrade Business Clusters](#upgrade-business-clusters-1)
...
1. Copy the installer to any node of the global cluster and extract it.
2. `cd` into the directory where installer was extracted to.
3. Choose which commands of the following cases to execute according to your use case. Following examples show how to upload the images before proceeding the upgrading, which saves time.
4. **For disaster recovery environment, first check whether the domain of platform access address is pointing to the standby cluster. Upload the images to the standby global cluster first. After the standby global cluster is upgraded, switch the resolution of domain to the primary cluster, and then upload the images to the primary global cluster.**
5. If the platform use the internal registry (provisioned by the ACP itself), run the following command to upload the images:
```shell
bash upgrade.sh --only-sync-image=true
```
6. If the platform use an external registry, run the following command to upload the images:
```shell
...
2. This upgrade document applies to upgrading business clusters from versions v3.16.0, v3.16.1 or v3.16.2 to v4.0.0 for both x86_64 and arm64 architectures.
