...
Before proceeding with the upgrade, you **must back up etcd on every master node** of all clusters (including the primary global cluster, secondary global cluster, and all workload clusters):
...
This backup command must be executed on **every master node** of **all clusters** before the upgrade begins.
</Directive>
