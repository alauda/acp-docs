...
**ACP GitOps**, built on Argo CD, uses the Git repository as the sole trusted source to store application, infrastructure configuration, and other files for rapid and accurate distribution and deployment to one or multiple Kubernetes clusters.
...
## ACP GitOps Architecture

**ACP GitOps** is installed as a cluster plugin on the `global` cluster and utilizes Argo CD for application distribution and infrastructure provisioning across multiple business clusters.

![ACP GitOps Architecture](./assets/architecture_alauda_gitops.png "ACP GitOps Architecture")
