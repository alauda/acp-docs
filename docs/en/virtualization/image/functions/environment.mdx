---
weight: 10
sourceSHA: 4f72c4bba057d25c2cdc26572dd8da3137a999e1864b03d5ea6e708927c9710a
---
...
- If you need to connect to the virtual machine directly via IP, the cluster must use the Kube-OVN Underlay network mode. You can refer to the best practices [Preparing Kube-OVN Underlay Physical Network](/networking/subnet/how_to/kubeovn_underlay_py.mdx).
...
## Deploying the KubeVirt Operator

Please refer to the [Installing Operator]() document to install the **KubeVirt HyperConverged Cluster Operator**.

**Note**: In the **Installation Mode** parameter, you must select **Namespace Mode** and deploy it to the **kubevirt** namespace.

## Creating a HyperConverged Instance\{#4}

1. Enter **Platform Management**.

2. In the left navigation bar, click **App Store Management** > **Operators**.

3. In the **Deployed Operators** tab, click **kubevirt-operator**.
...
5. Switch to YAML view and only replace the ***registry*** specified in the `spec.storageImport.insecureRegistries` field in the example with the correct **virtual machine image repository address**, for example: 192.168.16.214:60080, keeping other parameters at their default values.
...
6. Click **Create** and wait for the CDI and KubeVirt type instances to be automatically created in the resource list, while ensuring that the **status.phase** displayed in the YAML is `deployed`, indicating that the HyperConverged instance has been successfully created.
   ...
   <img src="/en/img/11kubevirt_instances.png" width="600" />

## Configuring Virtual Machine Overcommit Ratio (Optional)

If you need to configure an overcommit ratio for virtual machines, please enter the deployed KubeVirt Operator for separate configuration.

### Important Notes

- Before configuring the overcommit ratio for virtual machines, please first disable the overcommit ratio for the cluster where the virtual machines reside in **Cluster Management** > **Clusters**; or disable the overcommit ratio for the namespace where the virtual machines are located in **Project Management** > **Namespaces**.
...
4. Click on the **Resource Instances** tab, update the **kubevirt-hyperconverged** resource instance, switch to YAML view, and add the following field in the **annotations** section:
     ...
     [
       {
         ...
         "value": {"cpuAllocationRatio": 4}  # In this example, the CPU overcommit ratio is set to 4 
       }
     ]
   ```
