...
The operations below are available in both the classic and new consoles, from the virtual machine **list** (row ⋮ menu) and the **detail** page. Beyond these, the new console adds [Clone](./clone_virtual_machine.mdx), [Pause and Resume](./pause_resume_vm.mdx), and [Serial Console](./console_access.mdx). Each operation also has an equivalent API call — see **Using the API** below.
...
**Note**: For prerequisites, the cluster-wide Migration Tasks / Migration Settings, and the API, see [Virtual Machine Hot Migration](../how_to/vm_precopy.mdx).

1. Access the **Container Platform**.

2. In the left navigation bar, click **Virtualization** > **Virtual Machines**.

3. Locate the target virtual machine and click ⋮ > **Hot Migration**.
...
- **Live Migration** — create a `VirtualMachineInstanceMigration`; see [Virtual Machine Hot Migration](../how_to/vm_precopy.mdx).

## Imported Virtual Machines Using an InstanceType Spec

A virtual machine imported from another platform (for example via Forklift) may define its CPU and memory through a referenced `VirtualMachineClusterInstancetype` (`spec.instancetype`) instead of inline `domain.resources`. The new console resolves the referenced instance type and shows the CPU/memory in the list and detail views.

When you **edit** such a virtual machine's specification, it is converted **once** to a custom inline spec — the console writes `domain.resources` and removes `spec.instancetype` — and the change takes effect after a restart. A raw imported virtual machine without an ACP label can only be edited as YAML.
