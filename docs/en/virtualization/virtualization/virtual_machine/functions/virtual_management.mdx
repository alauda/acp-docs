...
A virtual machine imported from another platform (for example via Forklift) may define its CPU and memory through a referenced `VirtualMachineClusterInstancetype` (`spec.instancetype`) instead of inline `domain.resources`. The referenced instance type is resolved so that its effective CPU and memory can be displayed.

When you **edit** such a virtual machine's specification, it is converted **once** to a custom inline spec — `domain.resources` is written and `spec.instancetype` is removed — and the change takes effect after a restart. A raw imported virtual machine without an ACP label can only be edited as YAML.
