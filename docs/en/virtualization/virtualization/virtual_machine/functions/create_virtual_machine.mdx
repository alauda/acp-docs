...
> **New console**: Create Virtual Machine is a full-page **Form / YAML** wizard with the same configuration areas described below, with a few behavior changes:
>
> - **Boot image** — the boot disk always **clones an existing image**: choose **Provision Method** > **Image Instance** and select a [Bootable Volume](../bootable_volume.mdx). The picker is a single step that shows the OS icon before the name, and the clone source namespace is limited to your current namespace and `kube-public`.
> - **OS hardware profile** — the correct hardware profile (firmware, devices, clock) is applied automatically from the image's operating system; Windows 11 images are provisioned UEFI + Secure Boot + TPM 2.0 compliant by default. CPU and memory still use the resource-limit presets (Small / Medium / Large / Custom).
...
The boot disk is provisioned by the `dataVolumeTemplates` entry, which clones the referenced bootable volume. A virtual machine imported with a `spec.instancetype` reference (instead of inline resources) is also supported and is converted to an inline spec when you edit it — see [Managing Virtual Machines](./virtual_management.mdx).
