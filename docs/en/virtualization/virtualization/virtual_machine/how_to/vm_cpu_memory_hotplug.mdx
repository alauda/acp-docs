...
| Memory scale-down (guest ↓, ≥ boot value) | Requested live via automatic live migration; completion depends on the guest freeing the memory | — (no working driver) | — (no working driver) | Completed online when idle; stalls partially (no error) under memory pressure | Completed online when idle (with `viomem`) |
