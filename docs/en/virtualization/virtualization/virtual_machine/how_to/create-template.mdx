...
> In the **new console**, the building blocks of this workflow are first-class: create the snapshot from the virtual machine's **Snapshot** tab, manage the resulting `DataSource` on the [Bootable Volumes](../bootable_volume.mdx) page, and create a new virtual machine from it via **Provision Method** > **Image Instance**.
