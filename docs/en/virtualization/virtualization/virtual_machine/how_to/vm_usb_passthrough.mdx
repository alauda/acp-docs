...
Now, in the VM configuration, you can add `spec.template.spec.domain.devices.hostDevices.deviceName` to reference the `ResourceName` provided in the previous step and assign it a local name.
For example:
