...
> For the **new console** Clone action (VM detail / list **Actions** > **Clone**), see [Cloning a Virtual Machine](../functions/clone_virtual_machine.mdx). This page covers the command-line workflow.
...
3. **Monitor the Clone Status**:
   ...
   ```bash
   kubectl wait vmclone example-vm-clone --for=jsonpath='{.status.phase}'=Succeeded --timeout=300s
   ```
