...
GPU passthrough requires the GPU to use vfio-pci.
If you have already installed the NVIDIA driver for the GPU, please uninstall it first.
...
1. Execute the following command to deploy the gpu-operator.
   ...
   ```bash
   # Replace <registry> with the repository address where the gpu-operator image is located
   # e.g.: export REGISTRY=build-harbor.example.cn
...
2. In the terminal of the GPU node, execute the following command to obtain the pciDeviceSelector. The `10de:102d` part in the output is the value of pciDeviceSelector. <a id="pciDeviceSelector"></a>
...
5. Execute the following command to add the passthrough GPU.
   ...
   **Note**: When replacing the \<pci-devices-id> part in the command below with the pciDeviceSelector value obtained in [Step 2](#pciDeviceSelector), **all letters in the pciDeviceSelector must be converted to uppercase**. For example, if the pciDeviceSelector value obtained is `10de:102d`, it should be replaced with `export DEVICE=10DE:102D`.
   ...
   - Adding a single GPU card
     ...
     ```bash
     # Replace <pci-devices-id> with the pciDeviceSelector obtained in Step 2, e.g.: export DEVICE=10DE:102D
     export DEVICE=<pci-devices-id>
     # Replace <resource-name> with the resourceName obtained in Step 4, e.g.: export RESOURCE=nvidia.com/GK210GL_TESLA_K80
   ...
   - Adding multiple GPU cards
     ...
     **Note**: When adding multiple GPU cards, each pciDeviceSelector value used to replace \<pci-devices-id> must be unique.
     ...
     ```bash
     # Replace <pci-devices-id1> with the pciDeviceSelector obtained in Step 2
     export DEVICE1=<pci-devices-id1>
     # Replace <resource-name1> with the resourceName obtained in Step 4
     export RESOURCE1=<resource-name1>
     # Replace <pci-devices-id2> with the pciDeviceSelector obtained in Step 2
     export DEVICE2=<pci-devices-id2>
     # Replace <resource-name2> with the resourceName obtained in Step 4
   ...
   - Adding new GPU cards after already adding GPU cards
     ...
     ```bash
     # Replace <pci-devices-id> with the pciDeviceSelector obtained in Step 2
     export DEVICE=<pci-devices-id>
     # Replace <resource-name> with the resourceName obtained in Step 4
     export RESOURCE=<resource-name>
     # index is a zero-based array index, use the number to replace <index>
     # for example: if one GPU card has already been added, and now you want to add another one, index should be 1, i.e., export INDEX=1
     export INDEX=<index>
