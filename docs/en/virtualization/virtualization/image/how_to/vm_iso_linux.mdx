...
4. Build the container image by executing the following command.
   ...
   ```bash
   podman build -t build-harbor.example.cn/3rdparty/vmdisks/ubuntu-iso:24.04 -f Containerfile . # Please replace the repository according to your actual environment
   ```
