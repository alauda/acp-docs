...
> `virtualization.cpaas.io/*` labels used to mark a bootable volume. The image is fully
> usable through the API (by `sourceRef`, as in the next step); to also apply that label
> contract, see [Bootable Volumes](../../virtual_machine/bootable_volume.mdx).
...
> **Tip**: The uploaded image can also be referenced as a bootable volume when creating a
> virtual machine, once it carries the required labels — see
