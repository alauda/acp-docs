...
- `NoGuestAgent` — no guest agent was available, so an online snapshot is **crash-consistent** (equivalent to pulling the power and rebooting). Install the [guest agent](../../virtual_machine/how_to/guest_tools.mdx) for application-consistent snapshots.
