...
```bash
virtctl addvolume vm web-01 --volume-name=datadisk-1 --persist -n demo
```
...
```bash
virtctl removevolume vm web-01 --volume-name=datadisk-1 -n demo
```
...
## StorageProfile and cross-namespace clone permissions
...
**Cross-namespace clone permissions**: when a virtual machine or bootable volume clones a `DataVolume`/`DataSource` from **another** namespace, CDI checks that the requester may create `datavolumes/source` in the **source** namespace. Golden images in `kube-public` are clone-able by everyone out of the box (the `os-images.kubevirt.io` roles grant this to authenticated users). To allow cloning from a different non-public namespace, an administrator binds the CDI cloner ClusterRole (`datavolume-cloner`) in the source namespace:

```bash
kubectl create rolebinding allow-clone-from-src \
  --clusterrole=datavolume-cloner \
  --serviceaccount=<consumer-ns>:default \
  -n <source-ns>
```
