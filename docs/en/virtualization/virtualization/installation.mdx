...
> After you install or upgrade the **KubeVirt HyperConverged Cluster (HCO)** operator, the **Virtualization** entry and its pages appear automatically in the new web console, with the UI version matching the installed operator (including air-gapped clusters) — no manual plugin registration is needed.
