...
- __Supported worker-node operating systems updated.__ v1.2.0 ships pre-compiled driver images only for the (kernel, OS) pairs listed below; clusters running other kernels must build a matching image from [npu-driver-installer](https://gitcode.com/openFuyao/npu-driver-installer):

    | OS | Kernel | Status |
    |---|---|---|
    | KubeOS 6.6 (`oe2403sp3`) | `6.6.0-145.0.4.135` | New in v1.2.0, recommended |
    | openEuler 22.03 LTS SP3 | `5.10.0-182.0.0.95` | Bare-metal |
    | Ubuntu 22.04 | n/a | __Not shipped__ — v1.1.x DKMS path is gone; users wanting Ubuntu must build their own image |
