...
Based on the openFuyao [npu-operator 1.2.0](https://gitcode.com/openFuyao/npu-operator/tree/1.2.0) community release. v1.2.0 is the first release built for the __KubeOS__ immutable operating system and reworks how the driver is delivered, how devices reach workloads, and how the operator manages the driver lifecycle.
...
- __Delivery model changed from cluster plugin to operator.__ Earlier versions of __Alauda Build of NPU Operator__ shipped as a cluster plugin, installed from __Marketplace__ > __Cluster Plugins__. Starting with v1.2.0 the project is packaged as an OLM operator bundle and installed from __Marketplace__ > __OperatorHub__.
    ...
    :::warning
    In-place upgrade from v1.1.3 (or any earlier cluster-plugin release) is __not__ supported. To move to v1.2.0, uninstall the old cluster plugin first and then install the new operator from scratch:
...
- __OS support model changed: kernel-keyed, not distro-keyed.__ v1.2.0 does __not__ restrict to specific distros. Any arm64 Linux running containerd is supported, as long as a driver image matching the node's kernel is available. We publish pre-compiled images at [`docker.io/alaudadockerhub/ascend-driver`](https://hub.docker.com/r/alaudadockerhub/ascend-driver) (currently two kernel versions; new kernel builds can be added on demand from [npu-driver-installer](https://gitcode.com/openFuyao/npu-driver-installer)):

    | Chip | Kernel | OS stem (informational) | Tested distro |
    |---|---|---|---|
    | 910B | `6.6.0-145.0.4.135` | `oe2403sp3` | KubeOS 6.6 (recommended) |
    | 910B | `5.10.0-182.0.0.95` | `oe2203sp3` | openEuler 22.03 LTS SP3 (bare-metal) |
    | 310P | `6.6.0-145.0.4.135` | `oe2403sp3` | KubeOS 6.6 |

    Other kernels (including Ubuntu, RHEL, etc.) are not pre-built but can be produced with the same build pipeline by supplying a different `KERNEL_UNAME`. The v1.1.x DKMS host-side flow is gone in v1.2.0; the container-image model replaces it.

- __Workloads no longer need `runtimeClassName: ascend`.__ Device injection in v1.2.0 is driven by the Container Device Interface (CDI). Requesting `huawei.com/Ascend910` (or `Ascend310P`) is enough — containerd binds `/dev/davinciN` and the driver userspace libraries into the container automatically. Existing pods that still set `runtimeClassName: ascend` continue to work; the legacy RuntimeClass is retained for backwards compatibility.

- __Volcano components no longer bundled.__ `vc-controller` and `vc-scheduler` are owned by the standalone __Volcano__ cluster plugin starting with v1.2.0. Install Volcano separately from __Marketplace__ > __Cluster Plugins__ when you enable the __ClusterD__ component of the NPU Operator; otherwise it is not required.

### New features

- __KubeOS support.__ NPU nodes running KubeOS 6.6 (`oe2403sp3` kernel) are now fully supported. The operator stages the driver tree under `/var/lib/Ascend` / `/var/lib/ascend` / `/home/bios/driver` (all on writable partitions on KubeOS), inserts kernel modules directly from the driver pod, and never writes to `/usr` or relies on a package manager.
