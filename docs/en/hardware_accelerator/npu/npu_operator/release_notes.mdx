...
- __OS support model changed: kernel-keyed, not distro-keyed.__ v1.2.3 does __not__ restrict to specific distros. Any arm64 Linux running containerd is supported, as long as a driver image matching the node's kernel is available. We publish pre-compiled images at [`docker.io/alaudadockerhub/ascend-driver`](https://hub.docker.com/r/alaudadockerhub/ascend-driver) (currently two kernel versions; new kernel builds can be added on demand from [npu-driver-installer](https://gitcode.com/openFuyao/npu-driver-installer)):
    ...
    Other kernels (including Ubuntu, RHEL, etc.) are not pre-built but can be produced with the same build pipeline by supplying a different `KERNEL_UNAME`. The v1.1.x DKMS host-side flow is gone in v1.2.3; the container-image model replaces it.
...
- __Volcano components no longer bundled.__ `vc-controller` and `vc-scheduler` are owned by the standalone __Volcano__ cluster plugin starting with v1.2.3. Install Volcano separately from __Marketplace__ > __Cluster Plugins__ when you enable the __ClusterD__ component of the NPU Operator; otherwise it is not required.
