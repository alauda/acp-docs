...
- __Multi-node distributed workloads need manual HCCN IP configuration.__ Cross-node HCCL collective operations (allreduce, allgather, …) — used by multi-rank PyTorch / MindSpore training and by Prefill–Decode disaggregated inference where P and D run on different nodes — route over each NPU's on-die 100 GbE RoCE NIC, which requires an IP address. v1.2.3 does not assign these IPs automatically. Operators wanting multi-node distributed workloads must configure them by hand on each NPU node with `hccn_tool` and persist them via `/etc/hccn.conf` — see [Manual HCCN IP configuration for multi-node workloads](./installation.mdx#manual-hccn-ip-configuration-for-multi-node-workloads). Single-node multi-NPU workloads and pod-to-pod TCP communication are not affected. Automated HCCN IP allocation (via a new `HCCNConfig` CRD or as a driver runtime-init stage) is planned for a follow-up release.
