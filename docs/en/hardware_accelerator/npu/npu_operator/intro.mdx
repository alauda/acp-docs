...
v1.2.4 is the first release built for the __Alauda OS__ immutable OS, and is delivered as an OLM operator bundle rather than a cluster plugin. The key user-visible changes are:

- __Alauda OS / immutable-OS support__. NPU worker nodes are no longer restricted to traditional Linux distros. The operator handles Alauda OS's read-only `/usr` and absent package-manager constraints transparently — there is no need to pre-install kernel headers, DKMS, or any host package.
