...
v1.2.0 is the first release built for the __KubeOS__ immutable OS, and is delivered as an OLM operator bundle rather than a cluster plugin. The key user-visible changes are:

- __KubeOS support__. Worker nodes running KubeOS 6.6 (`oe2403sp3` kernel) can now host NPU workloads. The operator handles KubeOS's read-only `/usr` and absent package-manager constraints transparently — there is no need to pre-install kernel headers, DKMS, or any host package.
...
- __Volcano no longer bundled__. `vc-controller` / `vc-scheduler` are owned by the standalone __Volcano__ cluster plugin; install it separately if you enable the **ClusterD** component.
