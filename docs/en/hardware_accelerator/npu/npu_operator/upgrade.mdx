...
In manual mode, the operator upgrades NPU nodes one at a time and pauses each one at `node-reboot-required` until you approve it. Watch the per-node progress label:

```bash
kubectl get nodes -L npu.openfuyao.com/driver-upgrade-state -w
```

When a node's `DRIVER-UPGRADE-STATE` column shows `node-reboot-required`, approve the reboot:

```bash
kubectl annotate node ${nodeName} npu.openfuyao.com/approve-reboot=true
```

Repeat for each node as it reaches the gate.
