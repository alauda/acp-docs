...
The operator upgrades NPU nodes one at a time. Watch the per-node progress label:

```bash
kubectl get nodes -L npu.openfuyao.com/driver-upgrade-state -w
```

If `autoUpgrade: true`, the rollout completes without further intervention. If `autoUpgrade: false`, each node pauses at `node-reboot-required` and emits a `RebootRequired` event — approve the reboot when ready:
