...
1. **New driver image present in the cluster registry for every NPU node profile.** Reuse the `skopeo copy` step from [Step 1.1 of installation](./installation.mdx#sync-driver-images) for each `(chip, kernel, OS)` combination:
    ...
    If no tag in the source Docker Hub repo matches your node's kernel, contact <Term name="company"/> Customer Support — see the [installation note on missing tags](./installation.mdx#11-pull-driver-images-from-dockerhub).
...
Scale every NPU-requesting Deployment / StatefulSet / training job to zero before continuing. Restore them after every NPU node's `DRIVER-UPGRADE-STATE` label has cleared (see Step 4).
