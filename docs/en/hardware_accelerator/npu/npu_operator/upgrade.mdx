...
The upgrade walks through three ordered steps: prepare the cluster, trigger the version bump, and wait for the rollout to finish. Do not start triggering until Step 1 is done — that is the single most common cause of a stuck rollout.
...
#### 2.1 Pre-approve every NPU node {#pre-approve-workflow-batch-approval-before-patching}

By default the operator waits for an administrator to approve each node reboot. Annotate every NPU node up front so the rollout proceeds without further intervention:
...
To skip the annotation step entirely, set `spec.driver.upgradePolicy.autoUpgrade: true` on the `NPUOperatorCtl` so the operator reboots nodes automatically. Recommended only for routine version bumps on bare-metal clusters.

#### 2.2 Patch the driver version {#patch-the-driver-version}

Edit the `NPUOperatorCtl` instance you created at the end of [Step 5.3 of installation](./installation.mdx#53-create-the-npuoperatorctl-instance) and change `spec.driver.version`.
...
### Step 3: Wait for the rollout to finish {#step-3-wait-for-the-rollout}

The operator upgrades NPU nodes one at a time. Watch the per-node progress label until every node clears it:

```bash
kubectl get nodes -L npu.openfuyao.com/driver-upgrade-state -w
```

When all values in the `DRIVER-UPGRADE-STATE` column are empty, the rollout is finished. Verify each driver pod is running the new image and the host reports the new `npu-smi` version:
