...
4. (Optional) Run `npu-smi info` on the host. The operator does not symlink `npu-smi` into the host `PATH` (`/usr` is read-only on Alauda OS), so call the binary directly with its libraries loaded:
...
In v1.2.4 the driver pod stages the Huawei tools tree to `/var/lib/Ascend/driver/`, so the binary is at `/var/lib/Ascend/driver/tools/npu-smi`. No host `PATH` symlink is created (Alauda OS keeps `/usr` read-only). Call it with the matching `LD_LIBRARY_PATH`:
