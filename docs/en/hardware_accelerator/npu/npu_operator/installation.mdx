...
3. __Supported NPU hardware__. NPU worker nodes must carry one of:
    - `Ascend 910B`
    - `Ascend 310P`
4. __Worker-node OS — kernel is what matters, not the distro.__ The Huawei driver `.ko` binaries are compiled per kernel version. There is no distro lock-in: any arm64 Linux on containerd works, as long as a driver image matching your node's exact kernel exists at [`docker.io/alaudadockerhub/ascend-driver`](https://hub.docker.com/r/alaudadockerhub/ascend-driver) (or your private mirror).

    - **Check before installing**: run `uname -r` on each NPU node, then list DockerHub tags (see the snippet under [Step 1](#sync-driver-images)) and confirm a tag matches. New kernel images are added continuously — if yours isn't on DockerHub yet, file a build request with the maintainers or run the [npu-driver-installer](https://gitcode.com/openFuyao/npu-driver-installer) pipeline with your `KERNEL_UNAME`; the resulting tag goes to the same registry path with no operator code changes.
    - **Validated combinations** (smoke-tested on real hardware in this release): KubeOS 6.6 / kernel `6.6.0-145.x` (310P + 910B), openEuler 22.03 LTS SP3 / kernel `5.10.0-182.x` (910B bare-metal). Other kernels are not blocked — they just haven't been smoke-tested.
    - Worker nodes that do _not_ carry NPU cards can run any OS; the operator only touches nodes labelled by NFD as having an Ascend PCI device.
...
Unlike v1.1.x, v1.2.3 does __not__ download a `.run` package or run DKMS on the host. The driver, firmware, and runtime tooling are baked into a container image keyed on `(HDK, chip, kernel, OS-stem)`, and loaded into the host kernel directly from the running pod. The image must already exist in the cluster image registry before the driver DaemonSet starts.

Tag pattern:

```
ascend-driver:<HDK>-<chip>-<kernel>-<os-stem>
```

The OS stem is __informational only__ — it disambiguates near-identical kernels from different userspace builds. The operator resolves the exact tag at reconcile time from `spec.driver.version` plus the node's NFD labels; you do not write the full tag in the deployment form.

Public images are published to [`docker.io/alaudadockerhub/ascend-driver`](https://hub.docker.com/r/alaudadockerhub/ascend-driver) and the tag list grows over time as new kernels are built. For air-gapped clusters, mirror the tag(s) matching your nodes' kernels into your private registry — see [Step 1](#sync-driver-images). If your kernel isn't on the list yet, run [npu-driver-installer](https://gitcode.com/openFuyao/npu-driver-installer) with your `KERNEL_UNAME` (or file a build request); the output drops into the same registry path with no operator code changes.
...
Then, on a machine with internet access, mirror each selected tag into your cluster registry (replace the placeholder tag):

```bash
TAG=25.5.0-<chip>-<kernel>-<os-stem>   # e.g. one of the tags from the listing above
...
Mirror __only the tags that match kernels actually running on your NPU nodes__. The operator resolves tags from node labels — extra images in the registry don't hurt, but you must have at least the matching ones. The chart defaults `spec.driver.image.repository` to `mlops/ascend-driver`; override it in the deployment form if your cluster registry uses a different namespace.

#### 1.2 Allow the driver image in ImageWhiteList

ACP gates which images pods are allowed to pull. The bundle's own images get whitelisted automatically by OperatorHub, but the runtime-pulled driver image isn't part of the bundle — every tag the driver DaemonSet may pull has to be listed explicitly in an `ImageWhiteList`.
