...
:::warning
Do this __before__ installing the NPU Operator. The bundle's own images (operator / device-plugin / toolkit / MindCluster sub-components) are pulled and registered automatically by OperatorHub when you install the operator package — those don't need manual sync. The __driver image__ is different: it's pulled at runtime by the driver DaemonSet on each NPU node based on the node's kernel label, and lives at a separate `mlops/ascend-driver` path. If it isn't present in your cluster registry or the path isn't allowed by ImageWhitelist, the DaemonSet pods stay in `ImagePullBackOff` and the operator never reaches `Ready`.
:::
...
#### 1.2 Allow the driver image path in ImageWhitelist

ACP gates which image registries / repositories pods are allowed to pull from. The bundle's own images get whitelisted automatically by OperatorHub when you install the operator package, but the runtime-pulled driver image isn't part of the bundle — its path has to be added manually.

Create or patch an ImageWhitelist that allows the driver path:

```yaml
apiVersion: image-policy.acp.alauda.io/v1
kind: ImageWhitelist
metadata:
  name: npu-operator-driver
spec:
  registries:
...
If you overrode `spec.driver.image.repository` to a different path in the operator deployment form, allow that path instead.
