...
Unlike v1.1.x, v1.2.3 does __not__ download a `.run` package or run DKMS on the host. The driver, firmware, and runtime tooling are baked into a container image keyed on `(HDK, chip, kernel, OS-stem)`. The image must already exist in the cluster image registry before the driver DaemonSet starts.
