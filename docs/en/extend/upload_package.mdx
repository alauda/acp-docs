...
Before digging into the examples, here are some common parameters used in the commands:
...
--username <registry user>                   # The username of the image registry
--password <registry password>               # The password of the image registry
...
--dest-repo <image repository URL>           # Specify the destination image repository URL (used in DR scenarios)
...
```bash
# Either the authentication info of the image registry or `--no-auth` must be provided.
