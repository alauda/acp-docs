...
**Cluster mode is recommended**, as the first two modes will be deprecated in future OLM v1 versions.
...
If multiple Operators share the same namespace, they share a single InstallPlan:

* If any Operator in that namespace has a Manual approval strategy, all Operators in that namespace will switch to Manual mode and cannot upgrade automatically.



## Installing Operators via Web Console
...
3. If the status is **Absent**:
   ...
   * Upload the package to the target cluster using `violet` (see [CLI Documentation](../ui/cli_tools/index.mdx)).
...
```shell
kubectl -n harbor-ce-operator get subs harbor-ce-operator -oyaml
```
...
```shell
kubectl -n harbor-ce-operator get installplan install-27t29
```
...
```shell
kubectl -n harbor-ce-operator patch installplan install-27t29 --type=json -p='[{"op": "replace", "path": "/spec/approved", "value": true}]'
```
...
```shell
kubectl -n clickhouse-operator get subs clickhouse-operator -oyaml
```
