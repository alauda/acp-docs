...
LVM Storage is a node-local storage system based on the community TopoLVM CSI driver. It dynamically provisions persistent volumes from LVM volume groups on cluster nodes. The Marketplace package name is **Alauda Build of TopoLVM**.
...
- Optional: **Alauda Build of LocalStorage**, if you will use **Selection Device**. See [Install Local Storage](../local_storage/installation.mdx).
...
5. On the **Create Cluster** wizard page, add devices.
   ...
   To use **Selection Device**, install **Alauda Build of LocalStorage** first. It provides the device discovery results used by the LVM Storage wizard. See [Install Local Storage](../local_storage/installation.mdx).

   **Selection Device** is disabled until that operator is installed. **Legacy Selection Device** does not require Alauda Build of LocalStorage; it lists disks on the node without discovery results.
