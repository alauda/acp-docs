...
This chapter documents **LVM Storage**. The Marketplace package name remains **Alauda Build of TopoLVM**. For statically provisioned whole-disk or partition PersistentVolumes, see [Local Storage](../local_storage/index.mdx). To choose among Local Storage, LVM Storage, and Ceph Distributed Storage, see [Overview](../overview.mdx).

TopoLVM is a Container Storage Interface (CSI) plugin designed specifically for Kubernetes, aimed at providing efficient and convenient management of node-local LVM volumes.
