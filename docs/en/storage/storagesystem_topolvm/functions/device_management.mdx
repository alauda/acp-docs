...
Storage devices with similar characteristics are typically used in a centralized manner, and these devices are categorized under **Device Classes** in LVM Storage. Using device classes is equivalent to directly using disks, ensuring zero loss and high performance, while also reducing application awareness and dependence on specific devices.

<img src="../assets/topolvm_device.png" width="600" />

## Prerequisites

- At least 1 [Device Class (deviceClasses.classes)](/storage/storagesystem_topolvm/installation.mdx) must have been added when creating the LVM Storage cluster, including devices in the device class.

- There must be at least 1 bare disk present on the node.

## Adding Devices

To use **Selection Device**, install **Alauda Build of LocalStorage** first. See [Install Local Storage](../../local_storage/installation.mdx). **Legacy Selection Device** does not require that operator.
