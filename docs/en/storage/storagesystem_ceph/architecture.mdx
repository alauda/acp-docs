...
After the installed CSV is `Succeeded`, the platform writes the unique CSV `relatedImages` entry whose `name` is `ceph` onto internal `CephCluster.spec.cephVersion.image`. If that image is already the running value, Ceph daemons do not roll. External-mode clusters do not receive a local Ceph image write.
