...
Operator CSV upgrade does not change `CephCluster.spec.cephVersion.image`. Ceph daemons stay on the previous image until you patch that field to the unique CSV `relatedImages` entry whose `name` is `ceph`.
