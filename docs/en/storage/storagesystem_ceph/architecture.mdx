...
Alauda Build of Rook-Ceph follows OLM-managed upgrade workflows through `Subscription`, `InstallPlan`, and `ClusterServiceVersion` (CSV) resources. For the supported operator-plus-data-plane procedure, see [Upgrade](./upgrade.mdx).
...
On 4.4.1 and later, after the installed CSV is `Succeeded`, the platform writes the unique CSV `relatedImages` entry whose `name` is `ceph` onto internal `CephCluster.spec.cephVersion.image`. If that image is already the running value, Ceph daemons do not roll. External-mode clusters do not receive a local Ceph image write. On 4.4.0, the platform does not write that image; use kubectl as described in [Upgrade](./upgrade.mdx).

Then operator-level reconciliation finalizes the remaining configuration:
