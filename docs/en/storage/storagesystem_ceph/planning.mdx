...
If the cluster runs on Immutable Infrastructure, first review <ExternalSiteLink name="immutable-infra" href="/overview/index.html" children="About Immutable Infrastructure" />, then read [Prerequisites for Deploying on Immutable Infrastructure](./installation/immutable_infrastructure_prerequisites.mdx) before installation and use the provider links in that page to plan disks and node replacement.
...
### Internal Mode OSD supply

After you choose Internal Mode, choose how OSD devices are supplied. Do not mix the two backends on the same `CephCluster`.

| OSD supply | How devices are selected | Install procedure | Best fit |
| :--- | :--- | :--- | :--- |
| Host devices | The Internal Mode wizard selects disks or partitions on the node (`Selection Device` or **Input Device**). | [Deploying in Internal Mode](./installation/create_service_stand.mdx) | Default web-console install. |
| Local Storage Block volumes | You provision Block PersistentVolumes with Local Storage first, then Rook consumes them as OSDs. | [Deploying in Internal Mode with Local Storage](./installation/create_service_stand_local_storage.mdx) | You already use Local Storage, or OSDs must stay on the node that holds the volume. |

Host-device **Selection Device** can still use Local Storage discovery. That is not the same as PVC-backed OSDs. PVC-backed OSDs require Available Block PersistentVolumes from [Local Storage Guides](../local_storage/functions/index.mdx).
...
You can encrypt persistent volumes (block only) with storage class encryption using an external Key Management System (KMS) to store device encryption keys. Persistent volume encryption is only available for RADOS Block Device (RBD) persistent volumes. See [how to create a storage class with persistent volume encryption](./how_to/persistent-volume-encryption.mdx).

Storage class encryption is supported in Alauda Build of Rook-Ceph with both HashiCorp Vault KMS and Thales CipherTrust Manager KMS.
...
| :--- | :--- | :--- |
| Storage nodes | 3 nodes | 4 or more nodes distributed across failure domains |
...
For early sizing, start from an aggregate cluster budget rather than from per-component values alone. The following table is intended as a planning reference for a four-node highly available cluster before workload-specific tuning:

| Deployment pattern | Aggregate CPU to reserve for storage | Aggregate memory to reserve for storage | Notes |
| :--- | :--- | :--- | :--- |
| Internal, minimum baseline | 32 logical CPUs | 96 GiB | Entry-level four-node planning baseline when only the minimum deployment target is being met |
| Internal, standard baseline | 40 logical CPUs | 96 GiB | Better starting point for general production planning and future expansion |
| Internal, performance-oriented baseline | 60 logical CPUs | 128 GiB | Suitable when higher throughput or lower latency is required from the beginning |
...
These values should be treated as cluster-level planning targets, not exact scheduler reservations. To estimate per-node budget for a four-node cluster, divide the aggregate numbers evenly across the participating storage nodes.
...
For related operational guidance, see [Managing Storage Pools](./functions/pool_management.mdx), [Adding Devices/Device Classes](./functions/device_class.mdx) for host-device clusters, and [Deploying in Internal Mode with Local Storage](./installation/create_service_stand_local_storage.mdx) when OSDs use Local Storage Block PersistentVolumes.
...
- For a co-resident deployment on host devices, see [Deploying in Internal Mode](./installation/create_service_stand.mdx).
- For a co-resident deployment on Local Storage Block volumes, see [Deploying in Internal Mode with Local Storage](./installation/create_service_stand_local_storage.mdx).
...
- To consume storage services from another cluster or an external Ceph environment, see [Deploying in External Mode](./installation/access_storage_service.mdx).
