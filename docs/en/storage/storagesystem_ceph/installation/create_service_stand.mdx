...
# Deploying in Internal Mode

Internal mode (Standard cluster type in the UI) is the most common deployment method for Ceph storage. It creates a local Ceph cluster in the current business cluster and distributes data replicas across hard drives on different hosts to maintain availability during single-host failures.

This page uses host devices from the **Create Cluster** wizard. To back OSDs with Local Storage Block PersistentVolumes instead, see [Deploying in Internal Mode with Local Storage](./create_service_stand_local_storage.mdx).

:::info
For an Immutable Infrastructure cluster, create the provider MachineTemplate, ConfigPool, and MachineDeployment resources in the global cluster, normally in `cpaas-system`, and create the StorageCluster/CephCluster in the target business cluster. Use provider-managed persistent disks and stable `/dev/disk/by-id/...` identities for OSD devices. Do not follow [Replacing or Removing Storage Nodes](../how_to/replacing-or-removing-nodes.mdx) for an in-place node replacement: do not mark the OSDs `out`, remove the node entry, or purge the OSDs.
:::
...
Internal mode deployment and external mode deployment are mutually exclusive. Choose only one deployment method for a cluster.
...
### (Optional) Install Alauda Build of LocalStorage

To use **Selection Device** when adding host devices, install **Alauda Build of LocalStorage** first. It discovers unused disks on cluster nodes and writes `LocalVolumeDiscoveryResult` objects. **Input Device** does not require this operator.

See [Install Local Storage](../../local_storage/installation.mdx). To provision Block PersistentVolumes and create OSDs from those volumes, do not continue this wizard. Use [Deploying in Internal Mode with Local Storage](./create_service_stand_local_storage.mdx) after [Local Storage Guides](../../local_storage/functions/index.mdx).
...
4. In the **Deploy Operator** wizard page, click the **Deploy Operator** button at the bottom right.
   ...
   - To upgrade an existing Operator later, set the `Subscription` upgrade strategy to **Manual** before you upload a new package. See [Upgrade](../upgrade.mdx).
...
1. In the **Create Cluster** wizard page, configure the relevant parameters and click the **Create Cluster** button at the bottom right.
   ...
   | ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
   | **Cluster Type**     | Select **Standard** (Internal Mode).                                                                                                                                                                                        |
