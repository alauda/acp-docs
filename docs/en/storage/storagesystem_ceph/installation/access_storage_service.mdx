...
# Deploying in External Mode

External mode is used to consume an existing Ceph storage service instead of creating a new local Ceph cluster in the current business cluster. You can either connect to distributed storage in another business cluster on the platform or integrate an external Ceph environment.
...
**Note**: The following steps use **external Ceph storage** as an example. The process is similar when consuming distributed storage from another business cluster.
...
3. On the **Access Configuration** wizard page, choose **External Mode**, and then select **External Ceph**.
   ...
   | **Other Parameters** | Fill in the external Ceph authentication parameters obtained in the prerequisites.                                             |
