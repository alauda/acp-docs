...
# Create Storage Service - Create Standard Type Cluster

Distributed storage provides an integrated storage solution within the cluster offered by the platform. Based on the open-source ROOK + Ceph storage solution, distributed storage implements capabilities for automatic management, scaling, and recovery, satisfying the needs for block storage, file storage, and object storage for small to medium-sized environments.
...
### Create Extended Type Cluster

For details, please refer to [Create Storage Service - Create Extended Type Cluster](./create_service_extend.mdx).
