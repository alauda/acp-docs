...
These steps add host devices (`spec.storage.nodes`). They do not apply when OSDs use Local Storage Block PersistentVolumes. For that cluster, add a device class as described in [Deploying in Internal Mode with Local Storage](../installation/create_service_stand_local_storage.mdx).
