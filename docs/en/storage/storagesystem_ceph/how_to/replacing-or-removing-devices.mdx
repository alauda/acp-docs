...
This document describes how to remove a storage device (disk) from a **host-based** Rook-Ceph cluster (`CephCluster.spec.storage.nodes`). Depending on whether the remaining OSDs have sufficient capacity to absorb the data from the disk being removed, you may need to add a replacement disk first.

If OSDs are backed by Local Storage Block PVs (`CephCluster.spec.storage.storageClassDeviceSets`), use [Replacing or Removing PVC-backed OSD Devices](./replacing-or-removing-devices-pvc.mdx) instead. Do not edit `spec.storage.nodes`. Do not wipe a device that is still bound to a PV; after the PV is `Available`, follow that page to clear leftover BlueStore labels before reuse.
