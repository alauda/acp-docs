...
If OSDs are backed by Local Storage Block PVs (`CephCluster.spec.storage.storageClassDeviceSets`), use [Replacing or Removing PVC-backed OSD Devices](./replacing-or-removing-devices-pvc.mdx) instead. Do not edit `spec.storage.nodes` or wipe the device with `sgdisk`/`dd` for PVC-backed OSDs.
