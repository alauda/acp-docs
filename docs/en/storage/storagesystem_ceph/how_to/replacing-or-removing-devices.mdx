...
This document describes how to remove a storage device (disk) from a **host-based** Rook-Ceph cluster (`spec.ceph.hostStorage` / `CephCluster.spec.storage.nodes`). Depending on whether the remaining OSDs have sufficient capacity to absorb the data from the disk being removed, you may need to add a replacement disk first.

If OSDs are backed by Local Storage Block PVs (`spec.ceph.storageDeviceSets`), use [Replacing or Removing PVC-backed OSD Devices](./replacing-or-removing-devices-pvc.mdx) instead. Do not edit `spec.storage.nodes` or wipe the device with `sgdisk`/`dd` for PVC-backed OSDs.
