...
Distributed storage consists of several components:  
- **ceph-mon**: Primarily maintains the Cluster Map(cluster state diagram), recording cluster membership, configuration, and status information. Handles cluster member authentication and participates in quorum voting.
- **ceph-mgr**: Responsible for tracking runtime metrics and cluster state, providing additional monitoring and interface capabilities. Manages cluster resources (e.g., storage utilization, workload distribution) and offers a dashboard interface for administrators to monitor and manage the cluster.
- **ceph-osd**: Each OSD manages a physical disk. Handles data read/write operations, data replication, failure recovery, and automatic load balancing. Directly processes client I/O requests.
- **ceph-mds**: Manages metadata for the ceph file system. Maintains the directory hierarchy, file permissions, and attributes, while providing a POSIX-compliant file system interface. Deployed only when using ceph file system.
- **ceph-rgw**: Provides S3 and Swift-compatible RESTful APIs for object storage operations. Handles user authentication, access control, and supports multi-tenancy isolation. Deployed only when using ceph object storage.
...
- 2 ceph-mgr  
- multiple ceph-osd
- 2 ceph-mds (if using CephFS)
- 2 ceph-rgw (f using CephObjectStorage) 
...
|CPU|Memory|
|:---|:---|
|16c + (4c * ceph-osd per node)|20Gi + (8Gi * ceph-osd per node)|  
