...
This document describes how to remove a storage node from a **host-based** Rook-Ceph cluster (`CephCluster.spec.storage.nodes`). Depending on whether the remaining OSDs have sufficient capacity to absorb the data from the node being removed, you may need to add a replacement node first.

If OSDs are backed by Local Storage Block PVs (`CephCluster.spec.storage.storageClassDeviceSets`), remove or replace each OSD on the node by following [Replacing or Removing PVC-backed OSD Devices](./replacing-or-removing-devices-pvc.mdx) first, then drain and remove the node. Do not delete `spec.storage.nodes` entries for PVC-backed OSDs.

:::warning
These steps apply to OSDs created from host devices in [Deploying in Internal Mode](../installation/create_service_stand.mdx). Do not use them for OSDs created from Local Storage Block PersistentVolumes. See [Deploying in Internal Mode with Local Storage](../installation/create_service_stand_local_storage.mdx).

This procedure is for permanently removing a storage node and its OSD data. It does not apply to Immutable Infrastructure node replacement. For an Immutable Infrastructure worker whose provider-managed persistent disks are being remounted by pool slot, do not mark the OSDs `out`, delete the CephCluster node entry, purge the OSDs, or treat the persistent disks as replacement disks. Follow [Prerequisites for Deploying on Immutable Infrastructure](../installation/immutable_infrastructure_prerequisites.mdx) and the provider's node replacement procedure instead.
:::
...
### Adjust Component Deployment Configuration \{#adjust-component-deployment-configuration}
