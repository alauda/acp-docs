...
This document describes how to remove a storage node from a **host-based** Rook-Ceph cluster (`spec.ceph.hostStorage` / `CephCluster.spec.storage.nodes`). Depending on whether the remaining OSDs have sufficient capacity to absorb the data from the node being removed, you may need to add a replacement node first.

If OSDs are backed by Local Storage Block PVs (`spec.ceph.storageDeviceSets`), remove or replace each OSD on the node by following [Replacing or Removing PVC-backed OSD Devices](./replacing-or-removing-devices-pvc.mdx) first, then drain and remove the node. Do not delete `spec.storage.nodes` entries for PVC-backed OSDs.

:::warning
This procedure is for permanently removing a storage node and its OSD data. It does not apply to Immutable Infrastructure node replacement. For an Immutable Infrastructure worker whose provider-managed persistent disks are being remounted by pool slot, do not mark the OSDs `out`, delete the CephCluster node entry, purge the OSDs, or treat the persistent disks as replacement disks. Follow [Prerequisites for Deploying Rook-Ceph on Immutable Infrastructure](../installation/immutable_infrastructure_prerequisites.mdx) and the provider's node replacement procedure instead.
:::
