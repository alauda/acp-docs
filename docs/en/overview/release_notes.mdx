...
- **Alauda OS node storage stability** — Provider updates improve the Alauda OS node storage layout by keeping kubelet data on a separate disk and aligning pod-log storage with the container runtime. The provider release notes will identify the supported provider paths.
