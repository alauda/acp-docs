...
For a specific Operator or Cluster Plugin version, use the <Term name="company" /> Customer Portal `ACP compatible versions` field as the compatibility authority. The exact Extension version must list the target <Term name="productShort" /> release as compatible.
...
The default validated capabilities delivered through those mechanisms include:
