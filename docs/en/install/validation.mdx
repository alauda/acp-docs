...
For an external registry, the address must match the prepared registry and the second value must be `true`. Confirm that no discovered artifact is missing:
...
No output is expected. If an artifact reports `Absent` or a Pod cannot pull an image, follow [External Platform Image Registry Troubleshooting](./prepare/external-platform-image-registry.mdx#troubleshooting).
