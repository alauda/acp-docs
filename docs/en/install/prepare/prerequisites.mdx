...
Before running `setup.sh`, complete [Prepare an External Platform Image Registry](./external-platform-image-registry.mdx). That page defines the required address, network access, storage, and push and pull permissions; separates those requirements from production recommendations; and explains how to upload the complete target-version payload.
