...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>Please refer to [LoadBalancer Forwarding Rules](#port-forward) to ensure that the necessary ports are open.</td>
    </tr>
    <tr>
      <td style={{ whiteSpace: 'nowrap' }}>IP Address Range</td>
      <td>The `global` cluster nodes should avoid using the 172.17-18 network segment. If it has been used, please adjust the nerdctl configuration (add the bip parameter) to avoid conflicts.</td>
    </tr>
  </tbody>
</table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        ...
        <b>Note:</b> If you plan to use an external platform image registry instead of the Registry provided by the `global` cluster, you do not need to configure this port. See <a href="./external-platform-image-registry.mdx">Prepare an External Platform Image Registry</a> for the external registry network requirements.
      </td>
    </tr>
  </tbody>
</table>
...
Before installation, confirm that the registry:

- Is available through HTTPS with a certificate chain trusted by the installation node and every current or future cluster node.
- Resolves through DNS and is reachable from the installation node, `global` cluster nodes, and workload-cluster nodes.
- Provides sufficient capacity, availability, credentials, repository permissions, backup, and retention for every required Core and Extension version and architecture.
- Contains the complete target-version payload before `setup.sh` starts.

For the supported upload, installation, validation, upgrade, and troubleshooting workflow, see [Prepare an External Platform Image Registry](./external-platform-image-registry.mdx).
