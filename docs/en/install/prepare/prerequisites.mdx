...
* **Multi-Cluster**
  ...
  It is **not recommended** to run non-platform workloads on the Global cluster, as this may impact platform stability and performance.
...
* **Single Node**
  Select this architecture only for **testing or proof-of-concept** purposes, rather than for production environments.
  In this mode, all <Term name="productShort" /> components run on a single node, with **minimum hardware resources** allocated.
