...
The command above uses the platform built-in Registry. To use an external platform image registry, first upload the complete target-version payload and then start `setup.sh` with the registry parameters described in [Prepare an External Platform Image Registry](./prepare/external-platform-image-registry.mdx#start-the-installer).
...
<table>
  ...
  <tr>
    ...
    <td>
      ...
      <code>External</code> uses a customer-managed platform image registry. The external registry must contain the complete target-version Core and Extension payload before installation. See <a href="./prepare/external-platform-image-registry.mdx">Prepare an External Platform Image Registry</a>.
    </td>
  </tr>
  <tr>
...
| Pod is `ImagePullBackOff` or `ErrImagePull` | `kubectl describe pod <pod>` | Confirm the platform image source is reachable from the failing node and that its DNS, CA trust, credentials, and referenced manifest are available. For external registries, see [Troubleshooting](./prepare/external-platform-image-registry.mdx#troubleshooting). |
