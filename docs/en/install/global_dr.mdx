...
2. Install the standby cluster in the same way as the primary, referring to:
...
3. Get the ETCD encryption key on any of the standby cluster master nodes:
...
4. Merge the ETCD encryption key into the primary cluster's `/etc/kubernetes/encryption-provider.conf` file, ensuring the key names are unique. For example, if the primary cluster's key is `key1`, rename the standby's key to `key2`:
...
resources:
  - resources:
    ...
    - aescbc:
        keys:
        - name: key1
          secret: My4xNDE1OTI2NTM1ODk3OTMyMzg0NjI2
        - name: key2
          secret: MTE0NTE0MTkxOTgxMA==
```

5. Make sure the new `/etc/kubernetes/encryption-provider.conf` file overwrites every replicas on the control plane nodes of both clusters:
...
# and the 1.1.1.1 has already been configured to use both of the ETCD encryption keys:
for i in \
    2.2.2.2 3.3.3.3 \
