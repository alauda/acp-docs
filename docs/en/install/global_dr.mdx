...
* Monitoring and logging data is not synchronized either. Do not store workload cluster monitoring or logging data in the Primary Cluster or the Standby Cluster: install the VictoriaMetrics Center and the log storage that aggregate workload cluster data in a dedicated workload cluster, and let each `global` cluster collect, store, and query only its own metrics and logs. Observability data held in the Primary Cluster is unavailable after a failover, so a Standby Cluster that depends on it cannot observe the platform it has just taken over;
...
* Important: ensure the credentials of image repository and the <Term name="productShort" /> admin user match those set on the **Primary Cluster**.
