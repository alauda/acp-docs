...
By selecting the `History` function from the right-side menu of the specified instance, users will see a pop-up menu displaying all backup records for that instance, as shown in the following image.
...
On the backup history page, after selecting a backup record, users can choose the menu on the side and click the `Recovery` button. The system will pop up a tab for recovery options. Different components may support different recovery methods; however, by default, all will support recovery to a new instance, while MGR additionally supports recovery to an existing instance.
