...
Due to permission considerations, the Alert Management under the Data Services view and the Alert Management under the Platform Management view are isolated from each other at the data level. This means that if a user creates an alert policy in the Data Services view, the management of that policy (such as updating, viewing, and deleting) must still be performed within the Data Services view, and vice versa.
