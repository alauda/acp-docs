...
## Upgrading from 4.0.x to 4.2.1

:::warning
When upgrading from version 4.0.x to 4.2.1 with Application Services installed, manual intervention is required for workload clusters if the RDS updater becomes blocked.
:::

If the RDS updater is blocked, execute the following command:
```bash
kubectl delete moduleinfo -l cpaas.io/module-name=rds-operator-updater
```