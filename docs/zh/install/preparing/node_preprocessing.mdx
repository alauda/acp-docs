...
在安装 `global` 集群前，所有节点（控制节点及计算节点）必须完成预处理工作。

## 执行快速配置脚本

<Term name="productShort" /> 安装包中提供了快速配置节点的脚本。
...
`init.sh` 不能保证以下的所有检查项都得到妥善处理，您仍然需要继续下述步骤。
</Directive>
...
- **操作系统与内核**
  ...
  - ⚠️ 如果 `global` 集群计划使用 `Kube-OVN` CNI 则必须开启内核模块 `geneve`、`openvswitch`。
...
- **节点网络**
  ...
  - ✅ 各节点不得占用以下端口：
    ...
    - **计算节点：** `10249` ~ `10256`
  - ✅ `global` 集群使用 **Kube-OVN** 或 **Calico**，需确保以下端口未被占用：
    - **Kube-OVN：** `6641`、`6642`
    - **Calico：** `179`
...
- **软件与目录要求：**
  ...
  - ✅ 移除与 `global` 集群组件冲突的软件包（详见 [移除冲突软件包](#remove_conflicting_packages)）。
  - ✅ 下列文件若存在则必须删除：
    - `/var/lib/docker`
    - `/var/lib/containerd`
    - `/var/log/pods`
...
在安装之前，节点上可能已有应用在 docker/containerd 环境中运行，或已经安装了与 `global` 集群冲突的软件。因此需要检查并卸载冲突的软件包。

<Directive type="danger" title="DANGER">
- 为避免应用中断或数据丢失，请务必先确认是否存在冲突软件。在发现冲突时，请提前制定应用切换方案并做好数据备份，然后再进行卸载操作。
...
在 Linux 系统中，`/etc/resolv.conf` 文件用于配置 DNS 客户端的域名解析设置。其中，`search` 行指定了 DNS 查询的域名搜索路径。

**配置要求**

- **域名数量**：`search` 行中的域名数量应小于 `domainCountLimit - 3`（默认 `domainCountLimit` 为 32）。
- **单个域名长度**：每个域名不得超过 253 个字符。
- **总字符长度**：所有域名及空格总字符数不得超过 `MaxDNSSearchListChar`（默认为 2048）。
...
- 域名总数量为 3 。
- 单个域名，例如 `domain1.com` 长度为 11 。
- 总字符长度为 35 ，即 11 + 11 + 11 + 2(两个空格) 。

<Directive type="info" title="INFO">
- 如果 `/etc/resolv.conf` 文件中的 `search` 行不符合上述限制，可能会导致 DNS 查询失败或性能下降。
