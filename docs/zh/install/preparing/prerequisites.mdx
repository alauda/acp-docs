...
:::danger
平台目前**不支持在已有 Kubernetes 环境中直接安装 `global` 集群。若您的环境已存在 Kubernetes 集群，请先备份数据并清理环境后再进行安装。
:::
...
<Tabs>
  ...
  **架构说明**
  - `global` 集群以单节点形式部署，集成所有控制与计算功能
  - 所有应用工作负载均在该节点运行（建议 Demo Pod 数量 ≤ 10）
  - 不支持横向扩展，故不适合生产环境
  </Tab>
  ...
  **架构说明**
  - `global` 集群时同时运行平台组件和 ISV 应用 Pod，建议使用节点污点（Taint）对关键组件进行隔离
  - ISV 应用建议在专用计算节点上运行，避免与控制平面资源竞争
  - 扩容建议：每新增 50 个应用 Pod，按 ISV 应用资源需求增加 1 个计算节点
  </Tab>
...
<table>
    ...
    <tbody>
        <tr>
            ...
            <td>必须</td>
            <td>1</td>
            <td>用于集群内各节点访问 kube-apiserver，配置于负载均衡设备中以确保高可用性。此 IP 也可作为平台 Web UI 的访问地址。与 `global` 集群处于同一网络的业务集群也可通过此 IP 访问 `global` 集群。</td>
        </tr>
        <tr>
            <td style={{ whiteSpace: 'nowrap' }}>External IP</td>
            <td>可选</td>
            <td>1</td>
            <td>当存在与 `global` 集群不在同一网络的业务集群时，如混合云场景，则必须提供，其他网络下的业务集群通过此 IP 访问 `global` 集群。需将该 IP 配置于负载均衡设备以确保高可用。此 IP 也可作为平台 Web UI 的访问地址。</td>
        </tr>
        <tr>
            <td>域名</td>
            <td>可选</td>
...
<table>
    ...
    <tbody>
        <tr>
            ...
            <td>同一网络内 `global` 集群与业务集群速率 ≥1Gbps（建议10Gbps）；跨网络速率 ≥100Mbps（建议1Gbps）。速率不足将显著降低数据查询性能。</td>
        </tr>
        <tr>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>
              ...
              <b>注意：</b> 如果您计划使用外部镜像仓库安装 `global` 集群，而非默认的内部镜像仓库，则无需配置此端口。<br />
            </td>
        </tr>
    </tbody>
</table>
