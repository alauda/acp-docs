...
|---|---|
|**添加**|添加一个键值对形式的环境变量。|
|**引用**|引用 **配置字典或 Opaque、用户名/密码类型的保密字典** 中的字段作为环境变量，相关配置参数请参考 [配置字典](/application/building_application/functions/preparation_before_creating/add_configmap.mdx) 或 [保密字典](/application/building_application/functions/preparation_before_creating/add_secret.mdx)。
...
健康检查参数说明请参考 [健康检查](/application/building_application/functions/application_workloads/deployment_create.mdx)。
...
具体参数说明请参考 [配置水平伸缩](/application/building_application/functions/operation_after_creating/add_hpa.mdx)。
...
具体参数说明请参考 [创建入站规则](/application/building_application/functions/operation_after_creating/network_communications/ingress/create_ingress.mdx)。
...
为 Pod 添加资源请求和限制，具体的请求与限制说明请参考 [资源请求与资源限制](/application/building_application/functions/application_workloads/deployment_create.mdx)。
