...
您可通过编写 YAML 的方式，对接外部的 Elasticsearch 集群或 Kafka 集群。您可以根据实际业务需要，仅对接外部的 Elasticsearch 集群（在当前集群安装 Kafka）或同时对接外部的 Elasticsearch 集群及 Kafka 集群。
...
1. 在左侧导航栏中，单击 **集群管理** > **资源管理**，切换至需要安装插件的集群。
