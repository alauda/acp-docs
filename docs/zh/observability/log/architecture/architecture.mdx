...
### 组件安装方式
nevermore 以 daemonset 形式安装在各个集群的 cpaas-system 命名空间下，该组件一共是由 4 个 container 组成:
...
Razor 负责鉴权和接收转发日志消息
   ...
   - 如果安装的日志存储组件是 ElasticSearch，就会将对应的日志写入到 Kafka 集群内。
   - 如果安装的日志存储组件是 Clickhouse，就会将对应的日志传递给 Vector，并最终写入到 Clickhouse。
