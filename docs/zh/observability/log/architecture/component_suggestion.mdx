...
- 当前版本 Devops 产品内 Jenkins 流水线执行记录归档功能不支持使用 Clickhouse，如您需要使用 Jenkins 流水线功能，请谨慎选择 Clickhouse 日志存储组件。
- 当前版本 ServiceMesh 服务网格不支持对接 Clickhouse，如您需要使用服务网格功能，请谨慎选择 Clickhouse 日志存储组件。
- 当前版本 Clickhouse 日志存储组件不支持 IPv6单栈、IPv6双栈业务集群。
...
* 若您比较习惯 Elasticsearch 的使用方式，对 Lucene 语言有较高的依赖，建议您继续选择使用 Elasticsearch 日志存储组件。

* 若您对平台的 Jenkins 流水线或服务网格功能存有依赖，建议您继续选择使用 Elasticsearch 日志存储组件。

* 若您对日志组件的性能和资源消耗有较高的要求，但对于日志查询仅有较基本的使用诉求，建议您选择使用 Clickhouse 日志存储组件。
