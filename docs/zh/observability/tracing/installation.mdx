...
**调用链**组件和**服务网格**组件是互斥关系，如果您已经部署了服务网格组件，请先卸载服务网格组件。
...
* 在 **部署** 页面上：
  ...
  * 为 **升级策略** 选择 **手动**。对于 `手动` 审批策略，OLM 会创建更新请求。作为集群管理员，您必须手动批准 OLM 更新请求以将 Operator 更新到新版本。
  * 选择 **stable 频道**。
...
* 验证 **状态** 是否显示为 **部署成功** 以确认成功安装 Jaeger Operator。
...
* 验证 **状态** 是否显示为 **部署成功** 以确认成功安装 OpenTelemetry Operator。
...
登录已安装的集群，执行以下命令，删除 OpenTelemetry 实例及相关资源。
...
您可以使用 Web 控制台中的 **平台管理** 视图卸载 OpenTelemetry Operator。

**步骤**

- 从 **应用商店管理** → **Operators** → **已部署 Operators** 页面，使用 **搜索框** 搜索 `opentelemetry-operator`。
- 点击 **opentelemetry-operator** 名称，进入详情页。
- 在 **opentelemetry-operator** 详情页中，点击 **操作** →  **卸载 Operator**。
- 在 **确定卸载 opentelemetry-operator 吗?** 窗口中，点击 **卸载**。

### 删除 Jaeger 实例

登录已安装的集群，执行以下命令，删除 Jaeger 实例及相关资源。
...
您可以使用 Web 控制台中的 **平台管理** 视图卸载 Jaeger Operator。

**步骤**

- 从 **应用商店管理** → **Operators** → **已部署 Operators** 页面，使用 **搜索框** 搜索 `jaeger-operator`。
- 点击 **jaeger-operator** 名称，进入详情页。
