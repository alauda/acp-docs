...
在部署集群监控时，平台提供了 VictoriaMetrics 和 Prometheus 两种监控组件供您选择。本文将详细介绍这两种组件的特点和适用场景，帮助您做出最适合的选择。

## 重要说明

- 集群监控组件部署时只能选择 VictoriaMetrics 或 Prometheus 其中之一
...
|-----|------------|-----------------|------|
| 高可用部署 | ❌ | ✅ | VictoriaMetrics 支持真正的集群高可用，数据一致性更好 |
| 单节点部署 | ✅ | ✅ | 两者都支持单节点部署模式 |
...
上图展示了平台支持的监控组件部署架构和数据流向。平台提供以下两种部署方式供选择：
...
#### VictoriaMetrics 部署方式
VictoriaMetrics 支持以下两种部署模式：

1. **单集群部署模式**
   - 对应上图中 **cluster2** 的架构
   - 所有 VictoriaMetrics 组件部署在同一集群
...
2. **多集群部署模式**
   ...
   - 在业务集群部署 VMAgent 作为数据采集代理
   - VMAgent 将数据写入中心监控集群的 VictoriaMetrics
   - 支持多集群统一监控管理
   
   **提示**：部署 VMAgent 前需确保已在监控集群中部署 VictoriaMetrics 服务
