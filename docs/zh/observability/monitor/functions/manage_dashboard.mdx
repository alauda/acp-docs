...
监控面板目前仅支持查看平台安装的监控组件所采集的监控数据，因此在配置监控面板前，你需要事先做好以下准备工作：

- 确保需要配置监控面板的集群已安装了监控组件，即 **Prometheus** 或 **VictoriaMetrics** 插件。
...
- 监控面板会默认请求对应集群的监控数据，如果您在集群中安装的是 **VictoriaMetrics** 插件的代理模式，我们会为您请求存储集群，查询本集群的对应数据，无需您进行特殊配置。
