...
i18n:
  ...
  additionalPrompts: "请将“图表”词组翻译为 pannel，“面板”词组翻译为 dashboard"
---
...
- 面板内支持管理图表，可增、删、改面板内的图表，亦可放大缩小图表，通过托拉拽的方式移动图表位置
- 面板内支持设置自定义变量，用于过滤查询图表内数据。
- 面板内支持配置分组，用与管理面板内的图标。分组支持根据自定义变量重复展示。
- 支持的图表类型有：趋势图、折线图、柱状图、条形图、柱形测量图、环形测量图、表格、统计图、散点图、饼图、文本卡片
- 支持 Grafana 面板一键导入功能

### 功能优势
- 支持用户自定义监控场景,无需受限于预设的模板,真正实现个性化的监控体验。
- 提供丰富的可视化选项,包括折线图、柱状图、饼图等多种图表类型,以及灵活的布局和样式调整功能。
- 与平台的角色权限打通，支持业务视图也能够自行定义监控面板，且能做到数据隔离。
- 与容器平台各项功能深度集成,可即时获取容器、网络、存储等方面的监控数据,为用户提供全方位的性能观察和故障诊断。
- 完全兼容 Grafana 面板 Json，可以轻松的从 Grafana 迁移过来继续使用。
...
监控面板目前仅支持查看平台部署的监控组件所采集的监控数据，因此在配置监控面板前，你需要事先做好以下准备工作：

- 确保需要配置监控面板的集群已部署了监控组件，即 **Prometheus** 或 **VictoriaMetrics** 插件。
- 确保您所需要配置展示的数据已经被采集到了监控组件中。

### 监控面板与监控组件的关系说明

- 监控面板资源存储在 Kubernetes 集群中，您可以通过切换顶部的 **集群** 选项卡来切换查看不同集群中的监控面板。
- 监控面板依赖集群中的监控组件作为数据源查询数据，因此使用监控面板前，您需要确保当前集群已成功部署监控组件，且运行正常。
- 监控面板会默认请求对应集群的监控数据，如果您在集群中部署的是 **VictoriaMetrics** 插件的代理模式，我们会为您请求存储集群，查询本集群的对应数据，无需您进行特殊配置。
...
**变量** | 在创建面板时添加变量，以便在添加的图表中，作为指标参数引用，在面板主页中也可以作为筛选项进行过滤。

2. 添加完成后，单击 **创建**，即可完成面板的创建工作，接下来您需要 **添加变量**、**添加图表**、**添加分组** 以完成整个页面的布局设计。
...
1. 在变量表单区域，单击 **添加**。
    ...
    | 参数 | 说明 | 
    | ---- | ---- |
    |**类型**|当前仅支持 Query 类型的变量，它允许您根据时间序列的特征维度对数据进行过滤。并通过指定查询表达式来动态计算和生成查询结果。|
    |**展示筛选项**| 用于展示面板主页上的下拉筛选项的默认值，支持显示名称和值、值，或不展示，不展示时将隐藏筛选框。|
    |**查询设置**| 使用 Query 类型的变量可以让您根据时间序列的特征维度对数据进行过滤。在定义查询设置时，除了使用 PromQL 查询时间序列进行查询设置外，平台还提供了一些常用的变量和函数，参考 [常用函数以及变量](#ref-function)。|
    |**正则表达式**|使用正则表达式，您可以从变量查询返回的内容中过滤出需要的值。使变量中包含的每个选项的名称更符合预期，您可以在 **变量值预览** 中实时查看过滤后的值是否符合预期。|
    |**选择设置**|- **多选**：在面板主页的顶部筛选项中被选择时，允许同时选择多个选项。您需要在图表的查询表达式中引用该变量，才可以在图表中查看变量值对应的数据。<br />- **全部**：勾选后将在筛选项中启用一个包含 **全部** 的选项，用于全选变量数据。|
...
1. 单击 **添加图表**，参考以下说明，配置相关参数。

- **图表预览**：在图表预览区域将实时展示已添加的指标对应的数据信息。
- **添加指标**：在该区域配置图表标题和监控指标等参数。

- **添加方式**：支持使用内置指标或使用原生自定义指标，两种添加方式将取并集，同时生效。

  - **内置指标**：选择平台内置的常用指标以及图例参数，用于展示当前图表下的数据信息。
    - **注意**：在图表中添加的所有指标的指标单位必须统一，无法在一个图表中添加多个单位的指标。
...
- **图表设置**：支持选择不同的图表类型以可视化展示指标的数据信息，请参阅[管理图表](#pannel)。

2. 单击 **保存**，完成添加图表。

3. 您可以在面板内添加一个或多个图表。

4. 添加图表完成后，您可以使用这两项操作以确保图表的展示和大小能满足您的预期。
...
1. 单击 **添加图表** 的下拉菜单 > **添加分组**，参考以下说明，配置相关参数。
...
2. 添加完成分组后，您可以对分组进行以下操作，以达到管理面板内图表展示的目的。
...
平台提供了多种可视化方式，以支持不同的使用场景，本章节将主要介绍这些图表类型，配置选项和使用方法。

### 图表说明

| 序号 | 图表名称 | 说明 | 使用场景建议 |
| ---- | ---- | --- |----|
|**1**|趋势图|通过一条或多条折线展示数据随时间的变化趋势。|展示随时间变化的趋势,如 CPU 利用率、内存使用量等指标的变化情况。|
|**2**|阶梯折线图|在折线图的基础上,使用水平和垂直线段来连接数据点,形成阶梯状。|适用于展示离散事件发生的时间节点,如告警个数。|
|**3**|柱状图|使用垂直的矩形柱子来表示数据的大小,柱子的高度代表数值。|柱状图能直观比较数值的差异,有利于发现规律和异常，适用于需要关注数值变化的场景，例如容器组个数、节点个数等。|
|**4**|条形图|与柱状图类似,但使用水平的矩形条来表示数据。|当数据维度较多时,条形图能更好地利用空间布局,提高可读性。|
|**5**|环形测量图|使用半圆或圆环形状来表示指标当前的数值及其占总量的比例。|直观反映关键监控指标的当前状态,如系统CPU使用率、内存使用情况等。建议配合警示阈值,以颜色变化提醒指标异常情况。|
|**6**|柱形测量图|使用垂直的矩形柱来表示指标当前值及其占比情况。|直观反映关键指标的当前状态,如目标完成进度、系统负载情况等。在相同指标有多个分类时，更建议用柱形测量图，例如磁盘可用空间，或使用率。|
|**7**|饼图|使用扇形来表示数据各部分占整体的比例关系。|适合展示整体数据在不同维度上的构成情况，如一段时间内4XX、3XX、2XX返回码占比。|
|**8**|表格|以行列形式组织数据,方便查看和比较具体数值。|适合展示结构化的多维度数据，如节点详细信息、容器组详细信息等。|
|**9**|统计图|展示单个关键指标的当前值,通常需要配合文字说明。|适合展示重要监控指标的实时值，如容器组数、节点数、当前告警数等。|
|**10**|散点图|使用笛卡尔坐标系绘制一系列数据点,反映两个变量之间的相关性。|适合分析两个指标之间的关系，通过数据点的分布情况,能发现线性相关、聚类等模式,有助于挖掘指标间的关联性。|
|**11**|文本卡片|使用卡片形式展示关键文字信息,通常包含标题和简要描述。|适合呈现一些文字信息，例如图表说明，排障说明等。|
...
|参数|说明|
|---|---|
|**基本信息**|根据所选的指标数据选择合适的图表类型，并添加标题和描述；您可添加一个或者多个链接地址，添加后，可在标题右侧选择对应链接名称快速跳转。|
|**标准设置**|原生指标数据使用的单位。另外，环形测量图和柱形测量图还支持配置 **总值** 字段，在图中将按照 **当前值/总值** 的百分比显示。|
|**Tooltips**|Tooltips 是鼠标悬浮至图表时，鼠标右侧实时数据的显示开关，并支持选择排序。|
|**阈值参数**|配置图表阈值开关，开启后，阈值将在图表中以所选颜色的形式展示，可设定阈值大小。|
|**值**|设置值的计算方式，例如最新值，最小值。该配置项仅支持统计图、环形测量图。|
|**值映射**|将指定数值或数值范围进行重新定义，例如将数值 100 定义为满负荷。该配置项仅支持统计图、表格、环形测量图。|
...
|**环形测量图**|**环形测量图设置**|<br /> **展示方向**：当您单个图表内需要查看多个指标时，可设置这些指标是横向排列或是纵向排列。<br /> **单位重定义**：您可以为每个指标独立设置它的单位，如不设置则平台会使用 **标准设置** 中的单位进行展示。|
|**饼图**|**饼图设置**|<br /> **最多展示块数**：您可以通过设置此参数来减少饼图的块数，以减轻一些占比较低但数量较多的分类对您的干扰，超出的块数我们会为您合并展示为 **其他**。<br /> **标签展示字段**：您可以设置饼图的标签中所展示的字段。|
|**饼图**|**图形样式**|您可以选择饼图或环形图作为展示样式。|
|**表格**|**表格设置**|<br /> **隐藏列**：您可以通过设置此参数来减少表格的列数，使得您能够聚焦部分主要的列信息。<br /> **列对齐**：您可以通过此项设置修改列中数据的对齐方式。<br /> **显示名称与单位**：您可以可以通过此项设置修改列的列名以及它所使用的单位。|
|**文本卡片**|**图形样式**|<br /> **样式**：您可以选择使用富文本编辑框或 HTML 语言来编辑您期望在文本卡片中期望展示的内容|

## 通过CLI创建监控面板

1. 创建一个名为 `example-dashboard.yaml` 的新 YAML 配置文件。
2. 向 YAML 文件添加 MonitorDashboard 资源并提交该文件。以下示例创建一个名为 demo-v2-dashborad1 的监控面板：
...
metadata:
  ...
  name: demo-v2-dashborad1 # 名称
  namespace: cpaas-system  # 所在ns (管理视图创建都在此 ns)
spec:
  body: # 所有信息字段
    titleZh: 更新显示名称 # 内置字段 中文显示名称 （中文语言下创建为此字段）
    title: english_display_name # 内置字段 英文显示名称 （英文语言下创建为此字段）  内置面板可设置中英文翻译
    templating: # 自定义变量
      list:
        - hide: 0 # 0 表示都不隐藏，1 表示只隐藏标签，2 表示隐藏标签和值
          label: 集群 # 内置变量显示名称 （label 会根据语言不通设置不通 名称 英文下为 cluster)
          name: cluster # 内置变量名称 （唯一）
          options: # 可定义下拉选项， 如 query 能请求到，则使用请求的数据，反之使用 options  并且可设置默认值 （一般只用于设置默认值）
            - selected: false # 是否默认选择
              text: global
              value: global
          type: custom # 自定义变量类型 页面暂时只支持 内置（custom）、 query 两种（导入 grafana 支持 constant custom interval（导入后会转为 custom 变量不支持 auto）
 
        - allValue: "" # 选择所有传递的是 options选项下|分割(xxx|xxx|xxx); 可设置 allValue 进行转换 （grafana 为当前变量的所有数据  xxx|xxx|xxx 后面会调整成一致)
          current: null # 变量当前值， 如不设置 为 list 第一个
          ...
          sort: 2 # 排序，1：按字母顺序升序排序 2：按字母顺序降序排序 （暂时只支持这两种） 3：按数值升序排序 4：按数值降序排序
          type: query # 自定义变量类型
    time: # 面板时间
      from: now-30m # 开始时间
      to: now # 结束时间
    repeat: '' # row 重复行配置，选择的是自定义变量
    collapsed: 'false' # row 是否展开收起配置
    description: "123" # 描述 （标题后的问号描述）
    targets: # 数据源
      - indicator: cluster.node.ready # 指标
        expr: sum (cpaas_pod_number{cluster=\"\"}>0) # promql 表达式
        instant: false # 查询模式 true 查询时间点的数据
        legendFormat: "" # 图例
        range: true # 查询默认 range 查询时间范围的数据
        refId: 指标1 # 唯一标识 数据源显示名称
    gridPos: # 面板的位置信息
        h: 8 # 高度
        w: 12 # 宽度 （宽度为 24 栅格）
        x: 0 # 横坐标位置
    ...
    panels: # 面板数据
      title: 图表标题tab # 图表名称
      type: table # 图表类型 暂支持 timeseries、barchart、stat、gauge、table、bargauge、row、text、pie （阶梯图、散点图、柱状图、通过 drawStyle 属性配置）
      ...
      transformations: # 数据转换
        - id: "organize" # 类型 organize  用于排序、调整顺序、显示字段、是否展示
          options:
            excludeByName: # 隐藏字段
        ...
        - id: "merge" # 合并数据
          options:
      fieldConfig: # 用于定义 图表的属性和外观
        defaults: # 默认配置
          custom: # 自定义图标属性
            align: "left" # table 对齐方式 left center right
            cellOptions: # table 阈值配置
              type: color-text # 只支持 text 阈值颜色设置
            spanNulls: false # true 连接空值 false 不连接  number == 0 按照 0 连接空值
            drawStyle: line # 图表类型 line 线、 bars 条形图、 points 点图、
            fillOpacity: 20 # drawStyle 为area 时存在（暂不支持配置 area 下默认为20）
            thresholdsStyle: # 配置阈值展示形式 （暂只支持 线）
              mode: line # 线条形式展示，（area 暂不支持）
            lineInterpolation: 'stepBefore' # 折线图配置 默认只支持 stepBefore （stepAfter 后续会支持）
          decimals: 3 # 小数点
          min: 0 # 最小值 （暂不支持页面配置 支持导入已适配）
          max: 1 # 最大值 （页面配置仅适用于 stat gauge barGauge pie）
          unit: "%" # 单位
          mappings: # 值映射配置 （暂时只支持 value、range 两种类型、 数据上支持 special 类型)
            - options: # 值映射规则
                "1": # 对应的数值
                  index: 0
                  text: "Running" # 数值为1 展示为Running
              type: value # 值映射
            - options: # 范围映射规则
                from: 2 # 范围开始值
                to: 3 #  范围结束值
            ...
            - type: special # 特殊映射类型
              options:
                ...
                result:
                  ...
                  index: 2
          thresholds: # 阈值配置
            mode: absolute # 阈值配置模式 绝对值模式（暂时只支持 absolute、percentage 百分比模式暂不支持）
            steps: # 阈值步骤
              - color: "#a7772f" # 阈值颜色
                value: "2" # 阈值值
              - color: "#007AF5" # 无value 值的是默认的 Base
        overrides: # 覆盖配置
          - matcher:
              id: byName # 根据字段名称匹配
              options: node # 对应名称
            properties: # 覆盖配置 id 暂时只支持 displayName unit
              ...
              - id: noValue # 没有值展示
                value: 无值展示
      options:
        orientation: horizontal # 控制图表布局方向 生效于 gauge barGauge (stat 后续会支持)
        legend: # 图例配置
          calcs: # 计算方式（仅在 图例位置在 right 下展示）
            - latest # 暂时只支持最新值
          placement: right # 图例位置 右侧、底部、（默认为 bottom)
          placementRightTop: "" # 右上侧配置
          showLegend: true # 是否展示图例
        tooltip: # 工具提示
          mode: multi # 模式 多选模式 （仅支持多模式）鼠标移入展示所有数据
          sort: asc # 排序 asc desc 升序 降序
        reduceOptions: # 值 计算方式 （用于聚合数据）
          calcs: # 计算方式 （最新值、最小值、最大值、平均值、求和）
            - latest
          limit: 3 # pie 限制快数
        textMode: 'value' # stat 配置 定义指标数值的显示样式 auto value value_and_name name none （页面暂时不支持配置，支持导入适配）
        colorMode: 'value' # stat 配置定义指标数值显示颜色模式 none value background （默认 value,不支持配置, 但导入已适配）
        displayLabels: ['name','value', 'percent'] # 饼图 标签展示字段
        pieType: "pie" # 饼图类型 pie饼图 donut环形图
        mode: 'html' # text 图表类型 模式  html、richText富文本
        content: '<div>xxx</div>' # text 文本图表类型 内容
        footer:
          enablePagination: true # table 是否开启分页
```
...
| **label_values(label)** | 返回 Prometheus 所有监控指标中，标签名为 label 的所有可选值，例如：label_values(job) |
| **label_values(metric, label)** | 返回 Prometheus 所有监控指标 metric 中，标签名为 label 的所有可选值，例如：label_values(up{}, job) |
...
1. 添加第一个变量：namespace，使用 **query_result(query)** 函数查询值：`kube_namespace_labels`，并提取 namespace 。
    ...
    * **变量值预览** 区域，预览示例包含 `argocd` `cpaas-system` 等多个命名空间。
