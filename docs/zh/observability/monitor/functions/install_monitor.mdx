...
监控组件是可观测模块中监控、告警、巡检、健康检查等功能的基础设施。本文介绍如何在集群内部署 Monitoring with Prometheus 插件 或 Monitoring with VictoriaMetrics 插件。
...
* 如在业务集群部署，需确保 global 集群可访问业务集群的 11780 端口
* 如需使用存储类或持久卷存储监控数据，请提前在**存储**功能下创建相应资源

## 部署 Monitoring with Prometheus 插件

### 部署步骤

1. 导航至 **应用商店管理** > **集群插件**，选择目标集群
2. 找到 **Monitoring with Prometheus 插件** 插件并点击**部署**
3. 配置以下参数：
...
## 部署 Monitoring with VictoriaMetrics 插件
...
1. 导航至 **应用商店管理** > **集群插件**，选择目标集群
2. 找到 **Monitoring with VictoriaMetrics** 插件并点击**部署**
3. 配置以下参数：
