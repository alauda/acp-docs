---
weight: 50
sourceSHA: d1a436ccc3c064cac3323520d8cc0854c2368ae8f41d5ceb61ffd9f22c566ec7
---
...
平台的黑盒监控功能基于 Blackbox Exporter 实现，允许用户通过 ICMP、TCP 或 HTTP 对网络进行探测，以快速识别平台上发生的故障。
...
3. 参考以下说明配置相关参数。
   ...
   | 参数                | 说明                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
   | ------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
   | **探测方式**         | **ICMP**：通过对 **目标地址** 中输入的域名或 IP 地址进行 ping 探测，以检查服务器的可用性。<br />**TCP**：通过监听 **目标地址** 中输入的 `<域名:端口>` 或 `<IP:端口>` 中的业务端口进行探测。<br />**HTTP**：探测 **目标地址** 中输入的 URL 以检查网站的连通性。<br />**提示**：HTTP 探测方式默认仅支持 GET 请求；如需使用 POST 请求，请参见 [自定义 BlackboxExporter 监控模块](#Blackbox2)。 |
   | **探测间隔**         | 探测的时间间隔。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
   | **目标地址**         | 待探测的目标地址，最大长度为 128 个字符。<br />不同 **探测方式** 的目标地址输入格式有所不同：<br />**ICMP**：域名或 IP 地址，例如 `10.165.94.31`。<br />**TCP**：`<域名:端口>` 或 `<IP:端口>`，例如 `172.19.155.133:8765`。<br />**HTTP**：以 http 或 https 开头的 URL，例如 `http://alauda.cn/`。                                                                                                                                                               |

4. 单击 **创建**。

   创建成功后，您可以在列表页面实时查看最近的探测结果，并基于黑盒监控项[创建告警策略](#Blackbox1)。当检测到故障时，系统将自动触发警报，以通知相关人员进行修复。
...
3. 参考以下说明配置相关参数；更多参数信息请参见 [创建告警策略](./manage_alert.md#alert_ui)。
...
4. 单击 **创建**。提交告警策略后，您可以在告警策略列表中查看该策略。

## 自定义 BlackboxExporter 监控模块 \{#Blackbox2}

您还可以通过向 BlackboxExporter 的配置文件添加自定义监控模块来增强黑盒监控的功能。例如，添加 **http\_post_2xx** 模块后，当黑盒监控的探测方式设置为 `HTTP` 时，它将能够探测 POST 请求方法的状态。
...
1. 更新黑盒监控的配置文件，为 **key** `modules` 添加自定义监控模块。
   ...
   有关黑盒监控配置文件完整的 YAML 示例，请参见 [参考信息](#Blackbox3)。
...
4. 向 YAML 文件添加 PrometheusRule 资源并提交。以下示例创建一个名为 `policy` 的新告警策略：
