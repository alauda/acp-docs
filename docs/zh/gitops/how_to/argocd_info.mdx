...
5. **单点登录**
   ...
   - 未开启：平台部署账号默认使用 `admin`，密码信息则需在 **Kubectl** 中执行如下命令获取 [获取 Argo CD 密码](#argocd-password-cli)
   - 未开启：平台部署账号默认使用 `admin`，密码信息则需在 **Kubectl** 中执行如下命令获取 [获取 Argo CD 密码](#argocd-password-cli)
...
### 获取 Argo CD admin 账户密码<a id="argocd-password-cli"></a>
### 获取 Argo CD admin 账户密码<a id="argocd-password-cli"></a>
...
通过 **Alauda GitOps** 或者 **Argo CD Operator** 部署 Argo CD 自动创建的默认 `admin` 账号密码不支持通过 **Argo CD 原生 UI** 界面中修改密码，可通过 CLI 工具中执行下述命令进行修改。其中，`newpassword` 是需要修改的新密码。
