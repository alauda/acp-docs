...
# Alauda GitOps 介绍
Alauda Container Platform (ACP) GitOps (简称：Alauda GitOps) 是基于 Argo CD 实现。通过 Argo CD 监视 Git 仓库中的配置文件（应用程序、基础设施配置等）并将其自动同步到目标环境，实现了基于 Git 的持续交付流程，通过将整个交付过程纳入 Git 版本控制系统，提高了软件交付的速度、可靠性和安全性，实现应用快速精准分发、部署到一个或多个 Kubernetes 集群。
...
- **原生 Argo CD Operator 完整能力**

  - 提供原生 Argo CD Operator 完整产品功能，包括 Argo CD Operator 的部署、升级、回滚等操作以及 Argo CD 的完整产品功能。
