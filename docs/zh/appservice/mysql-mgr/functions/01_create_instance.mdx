...
- `<instance_name>` 是实例名称，用于标识实例，不可重复。
...
- `<instance_name>` 是实例名称，用于标识实例，不可重复
- `<namespace>` 是实例所属的命名空间
- `<spec.mgr.members>` 是 Group Replication 集群节点数量，建议至少3个节点
- `<spec.mgr.resources.server>` 是 MySQL 节点的资源配置，配置同 Pod 资源配置
- `<spec.mgr.volumeClaimTemplate>` 是 MySQL 节点的数据卷配置
- `<spec.mgr.router.replicas>` 是 MySQL Router 实例数量
- `<spec.params.mysql.mysqld>` 是 MySQL 参数配置，配置项同 MySQL 8.0 参数
- `<spec.mgr.monitor.enable>` 是否启用 Prometheus 监控
- `<spec.mgr.router.svcRW.type>` 读写服务类型，支持 ClusterIP/NodePort/LoadBalancer
- `<spec.version>` 必须指定为 "8.0"
