...
各字段的说明，参考 [RedisUser API](/apis/kubernetes_apis/appservice/redis/redisuser)。
...
Redis 5 不支持 ACL，也就没有 RedisUser 资源来管理 Redis 的用户。针对 Redis 实例，有字段 `spec.passwordSecret` 来记录当前实例的密码(具体文档参考：[Redis API 文档](/apis/kubernetes_apis/appservice/redis/redis))。
