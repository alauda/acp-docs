...
请参考官方 Kubernetes 文档：[Deployments](https://kubernetes.io/docs/concepts/workloads/controllers/deployment/)

> **Deployment** 是 Kubernetes 的一种高级工作负载资源，用于声明性地管理和更新应用程序的 Pod 副本。它提供了一种强大而灵活的方式来定义应用程序的运行方式，包括要维护多少个副本以及如何安全地执行滚动更新。

**Deployment** 是 Kubernetes API 中的一个对象，用于管理 Pods 和 ReplicaSets。当您创建一个 Deployment 时，Kubernetes 会自动创建一个 ReplicaSet，后者负责维护指定数量的 Pod 副本。

**通过使用 Deployments，您可以**：

- 声明式管理：定义应用程序的期望状态，Kubernetes 会自动确保集群的实际状态与期望状态匹配。
- 版本控制和回滚：跟踪每个 Deployment 的修订版本，并在出现问题时轻松回滚到先前的稳定版本。
- 零停机更新：使用滚动更新策略逐步更新应用程序，而不会中断服务。
- 自我修复：如果 Pod 实例崩溃、被终止或从节点中移除，Deployments 会自动替换它们，确保始终有指定数量的 Pods 可用。

**工作原理**：

1. 您通过 Deployment 定义应用程序的期望状态（例如，使用哪个镜像，运行多少个副本）。
2. Deployment 创建一个 ReplicaSet，以确保指定数量的 Pods 正在运行。
3. ReplicaSet 创建并管理实际的 Pod 实例。
4. 当您更新 Deployment（例如，改变镜像版本）时，Deployment 会创建一个新的 ReplicaSet，并根据预定义的滚动更新策略逐步用新的 Pods 替换旧的 Pods，直到所有新的 Pods 都在运行，然后移除旧的 ReplicaSet。
...
spec:
  ...
  selector:
    matchLabels:
      app: nginx # 选择器，用于匹配由此 Deployment 管理的 Pods
  template:
    metadata:
      labels:
...
```bash
# 步骤 1：通过 yaml 创建 Deployment
kubectl apply -f example-deployment.yaml

# 步骤 2：检查 Deployment 状态
kubectl get deployment example-deployment.yaml # 查看 Deployment
kubectl get pod -l app=nginx # 查看由此 Deployment 创建的 Pods
```
