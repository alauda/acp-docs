...
|Key|Value|说明|
|---|---|---|
|service.beta.kubernetes.io/aws-load-balancer-type|external：使用 AWS 官方的 AWS LoadBalancer Controller。|指定所使用的 LoadBalancer 类型内部路由控制器。<br/><br/><b>注意</b>：请提前联系平台管理员部署 AWS LoadBalancer Controller。|
|service.beta.kubernetes.io/aws-load-balancer-nlb-target-type|<ul><li>instance：流量将通过 NodePort 送往容器组。</li><li>ip：流量直接路由到容器组（集群需使用 Amazon VPC CNI）。</li></ul>|指定流量送达容器组的方式。|
|service.beta.kubernetes.io/aws-load-balancer-scheme|<ul><li>internal：内网。</li><li>internet-facing：公网。</li></ul>|指定使用内网或公网。|
|service.beta.kubernetes.io/aws-load-balancer-ip-address-type|<ul><li>ipv4</li><li>dualstack</li></ul>|指定支持的 IP 协议栈。|
...
|Key|Value|说明|
|---|---|---|
|service.beta.kubernetes.io/azure-load-balancer-internal|<ul><li>true：内网。</li><li>false：公网。</li></ul>|指定使用内网或公网。|
