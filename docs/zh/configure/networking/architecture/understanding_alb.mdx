...
4. 该前端本身的协议。
   - http|https|grpc|grpcs`，用于 [l7 proxy](../concepts/timeout.mdx)。
   - tcp|udp`，用于 [l4 proxy](../concepts/timeout.mdx)。
5. 对于 l4 代理，需要使用 `serviceGroup`。对于 l7 代理，`serviceGroup` 是可选的。
   是可选项。当请求到达时，ALB 将首先尝试根据与此 Frontend 关联的规则对其进行匹配。
   规则进行匹配。只有当请求与任何
   规则时，ALB 才会将其转发给前端配置中指定的默认 `serviceGroup` 。
