...
Alauda 容器平台支持 Kubernetes 生态系统中的多种 Ingress 流量规范。本文档对它们进行比较（[Service](../functions/configure_service.mdx), [Ingress](../functions/configure_ingress.mdx), [Gateway API](./gatewayapi.mdx), 和 [ALB Rule](../how_to/create_loadbalancer.mdx#alb_rule)），以帮助用户做出正确的选择。
