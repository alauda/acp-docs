...
身份验证是一种机制，用于在请求到达实际服务之前进行身份验证。它允许您在 ALB 层统一处理身份验证，而无需在每个后端服务中实现身份验证逻辑。
...
1. **转发身份验证（外部身份验证）**
   - 向外部身份验证服务发送请求以验证用户的身份
   - 适用场景：需要复杂的身份验证逻辑，如 OAuth、SSO 等
   - 工作流程：
     1. 用户请求到达 ALB
     2. ALB 将身份验证信息转发到身份验证服务
     3. 身份验证服务返回验证结果
     4. 根据身份验证结果，决定是否允许访问后端服务
...
2. **路径级别身份验证**
   - 在特定的 Ingress 路径上进行配置
   - 在特定的规则上进行配置
   - 可以覆盖全局身份验证配置

3. **禁用身份验证**
   - 针对特定路径禁用身份验证
   - 在注释中配置： `alb.ingress.cpaas.io/auth-enable: "false"`
   - 在规则中使用 CR 进行配置。
...
ingress-nginx 定义了一系列注释以配置身份验证过程的具体细节。以下是 ALB 支持的注释列表，其中 "v" 表示支持，"x" 表示不支持。

|                                                                                                                                                        | support | type                | note                                                                                       |
| ------------------------------------------------------------------------------------------------------------------------------------------------------ | ------- | ------------------- | ------------------------------------------------------------------------------------------ |
| forward-auth                                                                                                                                           |         |                     | 通过发送 http 请求进行转发身份验证                                                       |
...
| [auth-tls](https://github.com/kubernetes/ingress-nginx/blob/main/docs/user-guide/nginx-configuration/annotations.md#client-certificate-authentication) |         |                     | 当请求是 https 时，额外验证证书。                                                         |
...
值是以 `ns/name` 格式的 configmap 引用。
默认情况下，所有来自 cli-request 的头部将发送到 auth-server。可以通过 proxy\_set\_header 配置附加头部。默认情况下，发送的头部如下：
...
值是以逗号分隔的字符串，允许我们将特定头部从 auth-response 带到 app-request 中。
示例：
...
当 ALB 发起 app-request 时，它将包括来自 auth-response 头部的 Remote-User 和 Remote-Name。

### cookie 处理

auth-response 和 app-response 都可以设置 cookie。默认情况下，只有在 app-response.success 时，auth-response.set-cookie 将合并到 cli-response.set-cookie 中。
...
当 auth-server 返回 401 时，我们可以在 cli-response 中设置重定向头，以指示浏览器重定向到签名指定的 url 进行验证。

```mermaid
sequenceDiagram
client ->> alb: 客户端请求 (cli-request)
alb ->> auth-server: alb 请求到 auth-server (auth-request)
auth-server ->> alb: auth-server 回复未 200 (auth-response)
alb ->> client: 在身份验证失败的情况下，alb 回复 cli-response（带有重定向的 location 头）
```
...
基本身份验证是 [RFC 7617](https://datatracker.ietf.org/doc/html/rfc7617) 中描述的身份验证过程。
交互过程如下：
...
ALB CR 已添加身份验证相关配置项，可以在 ALB/Frontend/Rule CR 上配置。
...
在处理 ingress 的过程中，alb 根据注释的前缀来确定优先级。
优先级从高到低为：

- `index.$rule_index-$path_index.alb.ingress.cpaas.io`
- `alb.ingress.cpaas.io`
- `nginx.ingress.kubernetes.io`
  这可以解决与 ingress-nginx 的兼容性问题，并在特定的 ingress 路径上指定身份验证配置。
...
ALB 添加的新注释，用于指定是否为 ingress 启用身份验证功能。

## ingress-nginx 身份验证相关的其他功能

### global-auth

在 ingress-nginx 中，您可以通过 configmap 设置全局身份验证。这相当于为所有 ingress 配置身份验证。在 alb 中，您可以在 alb2 和 ft CRs 上配置身份验证。它们下面的规则将继承这些配置。

### no-auth-locations

在 alb 中，您可以通过在 ingress 上配置注释： `alb.ingress.cpaas.io/auth-enable: "false"` 来禁用该 ingress 的身份验证功能。
...
3. 不支持 auth-cache
4. 不支持 auth-tls
5. 基本身份验证仅支持 basic，不支持 digest
6. 基本身份验证基本仅支持 apr1 算法，不支持 bcrypt sha256 等。

## 故障排除

1. 检查 alb pod nginx 容器日志
