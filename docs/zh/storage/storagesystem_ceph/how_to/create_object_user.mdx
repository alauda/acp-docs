...
|**displayName**|用户的显示名称。|
|**ObjectStore**|对象存储池名称。|
|**clusterNamespace**|**可选**<br></br>`cepphcluster` 和 `CephObjectStore` 所在的命名空间。如果不指定，则用户必须与集群和对象存储在同一个命名空间中。要启用该特性，CephObjectStore `allowUsersInNamespaces` 必须包含该用户的命名空间。|
...
如果 CephObjectStoreUser 创建在非 Rook 集群命名空间中，则必须将该命名空间添加到允许的命名空间列表中，或者指定 `*` 表示允许所有命名空间。这对于需要在自己的名称空间中创建对象存储凭据的应用程序非常有用。
