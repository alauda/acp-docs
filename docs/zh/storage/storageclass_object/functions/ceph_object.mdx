...
* **对接平台内部 Ceph 对象存储**：\{#in}
    ...
    * **密钥**：在 **部署 Ceph 对象存储卷插件** 所在集群的 CLI 工具中依次执行如下六条命令，创建名称为 **ceph-cosi-ops-secret** 的保密字典并保存相关密钥信息。
        ...
        ```
        ```
        export ACCESS_KEY=$(kubectl exec -ti -n rook-ceph $(kubectl get pod -n rook-ceph | grep tools | awk '{print $1}') -- radosgw-admin user info --uid=cosi-ops | grep access_key | awk '{print $2}' | sed 's/"//g' | sed 's/,//g')
        ```
        ```
        export SECRET_KEY=$(kubectl exec -ti -n rook-ceph $(kubectl get pod -n rook-ceph | grep tools | awk '{print $1}') -- radosgw-admin user info --uid=cosi-ops | grep secret_key | awk '{print $2}' | sed 's/"//g' | sed 's/,//g')
        ```
        ```
