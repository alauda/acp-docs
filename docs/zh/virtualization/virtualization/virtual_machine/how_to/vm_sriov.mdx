...
1. 下载 <a href="/scripts/dpdk-devbind.py" download="dpdk-devbind.py">绑定脚本</a> 文件，并执行 `$ python3 dpdk-devbind.py -b vfio-pci <带域标识符的 VF 地址>` 命令将 enp5s0f1 网卡的 8 个 VF 绑定至 vfio-pci 驱动程序，具体示例如下。
    ...
    ```
2. 绑定成功后执行下述命令查看绑定结果，在回显结果的 **Network devices using DPDK-compatible driver** 区域下查看已经绑定的 VF 的驱动。其中，VF 设备 ID 为 `10ed`。\{#VF_ID}
...
4. 将当前网卡以 `**<网卡名称>: <厂商 ID> <设备 ID> <VF 设备 ID>**` 的形式，添加到 data 区域的支持列表中，具体示例如下。\{#configmap1}
