...
3. 正常关机前，执行下述命令驱逐需关机节点上的所有虚拟机 Pod，出现如下回显信息则表示已经驱逐成功。
    ...
    ```
5. 等所有虚拟机在其他节点上启动之后，将节点关机。   
4. 节点关机并重新开机后，执行下述命令，标记该节点为可调度节点。
...
5. 至此，该节点上的原虚拟机实例已迁移至其他正常节点，此节点重启后已允许新的 Pod 调度。
...
3. 执行下述命令删除该节点上的 volumeattachment。
    ...
    ```
4. 执行下述命令查询异常宕机节点上是否存在具有标签 kubevirt.io=virt-api 的 Pod。
...
5. 执行下述命令查询异常宕机节点上是否存在具有标签 kubevirt.io=virt-controller 的 Pod。
