...
4. 在表单页面填写下述参数，具体参数及配置请参考 [创建虚拟机](/virtualization/virtualization/virtual_machine/functions/create_virtual_machine.mdx)。
...
6. 将 spec.template.spec.domain.devices.disks 字段下的配置替换为下述内容。
...
7. 在 spec.template.spec.volumes 字段下添加下述内容。
...
9. 单击 **创建**。
10. 单击 **操作** > **VNC 登录**。
11. 当出现 **press any key boot from CD or DVD** 提示信息后，键入任意键，进入 Windows 安装程序；若没有看到提示信息，则单击页面左上角 **发送远程命令**，在弹出的下拉菜单中，单击 **Ctrl-Alt-Delete**，重启服务器。
...
3. 单击列表中 **虚拟机名称** 进入详情信息页面。
4. 单击 **停止**。
5. 单击右上角 **操作** > **更新**。
6. 切换至 YAML。
7. 确认 spec.template.spec.domain.devices.disks 字段下名称为 **rootfs** 的盘，其 bootOrder 为 1，若不为 1 则修改为 1。
8. 删除 spec.template.spec.domain.devices.disks 字段下名称为 **containerdisk** 的相关内容，具体删除内容如下。
    ...
    ```yaml
                - bootOrder: 1
                  ...
                  name: containerdisk
    ```
9. 删除 spec.template.spec.volumes 字段下名称为 **containerdisk** 的相关内容，具体删除内容如下。
