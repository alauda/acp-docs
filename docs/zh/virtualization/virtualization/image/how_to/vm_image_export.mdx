...
2. 在左侧导航栏中，单击 **虚拟化管理** > **虚拟机**。

3. 单击需要导出系统镜像的虚拟机名称，页面跳转至 Container Platform 的虚拟机详情页面。
...
2. 执行下述命令设置变量。
    ...
    ```
    参数说明：
    * **NAMESPACE**：虚拟机所在的命名空间名称，使用此名称替换 *\<namespace>* 部分。
...
1. 执行如下命令配置 mc 工具，并连接到指定的 S3 存储服务。
    ...
    ```
     参数说明：
     * ENDPOINT：S3 存储服务的地址，使用此地址替换 `<ENDPOINT>` 部分。
...
4. 镜像地址填写为 *\<ENDPOINT>*/vmdisks/disk.img.gz，需使用 S3 存储服务的地址替换 \<ENDPOINT> 部分。其他参数说明请参考 [添加虚拟机镜像](../functions/add_image.md)。
