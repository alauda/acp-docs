...
**Этот документ развертывания применим только к сценариям интеграции контейнерной платформы с системой трассировки.**

Компоненты **Tracing** и **Service Mesh** исключают друг друга. Если вы уже развернули компонент Service Mesh, сначала удалите его.

:::

Этот руководств предоставляет администраторам кластеров процесс установки системы трассировки на кластере Alauda Container Platform.
...
- Компонент `Elasticsearch` настроен для хранения данных трассировки, включая URL для доступа и информацию `Basic Auth`.

## Установка оператора Jaeger

### Установите оператор Jaeger с помощью веб-консоли

Вы можете установить оператор Jaeger из раздела **Управление App Store** → **Операторы** на платформе Alauda Container, где перечислены доступные операторы.

**Шаги**

- В представлении **Управление платформой** веб-консоли выберите **кластер**, в который вы хотите развернуть оператор Jaeger, затем перейдите в раздел **Управление App Store** → **Операторы**.

- Используйте поле поиска, чтобы найти `Jaeger Operator` в каталоге. Нажмите на заголовок **Jaeger Operator**.

- Прочитайте вводную информацию об операторе на странице **Jaeger Operator**. Нажмите **Развернуть**.
...
- На странице **Развертывание**:
  ...
  - Выберите **Рекомендуемое пространство имен** для **Namespace**. Установите оператор в рекомендуемом пространстве имен `jaeger-operator`, чтобы оператор мог отслеживать и быть доступным во всех пространствах имен внутри кластера.

- Нажмите **Развернуть**. Вы увидите оператор на странице **Развернутые операторы**.

- Убедитесь, что **Статус** отображается как **Развертывание выполнено успешно**, чтобы подтвердить, что оператор Jaeger был установлен правильно.
...
- Проверьте, что все компоненты оператора Jaeger были успешно установлены. Войдите в кластер через терминал и выполните следующую команду:
  ...
  **Пример вывода**
  ...
  Если поле `PHASE` отображает `Succeeded`, это означает, что оператор и его компоненты были успешно установлены.
...
- `--es-url`: URL доступа для Elasticsearch.
...
Скопируйте установочный скрипт из **ДЕТАЛИ**, войдите в кластер, в который вы хотите его установить, сохраните его как `install-jaeger.sh` и выполните после предоставления разрешений на выполнение:
...
# получение глобальной информации из ConfigMap
...
Вы можете установить оператор OpenTelemetry из раздела **Управление App Store** → **Операторы** на платформе Alauda Container, где перечислены доступные операторы.

**Шаги**

- В представлении **Управление платформой** веб-консоли выберите **кластер**, в который вы хотите развернуть оператор OpenTelemetry, затем перейдите в раздел **Управление App Store** → **Операторы**.

- Используйте поле поиска, чтобы найти `OpenTelemetry Operator` в каталоге. Нажмите на заголовок **OpenTelemetry Operator**.

- Прочитайте вводную информацию об операторе на странице **OpenTelemetry Operator**. Нажмите **Развернуть**.

- На странице **Развертывание**:

  - Выберите **Ручной** для **Стратегии обновления**. Для стратегии одобрения `Ручной` OLM создаст запросы на обновление. Как администратор кластера, вы должны вручную одобрить запросы на обновление OLM, чтобы обновить оператор до новой версии.
  - Выберите **alpha канал**.
  - Выберите **Рекомендуемое пространство имен** для **Namespace**. Установите оператор в рекомендуемом пространстве имен `opentelemetry-operator`, чтобы оператор мог отслеживать и быть доступным во всех пространствах имен внутри кластера.

- Нажмите **Развернуть**. Вы увидите оператор на странице **Развернутые операторы**.

- Убедитесь, что **Статус** отображается как **Развертывание выполнено успешно**, чтобы подтвердить, что оператор OpenTelemetry был установлен правильно.
...
Скопируйте установочный скрипт из **ДЕТАЛИ**, войдите в кластер, в который вы хотите его установить, сохраните его как `install-otel.sh` и выполните после предоставления разрешений на выполнение:
...
## Включить переключатель функций

Система трассировки в настоящее время находится на стадии **Alpha** и требует, чтобы вы вручную включили переключатель функций `acp-tracing-ui` в представлении **Переключатель функций**.

Затем перейдите к представлению **Container Platform** и перейдите в **Observability** → **Tracing**, чтобы просмотреть функцию трассировки.
...
Вы можете удалить оператор OpenTelemetry, используя представление **Управление платформой** в веб-консоли.

**Шаги**

- С страницы **Управление App Store** → **Операторы** → **Развернутые операторы** используйте **поле поиска**, чтобы найти `opentelemetry-operator`.
- Нажмите на имя **opentelemetry-operator**, чтобы перейти к его деталям.
- На странице с деталями **opentelemetry-operator** нажмите **Действия** → **Удалить оператор**.
...
Вы можете удалить оператор Jaeger, используя представление **Управление платформой** в веб-консоли.

**Шаги**

- С страницы **Управление App Store** → **Операторы** → **Развернутые операторы** используйте **поле поиска**, чтобы найти `jaeger-operator`.
- Нажмите на имя **jaeger-operator**, чтобы перейти к его деталям.
- На странице с деталями **jaeger-operator** нажмите **Действия** → **Удалить оператор**.
