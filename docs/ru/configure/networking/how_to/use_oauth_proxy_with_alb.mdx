...
Следуйте этим шагам, чтобы использовать функционал:
...
- Создайте [приложение oauth от github](https://docs.github.com/en/apps/oauth-apps/building-oauth-apps/creating-an-oauth-app)
  - Обратите внимание, что `$GITHUB_CLIENT_ID` и `$GITHUB_CLIENT_SECRET` будут получены на этом этапе, которые необходимо установить в переменных окружения
- Настройте dns
  - Здесь мы используем echo.com в качестве домена приложения, auth.alb.echo.com и alb.echo.com
- Разверните oauth-proxy

> oauth2-proxy необходимо получить доступ к github, что может потребовать настройки переменной окружения HTTPS_PROXY
...
- Мы настроим два ingress'а: auth.alb.echo.com и alb.echo.com
...
- После доступа по адресу alb.echo.com вы будете перенаправлены на страницу аутентификации github, и после проверки вы сможете увидеть вывод приложения.
