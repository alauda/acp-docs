...
# Решение для балансировки нагрузки в программном центре обработки данных (Alpha)

Разверните чисто программный балансировщик нагрузки (LB) в центре обработки данных, создав высокодоступный балансировщик нагрузки вне кластера, предоставляющий возможности балансировки нагрузки для нескольких ALB, чтобы обеспечить стабильную работу бизнеса. Он поддерживает конфигурацию только для IPv4, только для IPv6 или для обоих IPv4 и IPv6 в режиме двойного стека.
...
2. Предварительно установите следующее программное обеспечение на всех узлах-хостах внешнего LB (в этой главе в качестве примера рассматриваются два узла-хоста внешнего LB):
...
**Примечание**: Следующие операции должны быть выполнены один раз на каждом узле-хосте внешнего LB, и `hostname` узлов-хостов не должен дублироваться.
...
5. Измените элементы конфигурации в соответствии с комментариями в следующем файле и сохраните их в папке `/etc/keepalived/`, назвав файл `alive.yaml`.
   ...
   instances:
     - vip: # Можно настроить несколько VIP
         ...
         interface: "eth0"
         check_interval: 1 # необязательный, по умолчанию 1: интервал для выполнения проверочного скрипта
         check_timeout: 3  # необязательный, по умолчанию 3: период времени ожидания для проверочного скрипта
         name: "vip-1" # Идентификатор этого экземпляра, может содержать только алфавитно-цифровые символы и дефисы, не может начинаться с дефиса
         peer: [ "192.168.128.116", "192.168.128.75" ] # IP узла Keepalived, фактически сгенерированный keepalived.conf удалит все IP на интерфейсе https://github.com/osixia/docker-keepalived/issues/33
         kube_lock:
       ...
       ipvs: # Конфигурация для опции IPVS
         ips: [ "192.168.143.192", "192.168.138.100","192.168.129.100" ] # IPVS бэкенд, измените IP узла k8s master на IP узла ALB
...
6. Выполните следующую команду в бизнес-кластере, чтобы проверить дату истечения сертификата в конфигурационном файле, убедившись, что сертификат все еще действителен. Функциональность LB станет недоступной после истечения срока действия сертификата, потребуется связаться с администратором платформы для обновления сертификата.

   ```
   openssl x509 -in <(cat /etc/kubernetes/admin.conf | grep client-certificate-data | awk '{print $NF}' | base64 -d ) -noout -dates
   ```

7. Скопируйте файл `/etc/kubernetes/admin.conf` с трех узлов Master в кластере Kubernetes в папку `/etc/keepalived/kubecfg` на узлах внешнего LB, назвав их с индексом, например, `kubecfg01.conf`, и измените адреса узлов `apiserver` в этих трех файлах на фактические адреса узлов кластера Kubernetes.

   **Примечание**: После обновления сертификата платформы этот шаг необходимо выполнить снова, перезаписав оригинальные файлы.
...
2. Выключите узел LB, на котором находится VIP, и проверьте, может ли VIP как для IPv4, так и для IPv6 успешно мигрировать на другой узел, обычно в течение 20 секунд.
