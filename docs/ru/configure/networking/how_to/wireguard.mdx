...
Calico поддерживает шифрование WireGuard как для IPv4, так и для IPv6 трафика, которое может быть независимо включено с помощью параметров в ресурсе FelixConfiguration.
...
| Термин                   | Объяснение                                                        |
| ---------------------- | ------------------------------------------------------------------ |
| **wireguardEnabled**   | Включить шифрование для трафика IPv4 через подложную сеть IPv4. |
| **wireguardEnabledV6** | Включить шифрование для трафика IPv6 через подложную сеть IPv6. |

## Примечания

1. При использовании сетевого плагина Calico убедитесь, что параметр `natOutgoing` установлен в `true`, чтобы поддерживать шифрование WireGuard. По умолчанию этот параметр правильно настроен для подсети Calico при создании кластера, не требуя дополнительной конфигурации.

2. WireGuard поддерживает шифрование как для трафика IPv4, так и для IPv6; если вам нужно зашифровать оба типа трафика, конфигурация должна быть выполнена отдельно. Для подробной настройки параметров обратитесь к [документации по конфигурации Felix](https://docs.tigera.io/calico/latest/reference/resources/felixconfig#felix-configuration-definition), настраивая параметры `wireguardEnabled` и `wireguardEnabledV6`.

3. Если WireGuard не установлен по умолчанию, обратитесь к [руководству по установке WireGuard](https://www.wireguard.com/install/) для ручной установки, хотя в некоторых случаях ручная установка модуля WireGuard может не сработать.

4. Трафик между контейнерами на разных узлах будет зашифрован, включая сетевой трафик от одного хоста к другому; однако связь между Pods на одном узле и трафик между Pod и узлом его хоста не будут зашифрованы.

## Предварительные условия

- WireGuard должен быть установлен на всех узлах в кластере заранее. Для получения подробной информации обращайтесь к [документации по установке WireGuard](https://www.wireguard.com/install/). Узлы без установленного WireGuard не поддерживают шифрование.

## Шаги

1. Включите или отключите шифрование IPv4 и IPv6.

   **Примечание**: Следующие команды должны выполняться в инструменте CLI на главном узле, где находится узел.
   ...
   - Отключить шифрование IPv4 и IPv6

     - Метод 1: Выполните команду в инструменте CLI для отключения шифрования.
     ...
     - Метод 2: Измените файл конфигурации felixconfiguration, чтобы отключить шифрование.
       ...
       2. Установите параметры `wireguardEnabled` и `wireguardEnabledV6` в значение false, чтобы отключить шифрование WireGuard.
...
2. После завершения настройки шифрования WireGuard в Calico выполните следующую команду, чтобы подтвердить статус шифрования WireGuard. Если шифрование IPv4 и IPv6 включено, наличие `wireguardPublicKey` или `wireguardPublicKeyV6` в поле `Status` указывает на успешную активацию; если шифрование IPv4 и IPv6 отключено, эти поля не будут содержать `wireguardPublicKey` или `wireguardPublicKeyV6`, что указывает на успешное отключение.
...
1. После настройки шифрования WireGuard проверьте маршрутизацию, где трафик между узлами предпочтительно использует интерфейс wireguard.cali для пересылки сообщений.

   ```bash
   root@test:~# ip rule   # Просмотреть текущие правила маршрутизации
        0: from all lookup local
        99:  not from all fwmark 0x100000/0x100000 lookup 1    # Для всех пакетов, не отмеченных как 0x100000, используйте таблицу маршрутизации 1 для поиска маршрутов
        32766:  from all lookup main
        32767 :  from all lookup default
    
   root@test:~# ip route show table 1    # Показать записи маршрута для таблицы 1.
   ...
   root@test:~# ip r get 10.10.10.202    # Маршрут от текущего узла до целевого IP-адреса 10.10.10.202
       10.10.10.202 dev wireguard.cali table 1 src 10.10.10.127 uid 0  cache   # При доступе к целевому IP-адресу 10.10.10.202 с текущего узла пакет будет отправлен через интерфейс wireguard.cali, используя таблицу маршрутизации 1, и исходный адрес будет установлен на 10.10.10.127
...
2. Захватите пакеты на узле, чтобы наблюдать за межузловым трафиком.
   ...
   root@test:~# tcpdump -i wireguard.cali -nnve icmp   # Захватите и отобразите пакеты ICMP через wireguard.cali
...
3. Тестирование показывает, что трафик типа IPv4 перенаправляется через интерфейс wireguard.cali.
